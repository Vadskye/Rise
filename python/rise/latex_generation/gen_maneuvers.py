    ...
    for rank in maneuvers_by_rank.keys():
        ...
        for maneuver in maneuvers:
            try:
                ...
                raise Exception(
                    f"Error converting maneuver '{maneuver.name}' to LaTeX"
                ) from e
    return latexify("\n".join(maneuver_texts))
...
def latex_for_source(source, maneuvers):
    ...
    for maneuver in sorted(maneuvers, key=lambda m: str(m.rank) + m.name):
        maneuver_headers.append(
            f"\\maneuverhead[{maneuver.rank}]<{maneuver.name}> {maneuver.short_description}."
        )
...
def write_to_file():
    ...
    with open(
        book_path("maneuver_descriptions.tex"), "w"
    ) as maneuver_descriptions_file:
