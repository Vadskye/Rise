...
import click
from rise.simulator.run_combat import format_combat_results, run_combat
from rise.simulator.creature_parsing import parse_leveled_creature_groups

def output_combat(blue_descriptions, red_descriptions, levels=None):
    creature_groups = parse_leveled_creature_groups(blue_descriptions, red_descriptions, levels)
    for group in creature_groups:
        results = run_combat(group['blue'], group['red'])
        print(f"'{blue_descriptions}' vs '{red_descriptions}'"
...
def main(blue_grep, grep, levels, modifiers, red_grep):
    ...
    output_combat(blue_descriptions, red_descriptions, levels)
