...
\condition{Blinded} A blinded creature cannot see. It automatically fails at actions which depend on vision, including simply seeing the locations of other objects and creatures (but see \pcref{Perception}). It is \severelyimpaired at any vision-related actions, such as physical attacks and targeted spells, even if it knows the location of its targets.

In addition, a blinded creature is defenseless, causing it to provoke attacks of opportunity for its actions.
...
\condition{Bloodied} At or below half hit points. Bloodied creatures take a \minus5 penalty to Fortitude and Will defense.
...
\condition{Dead} A dead creature's soul leaves its body. Dead creatures cannot benefit from normal or magical healing, but they can be restored to life via magic (see \pcref{Bringing Back the Dead}). A dead body decays normally unless magically preserved.
...
\condition{Frightened} A frightened creature flees by any means necessary if it is within 30 feet of the source of its fear. If unable to flee, it may fight.
...
\condition{Helpless} A helpless creature is completely at an opponent's mercy. Its physical defenses are equal to 10 \add its size modifier. In addition, it cannot take attacks of opportunity. Paralyzed, bound, and unconscious creatures are helpless. Helpless creatures can be killed instantly by a coup de grace (see \pcref{Coup de Grace}).
...
\condition{Impaired} An impaired creature has a 20\% chance to fail at a particular kind of action. The type of action that is impaired is defined in the ability which impairs the creature. For example, a creature with a visual impairment suffers a 20\% chance of failure at all tasks that depend on sight.
...
\condition{Negative Levels} A creature with a negative level takes a \minus1 penalty to attacks, special defenses, and checks. Additionally, the creature's maximum and current hit points are reducedby 5. If the creature has at least as many negative levels as it has levels, it dies.
...
\condition{Shaken} A shaken creature is unable to move closer than 30 feet towards the source of its fear. It can otherwise act normally.
