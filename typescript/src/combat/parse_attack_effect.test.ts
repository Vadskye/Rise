...
tap.test('parseAttackEffect', (t) => {
  ...
  t.test('Mending', (t) => {
    ...
    // dr4 is 2d6 + power. Power 3 -> +3.
    // Base dice for dr4 is 2d6. So 2d6+3.
    t.equal(
      parsed?.damage.toString(),
      '2d6+3',
      'should calculate correct healing (dr4 for power 2)',
    );
    t.end();
  });
  ...
  t.test('Flame Breath', (t) => {
    ...
    t.equal(parsed?.damage.toString(), '2d6+3', 'should calculate correct damage');
    t.end();
  });
