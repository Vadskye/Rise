...
function printSpell(s: SpellDetail, reason: string) {
  ...
  if (s.narrative) {
    console.log(`  Narrative: ${s.narrative.trim()}`);
  }
  if (s.effect) {
    console.log(`  Effect: ${s.effect.trim()}`);
  }
  if (s.attackHit) {
    console.log(`  Hit: ${s.attackHit.trim()}`);
  }
  if (s.attackTargeting) {
    console.log(`  Targeting: ${s.attackTargeting.trim()}`);
  }
  console.log('---');
}
...
for (const s of allSpells) {
  ...
  const manifestsObjectOrCreature =
    ...
    text.includes('servant') ||
    text.includes('weapon') ||
    text.includes('armor') ||
    text.includes('skin') ||
    text.includes('carapace') ||
    text.includes('stick') ||
    text.includes('club');
  ...
  const physicalRestraint =
    text.includes('trap') ||
    text.includes('entangle') ||
    text.includes('pin') ||
    text.includes('bind') ||
    text.includes('grapple') ||
  ...
  if (hasManif && !hasPhys) {
    if (
      ...
      printSpell(
        s,
        'Has Manifestation, missing Physical (creates physical objects/structures/restraints/coatings)',
      );
    }
  }
  ...
  if (hasPhys && !hasManif) {
    if (
      manifestsObjectOrCreature ||
      createsPhysicalStructure ||
      s.type?.includes('Sustain') ||
      s.type?.includes('Attune')
    ) {
      printSpell(
        s,
        'Has Physical, missing Manifestation (creates temporary physical objects/forms/effects)',
      );
    }
  }
