...
for (const s of baseSpells) {
  ...
  const createsTemporaryObjectOrBarrier = 
    (hasBarrier && !hasManif) ||
    textLower.includes('creates a wall') || textLower.includes('create a wall') ||
    textLower.includes('creates a huge grove') || textLower.includes('grows around') ||
    textLower.includes('creates a small creature') || textLower.includes('summon') ||
    textLower.includes('creates a circular platform') || textLower.includes('creates a temporary') ||
    textLower.includes('transforms an unattended') || textLower.includes('grows from nowhere') ||
    textLower.includes('embeds itself in a foe and grows') || textLower.includes('seeds') ||
    textLower.includes('barrier') || textLower.includes('shelter') || textLower.includes('cage');
  ...
  // Cryomancy (Ice structures / walls / cages / armor)
    ...
    if (s.name.includes('Wall of Ice') || s.name.includes('Ice Barrier') || s.name.includes('Glacier') || textLower.includes('wall of ice') || textLower.includes('ice wall')) {
      if (!hasPhys) missing.push('Physical');
      if (!hasManif) missing.push('Manifestation');
      reason = 'Creates a physical wall/barrier of solid ice with HP that physically blocks line of sight/effect and movement.';
    } else if (s.name.includes('Ice Cage') || textLower.includes('trapped in ice') || textLower.includes('encased in ice') || textLower.includes('ice encases')) {
      if (!hasPhys) missing.push('Physical');
      if (!hasManif) missing.push('Manifestation');
      reason = 'Encases target in a solid physical cage/block of ice (Physical; incorporeal creatures pass through ice).';
    } else if (s.name.includes('Ice Armor') || s.name.includes('Frost Armor') || textLower.includes('coat of ice')) {
      if (!hasPhys) missing.push('Physical');
      if (!hasManif) missing.push('Manifestation');
  ...
  // Terramancy (Earth / Stone walls, armors, cages, missiles)
    ...
    if (s.name === 'Wall of Stone') {
      if (!hasPhys) missing.push('Physical');
      reason = 'Creates a physical wall of solid stone (has Manifestation & Earth, but missing Physical). Incorporeal creatures pass through solid stone.';
    } else if (textLower.includes('wall of earth') || textLower.includes('earthen wall') || textLower.includes('stone wall') || textLower.includes('pillar of stone')) {
      if (!hasPhys) missing.push('Physical');
      if (!hasManif) missing.push('Manifestation');
      reason = 'Creates physical stone/earth barriers or structures.';
    } else if (s.name.includes('Sinkhole') || s.name.includes('Quicksand') || textLower.includes('entombs') || textLower.includes('swallows')) {
      if (!hasPhys) missing.push('Physical');
      reason = 'Traps/buries targets in physical earth/mud (should not affect incorporeal creatures).';
    }
  }

  // Fabrication (Created items, barriers, gadgets, weapons)
  else if (s.sphere === 'Fabrication') {
    if (textLower.includes('wall') || textLower.includes('barrier') || textLower.includes('bridge') || textLower.includes('sphere')) {
      if (hasManif && !hasPhys && (textLower.includes('solid') || textLower.includes('physical') || textLower.includes('wood') || textLower.includes('metal') || textLower.includes('stone') || textLower.includes('shrapnel'))) {
  ...
  // Polymorph (Body modifications & physical transformations)
    ...
    if (!hasPhys) {
      missing.push('Physical');
      reason = 'Polymorph ability modifying physical anatomy/body (should be tagged Physical like almost all Polymorph spells).';
    }
    if (textLower.includes('manifest') || textLower.includes('creates') || textLower.includes('grows new') || textLower.includes('duplicate')) {
      if (!hasManif) {
  ...
  // Vivimancy (Flesh, bone, blood, biological constructs)
    ...
    if (textLower.includes('bone') || textLower.includes('flesh') || textLower.includes('organ') || textLower.includes('claw') || textLower.includes('carapace')) {
      if (!hasPhys) missing.push('Physical');
      if (textLower.includes('creates') || textLower.includes('grows') || textLower.includes('forms')) {
        if (!hasManif) missing.push('Manifestation');
      }
