...
for (const s of baseSpells) {
  ...
  if (s.name === 'Treeseal') {
    ...
    reason =
      'Creates a physical grove of trees with HP/defenses that traps the target (Physical; incorporeal entities are unaffected) and is formed temporarily from magic (Manifestation).';
  }
  // 2. Shillelagh - Transforms existing stick, so ONLY Physical, NOT Manifestation
    ...
    reason =
      'Transforms an existing physical stick into a weapon. Since it alters an existing object rather than creating a new temporary object from raw magic, it is Physical but NOT Manifestation.';
  }
  // 3. Barkskin & Greater Barkskin - Enhances physical armor/body
    ...
    reason =
      "Hardens the target's physical body/skin into bark granting an Armor bonus (Physical armor enhancement).";
  }
  // 4. Embedded Growth
    ...
    reason =
      'Embeds a physical seed into flesh (Physical) that manifests temporary plant growth/undergrowth as it erupts (Manifestation).';
  }
  // 5. Entangle, Entangling Field, Tripping Vine - Have Manifestation, missing Physical
    ...
    reason =
      'Plants grow from nowhere to physically restrain/slow grounded targets. Since incorporeal creatures cannot be held by physical vines, these require Physical.';
  }
  // 6. Wall of Thorns, Wall of Ice, Ice Globe, Wall of Stone
  else if (
    s.name === 'Wall of Thorns' ||
    s.name === 'Wall of Ice' ||
    s.name === 'Ice Globe' ||
    s.name === 'Wall of Stone'
  ) {
    if (!hasPhys) {
      missing.push('Physical');
    }
    reason =
      'Creates a physical wall/structure of thorns, ice, or stone with HP. Incorporeal entities pass through physical walls without impediment.';
  }
  // 7. Icecraft & Ice Shield
  else if (
    s.sphere === 'Cryomancy' &&
    (s.name === 'Icecraft' || s.name === 'Ice Shield' || s.name === 'Greater Ice Shield')
  ) {
    if (!hasPhys) {
      missing.push('Physical');
    }
    ...
    reason =
      'Creates temporary physical weapons, armor, or shields out of ice/water that vanish when destroyed or duration ends.';
  }
  // 8. Swallowed by Earth
    ...
    reason =
      'Traps/buries targets in physical earth/rock (incorporeal creatures are immune to physical earth trapping).';
  }
