...
console.log(`Loaded ${allSpells.length} total spell entries (${uniqueSpells.length} unique base spells/cantrips/rituals).\n`);
...
for (const sphere of spheres) {
  ...
  for (const s of sphereSpells) {
    ...
    console.log(`- ${s.name} [Rank ${s.rank}] | Tags: [${s.tags.join(', ')}] | Type: ${s.type || 'Standard'}`);
    if (s.narrative) console.log(`  Narrative: ${s.narrative}`);
    if (s.effect) console.log(`  Effect: ${s.effect}`);
    if (s.attackHit) console.log(`  Hit: ${s.attackHit}`);
    if (s.attackTargeting) console.log(`  Targeting: ${s.attackTargeting}`);
    console.log('');
  }
}
