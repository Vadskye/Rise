...
t.test('recalculation on sheet:opened', (t) => {
    t.test('recalculates trained skills', (t) => {
        ...
        t.equal(sheet.getPropertyValue('awareness_is_trained'), '1', 'Initial set should work via sheet:opened');
        ...
        t.equal(sheet.getPropertyValue('awareness_is_trained'), '1', 'Recalculation should restore trained status');
        t.end();
    });
    ...
    t.test('recalculates attuned effects', (t) => {
        ...
        t.equal(sheet.getPropertyValue('active_attunement_count'), 1, 'Initial set should work via sheet:opened');
        ...
        t.equal(sheet.getPropertyValue('active_attunement_count'), 1, 'Recalculation should restore attunement count');
        t.end();
    });
    ...
    t.test('recalculates vital wounds', (t) => {
        ...
        t.equal(sheet.getPropertyValue('all_defenses_vital_wound_modifier'), -1, 'Initial set should work via sheet:opened');

        sheet.setProperties({ 'all_defenses_vital_wound_modifier': 0 });
        t.equal(sheet.getPropertyValue('all_defenses_vital_wound_modifier'), 0, 'Manual overwrite should work');

        sheet.triggerRecalculation();

        t.equal(sheet.getPropertyValue('all_defenses_vital_wound_modifier'), -1, 'Recalculation should restore vital wound penalty');
        t.end();
    });
