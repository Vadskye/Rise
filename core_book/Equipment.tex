...
\section{Weapons}\label{Weapons}
    ...
    \subsection{Weapon Tags}\label{Weapon Tags}
        ...
        \weapontagdef{Forceful} Whenever you deal damage to a target with a strike using this weapon, you can \glossterm{knockback} the target up to 10 feet.
        ...
        % 20% chance of +5.5a is slightly better than +1a, but only for getting critical hits
        \weapontagdef{Impact} If you roll an 8 or 9 on an attack roll with this weapon, the attack roll \glossterm{explodes} (see \pcref{Exploding Attacks}).
        ...
        \weapontagdef{Massive} This weapon cannot be used to attack individual creatures.
        Instead, a strike with a Massive weapon affects everything in a cube.
        Unlike a \abilitytag{Sweeping} weapon, this is an area attack, not a \glossterm{targeted} attack.
        On a miss, each target still suffers a \glossterm{glancing blow}.
    ...
    \subsection{Weapon Table}
        ...
        \begin{longtablewrapper}
            ...
            \begin{longtable}{p{12em} c c >{\ccol}p{7em} c >{\ccol}p{16em}}
                ...
                \tb{Exotic Weapons}
                \label{cap:Exotic Weapons}        & \tb{Accuracy} & \tb{Damage} & \tb{Damage Type}         & \tb{Item Rank (Cost)}\fn{1} & \tb{Weapon Tags} \tableheaderrule
                Armor                             &               &             &                          &                             &                                             \\
    ...
    \subsection{Weapon Special Materials}\label{Weapon Special Materials}
        ...
        \subsubsection{Weapon Special Material Prices}
            ...
            This is the price for an amount of the special material sufficient to forge a non-\weapontag{Heavy} weapon, when combined with other normal weaponsmithing materials.
        ...
        \begin{dtable!*}
            ...
            \begin{dtabularx}{\textwidth}{l >{\lcol}X l}
                ...
                % object damage
                \tind Adamantine          & \plus1d, double damage to objects         & 4 (5,000 gp)   \\
                \tind Adamantine, pure    & \plus2d, double damage to objects         & 6 (125,000 gp) \\
                % just vulnerabilities
                \tind Cold iron           & Common vulnerabilities                    & 2 (200 gp)     \\
                % crit accuracy
                \tind Diamondsteel        & \plus2 accuracy with critical hits        & 3 (1,000 gp)   \\
                \tind Diamondsteel, pure  & \plus4 accuracy with critical hits        & 5 (25,000 gp)  \\
                % energy damage
                ...
                \tind Mithral             & Lighter, \minus1d damage                  & 3 (1,000 gp)   \\
                \tind Mithral, pure       & \plus1 accuracy, lighter, \minus2d damage & 5 (25,000 gp)  \\
                % just vulnerabilities  
                \tind Silvered            & Common vulnerabilities                    & 2 (200 gp)         \\
            \end{dtabularx}
        \end{dtable!*}

        \parhead{Adamantine} An adamantine weapon has a \plus1d bonus to its damage dice.
        In addition, \glossterm{strikes} with it deal double damage to objects that are not made of pure or ordinary adamantine.

        \parhead{Adamantine, Pure} A pure adamantine weapon has a \plus2d bonus to its damage dice.
        ...
        However, mithral weapons have a \minus1d penalty to their \glossterm{weapon damage}.
        ...
        A Heavy mithral weapon loses that tag and instead gains the \weapontag{Versatile Grip} weapon tag.
        However, pure mithral weapons have a \minus2d penalty to their \glossterm{weapon damage}.
...
\section{Armor}\label{Armor}
    ...
    \subsection{Armor Table}
        ...
        \begin{dtable!*}
            ...
            \begin{dtabularx}{\textwidth}{l c c c c c >{\lcol}X c}
                ...
                Medium armor          &              &                        &                  &              &                &               &              \\
                \tind Hide            & \plus3       & \plus4                 & \plus3           & \tdash       & \mult1/2       & Leather       & 1 (40 gp)    \\
    ...
    \subsection{Armor Special Materials}\label{Armor Special Materials}
        ...
        \subsubsection{Armor Special Material Prices}
            ...
            This is the price for an amount of the special material sufficient to forge light armor, when combined with other normal armorsmithing materials.
            Medium body armor requires twice as much of the special material, and heavy body armor requires three times as much of the special material.

        % The baseline value for magic armor multiplier for (light/medium/heavy):
        % * R1-3: (3/5/10)
        % * R4: (6/10/20)
        % * R5: (9/15/30)
        % * R6: (12/20/40)
        % * R7: (18/30/60)
        % Using optimal deepforged/elvenweave/dragonhide:
        % * R3: (6/8/12)
        % * R4: (9/12/18)
        % * R5: (12/16/24)
        % * R6: (18/24/32)
        ...
        % Alternate special material calcs
        % * R3: (6/10/20)
        % * R4: (9/15/30)
        % * R5: (12/20/40)
        % * R6: (18/30/60)
        ...
        \begin{dtable!*}
            ...
            \begin{dtabularx}{\textwidth}{l l l l >{\ccol}X l}
                ...
                Adamantine                 & \mult4                 & \plus2           & \tdash                                 & Metal         & 4 (5,000 gp)   \\
                Adamantine, pure           & \mult8                 & \plus2           & \tdash                                 & Metal         & 6 (125,000 gp) \\
                % magic defense                                                                                                                  
                Cold iron\fn{1}            & \tdash                 & \tdash           & \minus1 Armor, \plus1 defense vs magic & Metal         & 2 (200 gp)     \\
                Cold iron, pure\fn{1}      & \mult2                 & \tdash           & \minus1 Armor, \plus2 defense vs magic & Metal         & 4 (5,000 gp)   \\
                % metal resistance                                                                                                               
                Deepforged                 & \mult2                 & \tdash           & \tdash                                 & Metal         & 3 (1,000 gp)   \\
                Deepforged, pure           & \mult4                 & \tdash           & \tdash                                 & Metal         & 5 (25,000 gp)  \\
                % metal crit resistance                                                  
                Diamondsteel\fn{1}         & \tdash                 & \tdash           & \plus4 defense vs strike crits         & Metal         & 3 (1,000 gp)   \\
                Diamondsteel, pure\fn{1}   & \mult2                 & \tdash           & \plus4 defense vs crits                & Metal         & 5 (25,000 gp)  \\
                ...
                Dragonhide, ancient\fn{1}  & \mult6                 & \tdash           & Immune to specific damage type         & Leather       & 6 (125,000 gp) \\
                ...
                Dragonscale, ancient\fn{1} & \mult6                 & \tdash           & Immune to specific energy type         & Metal         & 4 (5,000 gp)   \\
                % leather resistance                                                             
                Elvenweave                 & \mult2                 & \tdash           & \tdash                                 & Leather       & 3 (1,000 gp)   \\
                Elvenweave, pure           & \mult4                 & \tdash           & \tdash                                 & Leather       & 5 (25,000 gp)  \\
                % metal encumbrance reduction                                            
                Mithral                    & \tdash                 & \minus2          & \tdash                                 & Metal         & 3 (1,000 gp)   \\
                Mithral, pure\fn{1}        & \mult2                 & \minus2          & Improve Dex multiplier                 & Metal         & 5 (25,000 gp)  \\
                % metal resistance, recovery, encumbrance penalty                        
                Starmetal\fn{1}            & \mult2                 & \plus2           & Recover damage resistance              & Metal         & 3 (1,000 gp)   \\
                Starmetal, pure\fn{1}      & \mult4                 & \plus2           & Recover damage resistance              & Metal         & 5 (25,000 gp)  \\
            \end{dtabularx}
        ...
        You are \trait{impervious} to damage of the type dealt by that dragon's breath weapon (see \tref{Dragon Types}).
        In addition, it provides \mult3 \glossterm{damage resistance} and is not considered to be metal, which may affect abilities like the \spell{heat metal} spell.

        \parhead{Dragonscale, Ancient} Each ancient dragonscale body armor is made from the scales of a particular type of dragon.
        You are \trait{immune} to damage of the type dealt by that dragon's breath weapon (see \tref{Dragon Types}).
        In addition, it provides \mult6 \glossterm{damage resistance} and is not considered to be metal, which may affect abilities like the \spell{heat metal} spell.

        \parhead{Mithral, Pure} Pure mithral body armor increases the contribution of your Dexterity to your Armor defense relative to the normal limit for that type of armor.
        A multiplier of \mult1/2 becomes \mult1, and a multiplier of \mult0 becomes a \mult1/2.
        Ina ddition, it provides \mult2 \glossterm{damage resistance} and reduces the \glossterm{encumbrance} of the armor by 2.

        \parhead{Starmetal} When you use the \ability{recover} ability while wearing starmetal body armor, you also regain an amount of \glossterm{damage resistance} equal to half the damage resistance provided by the armor.
        This effect is \abilitytag{Swift}, like the \ability{recover} ability.
        In addition, it provides \mult2 \glossterm{damage resistance} and increases the \glossterm{encumbrance} of the armor by 2.

        \parhead{Starmetal, Pure} When you use the \ability{recover} ability while wearing pure starmetal body armor, you also regain an amount of \glossterm{damage resistance} equal to half the damage resistance provided by the armor.
        This effect is \abilitytag{Swift}, like the \ability{recover} ability.
        In addition, it provides \mult4 \glossterm{damage resistance} and increases the \glossterm{encumbrance} of the armor by 2.
...
\section{Magic Implements}\label{Implements}
    ...
    However, it does not remove the possibility of \glossterm{somatic component failure}.
