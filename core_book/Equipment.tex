...
You can gain proficiency with exotic weapons with the Weapon Focus feat (see page \featpref{Weapon Focus}).
...
In addition, light weapons are easier to use while making dual strikes (see \pcref{Dual Strikes}) or while grappling.
...
A creature with multiple natural weapons of the same type can designate one as a primary attack and one as a secondary attack, allowing it to fight with both at once (see \pcref{Dual Strikes}). You are only considered to have one unarmed strike, so you cannot dual wield with only your unarmed strike (but see the unarmed warrior monk ability, \pref{Mnk:Unarmed Warrior}).
...
\parhead{Double} This weapon has more than one striking surface. You can fight with both ends simultaneously, just like wielding two weapons at once (see \pcref{Dual Strikes}). Alternately, you can attack with one end at a time. If you have the ability to use a double weapon in one hand, you can only fight with one end at a time, not both.
...
\parhead{Rapier} A rapier is treated as a medium weapon if it is used as a secondary weapon when making dual strikes (see \pcref{Dual Strikes}).
