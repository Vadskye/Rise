\chapter{What Makes Rise Different?}
  ...
  \subsection{Minimal Resource Management}
    ...
    There are a small number of abilities with brief cooldowns, and a universal ability that can only be used once per short rest.
    However, there is no time tracking in the system longer than ``next turn''.
    ...
    Rise does use a single universal resource, called ``stamina'', that recovers based on long rests.
  ...
  % TODO: describe grouped initiative here
  ...
  \subsection{Bounded Action Economy}
    ...
    Generally, characters can only take one move action and one standard action each turn.
    Some characters can use a minor action to accomplish something useful, but this is rare.
    That essentially marks the end of action economy scaling, even up to the maximum level.
