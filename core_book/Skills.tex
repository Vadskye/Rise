...
\skill{Awareness}{Per}
    ...
    \subsection{Discern Illusion [Hidden]}
        ...
        If the illusion is completely missing a sense that should logically be present, such as a silent illusion of people marching in heavy armor, the \glossterm{difficulty rating} to interact with the illusion with that sense is lowered by 5.
...
\skill{Balance}{Dex}
    ...
    \subsection{Agile Charge}\label{Agile Charge}
        You can make a \glossterm{difficulty rating} 15 Balance check while \glossterm{charging} to change directions while charging.
    ...
    \subsection{Balance}\label{Balance}
        ...
        The \glossterm{difficulty rating} of Balance checks to balance varies with the surface, as described in \trefnp{Balancing Difficulty Ratings}.
        In addition, if you are forcibly moved while on a slippery or narrow surface, you must make an Balance check against the same \glossterm{difficulty rating}.\@
    ...
    \subsection{Rapid Stand}\label{Rapid Stand}
        ...
        \begin{freeability}{Rapid Stand}[Swift]
            You make a \glossterm{difficulty rating} 15 Balance check to stand up from a prone position quickly.
...
\skill{Climb}{Str}
    ...
    \subsection{Wallrun}\label{Wallrun}
        As part of movement, you can make a Climb check to run along a wall rather than climbing it. The \glossterm{difficulty rating} is 5 higher than normal for the wall, but this does not require free hands. Success means you move up to your land speed horizontally, or up to a half of your land speed vertically. Failure means you fall. Critical failure means you are prone when you land. For every round you spend running on a wall, the \glossterm{difficulty rating} increases by 10.
...
\skill{Craft}{Int}
    ...
    \subsection{Create Item}
        ...
        In order to craft an item, you must make a Craft check against the item's Craft \glossterm{difficulty rating}, as shown on \trefnp{Craft Requirements}.
        ...
        \begin{dtable!*}
            ...
            \begin{dtabularx}{\textwidth}{>{\lcol}X l l l}
                \tb{Item}                   & \tb{Craft Skill} & \tb{Difficulty Rating} & \tb{Crafting Time} \tableheaderrule
    ...
    \subsection{Appraise Item}
        ...
        The \glossterm{difficulty rating} is equal to 5 \add the item's level.
    ...
    \subsection{Create Forgery}
        ...
        This functions like crafting the item from scratch, except that the Craft \glossterm{difficulty rating} is 5 lower than normal, and you use one-half the item's price to determine the price of raw materials and the crafting time.
...
\skill{Devices}{Int}
    ...
    With enough skill, you can even manipulate magical devices.
        ...
        The \glossterm{difficulty rating} of a Devices check depends on the device being manipulated. In addition, some actions are easier than others, and modify the \glossterm{difficulty rating} accordingly. Standard \glossterm{difficulty ratings} are listed on \trefnp{Devices Difficulty Ratings}.
    ...
    \subsection{Break Device}
        ...
        This requires thieves' tools, though you may be able to improvise from your surroundings with a \minus5 or greater penalty.
        The \glossterm{difficulty rating} is 5 lower than normal. This is generally not subtle, such as jamming a lock or breaking a trap while triggering it in the process. It cannot change the state of a device, but you can prevent it from being used normally; a locked door will remain locked, but you can prevent it from being unlocked with its key. Failure means the device continues to function. Critical failure may cause you to think that you successfully broke the device, while in fact it functions normally.
...
\skill{Endurance}{Con}
    ...
    %     The \glossterm{difficulty rating} starts at 10.
    ...
    \subsection{Hold Breath}\label{Hold Breath}
        ...
        The \glossterm{difficulty rating} starts at 5, and increases by 1 in each subsequent round.
    ...
    \subsection{Delay Vital Wound}\label{Delay Vital Wound}
        Whenever you receive a \glossterm{vital wound}, you can make \glossterm{difficulty rating} 15 Endurance check.
...
\skill{Flexibility}{Dex}
    ...
    \subsection{Tight Squeeze}
        ...
        \begin{freeability}{Tight Squeeze}
            ...
            A \glossterm{difficulty rating} 25 check allows you to move in a space that can fit your head, but not your shoulders.
...
\skill{Jump}{Str}
    ...
    \subsection{Mitigate Fall}
        ...
        For every 5 points by which you beat that \glossterm{difficulty rating}, you can reduce the falling damage by 10 additional feet.
...
\skill{Medicine}{Int}
    ...
    \subsection{Accelerate Recovery}\label{Accelerate Recovery}
        You can make a \glossterm{difficulty rating} 15 Medicine check to accelerate the recovery of up to four willing creatures, potentially including yourself, during a \glossterm{long rest}.
...
\skill{Persuasion}{---}
        ...
        The base \glossterm{difficulty rating} for a Persuasion check against a group is equal to 5 \add the highest Social Insight of any character in the group or half the highest level of any character in the group, whichever is higher.
...
\skill{Ride}{Dex}
    ...
    If a mount is not trained as a mount, the \glossterm{difficulty rating} to ride it increases by 5.
    If it lacks a saddle and other riding gear, the \glossterm{difficulty rating} to ride it increases by 5.
    If it takes a standard action other than movement, such as attacking, the \glossterm{difficulty rating} to ride it that round increases by 5.
...
\skill{Social Insight}{Per}
    ...
    \subsection{Discern Enchantment [Hidden]}
        ...
        If the creature's behavior is currently being altered by a \abilitytag{Compulsion} effect, the \glossterm{difficulty rating} is 10, and success means you identify the presence of a Compulsion effect.
        If the creature's behavior is currently being altered by an \abilitytag{Emotion} effect, the \glossterm{difficulty rating} is 20, and success means you identify the presence of an Emotion effect.
        Failure means you do not notice any such effects on the creature.

        You can also make this check to identify \abilitytag{Subtle} effects on yourself, using the same \glossterm{difficulty ratings}.
...
\skill{Spellsense}{Per}
    ...
    \subsection{Notice Magical Effect}
        ...
        The \glossterm{difficulty rating} for an \glossterm{unattended} magical item or active magical effect is equal to 15 \sub the \glossterm{power} of the effect.
        ...
        The \glossterm{difficulty rating} to notice a \abilitytag{Subtle} effect is equal to 15 \add the \glossterm{power} of the effect.
    ...
    \subsection{Identify Magical Effect}
        ...
        The \glossterm{difficulty rating} is equal to 5 \add twice the \glossterm{rank} of the effect.
    ...
    \subsection{Identify Spellcasting}
        ...
        The \glossterm{difficulty rating} is equal to 5 \add twice the \glossterm{rank} of the spell.
    ...
    \subsection{Identify Potion}
        ...
        For most potions, the \glossterm{difficulty rating} is 10, and success means you identify what spell the potion contains.
        Failure means you do not learn anything about the potion's nature.

        Potions can be crafted to conceal their true nature.
        The \glossterm{difficulty rating} to identify such potions is usually 20.
    ...
    \subsection{Identify Magical Writing}\label{Identify Magical Writing}
        ...
        If the writing describes a spell or ritual, the \glossterm{difficulty rating} is equal to 10 \add twice the \glossterm{rank} of the spell or ritual.
...
\skill{Survival}{Per}
    ...
    \subsection{Use Rope}
        You can make a Survival check as a standard action to tie knots. Success means you tie the knot successfully. Failure means your action is wasted, but you can try again. Tying a typical knot is \glossterm{difficulty rating} 5. Tying a special knot, such as one that slips, slides slowly, or loosens with a tug is \glossterm{difficulty rating} 10. Knots can also be tied with the Devices skill.
...
\skill{Swim}{Str}
    ...
    \subsection{Swimming}
        ...
        The \glossterm{difficulty rating} depends on the turbulence of the water, as shown on \trefnp{Swim Difficulty Ratings}.
