...
\skill{Creature Handling}{Per}
    ...
    \subsection{Handling Creatures}
        ...
        \begin{freeability}{\labeltext{Pacify}}[\glossterm{Compulsion}, \glossterm{Mind}]
...
\skill{Heal}{Int}
    ...
    \subsection{First Aid}
        As a standard action, you can make a DR Heal check to stabilize a dying character (see \pcref{Dying}). The target can use your Heal check in place of its Fortitude defense to avoid dying. In addition, if your check result is at least 10, the target is partially stabilized, causing it to only make \glossterm{stabilization checks} once per minute.
