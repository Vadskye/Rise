    ...
    \subsection{Class Skills}
        ...
        \begin{dtable!*}
            ...
            \begin{dtabularx}{\textwidth}{>{\lcol}p{12em} *{11}{>{\ccol}X} >{\ccol}p{4em}}
                ...
                Tumble            & C  & \tdash & \tdash & C  & C  & \tdash & C  & C  & \tdash & \tdash & \tdash & Dex    \\
                \tb{Skill}   & \tb{Bbn} & \tb{Clr} & \tb{Drd} & \tb{Ftr} & \tb{Mnk} & \tb{Pal} & \tb{Rgr} & \tb{Rog} & \tb{Sor} & \tb{Spl} & \tb{Wiz} & \tb{Key Ability} \\
                \hline
                ...
                Awareness         & C  & \tdash & C  & C  & C  & C  & C  & C  & \tdash & \tdash & \tdash & Per          \\
                ...
                Bluff             & \tdash & C  & \tdash & C  & C  & C  & C  & C  & C  & C  & \tdash & \tdash\fn{3}     \\
                Persuasion        & \tdash & C  & C  & C  & C  & C  & C  & C  & C  & C  & \tdash & \tdash\fn{3}     \\
            \end{dtabularx}
            C\@: class skill \\
...
\section{Skill Description Format}
    ...
    \subsection{Discern Illusion [Passive]}
        ...
        If the illusion is completely missing a sense that should logically be present, such as a \spell{silent image} of people marching in heavy armor, the DR to interact with the illusion with that sense is lowered by 10.
    ...
    \skill{Balance}{Dex}
        The Balance skill represents your physical steadiness and poise. All Balance checks are made as part of movement, so they require no special action to perform.
    ...
    \subsection{Secret Message}
        ...
        Anyone witnessing the exchange may make a Sense Motive check against the same DR to identify the hidden message. Creatures who know in advance that the message will be conveyed -- normally, the intended recipient -- receive a \plus10 bonus on this check. Creatures who know your system for conveying hidden messages also receive a \plus10 bonus.
    ...
    \subsection{Climb}
        ...
        \begin{dtable}
            ...
            \begin{dtabularx}{\columnwidth}{l X}
                ...
                \minus5 & Climbing a free-hanging object, such as a rope, where you can brace against a nearby wall \\
    ...
    \subsubsection{Grab Edge}\label{Grab Edge}
        ...
        If you can't reach an edge, you can jump to grab it (see \pcref{High Jump}).
    ...
    The Creature Handling skill represents your ability to handle creatures without being able to speak with them. With it, you can convince them to do what you want or train them to follow commands. This skill can only be used with creatures with an Intelligence of \minus5 or lower.
    ...
    \subsection{Handling Creatures}
        ...
        \parhead{Perform Trained Action} As a free action, you can make a DR 10 check to convince a creature to perform an action it is trained to perform. Generally, wild animals are not trained in any actions, so this is not effective on them.
    ...
    \subsection{Disguise Creature}
        ...
        Creating a disguise takes 1d4 \x 10 minutes. You can take a \minus10 penalty to reduce the time to 1d4 minutes, or a \minus20 penalty to reduce the time to 1d4 rounds.
        ...
        \begin{dtable}
            ...
            3 You cannot disguise yourself as a different size category without the Legendary Disguise feat (see page \featpref{Legendary Disguise}).
        \end{dtable}
    ...
    \skill{Jump}{Str}
        ...
        \parhead{Running Start}\label{Running Start} If you move at least twenty feet before jumping, you have a running start. Jumping without a running start is difficult, and you take a \minus10 penalty to all Jump checks.

        \parhead{Land Speed} For every 5 feet by which your land speed is slower than 30 feet, you take a \minus4 penalty to Jump checks. If you jump with a running start, for every 5 feet by which your land speed exceeds 30 feet, you gain a \plus2 bonus to your Jump checks.
    ...
    \subsection{Long Jump}\label{Long Jump}
        As part of movement, you can make a Jump check to jump forward. You move forward any number of feet, up to a maximum equal to your Jump check result. At the midpoint of the jump, you achieve a height equal to a quarter of your forward distance. If you do not have a running start, making a long jump is more difficult (see \pcref{Running Start}).

        If your movement is not stopped by hitting the ground after travelling the normal distance, such as if you jump off a ledge, you move one quarter of your jump distance farther forward as you fall before your fall becomes entirely downward. If an insufficiently long jump would cause you to fall into a gap, you can attempt to stop your fall (see \pcref{Stop Fall}) if you can reach the wall.

    \subsection{High Jump}\label{High Jump}
        As part of movement, you can make an Jump check to jump up. You can jump up any number of feet, up to a maximum equal to one quarter of your check result in feet. If you do not have a running start, making a high jump is more difficult (see \pcref{Running Start}).

        If you jumped up to grab something, you can grab it and pull yourself up with a Climb check (see \pcref{Grab Edge}).
    ...
    \skill{Perform}{Varies}
        ...
        This penalty stacks.
        For example, if you were playing a lyre, singing, and juggling balls with your feet, you could take a \minus10 penalty to your singing and a \minus20 penalty to your juggling.
    ...
    \subsection{Compel Belief}\label{Compel Belief}
        ...
        \begin{dtable}
            ...
            \begin{dtabularx}{\columnwidth}{X l}
                ...
                Virtually impossible (``You are secretly a werewolf.'')                       & \plus20 or more  \\
    ...
    \subsection{Discern Enchantment [Passive]}
        As a swift action, you can try to tell whether a creature is affected by mind-controlling magic -- including yourself. The DR to identify an Emotion effect such as \spell{charm person} is 25, while the DR to identify a Compulsion effect such as \spell{dominate person} is 15. Success means you recognize that the creature is affected by mind-affecting magic. Failure means you don't notice anything amiss.
