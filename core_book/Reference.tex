...
\section{Ability Tags}\label{Ability Tags}
  ...
  Many Brawling abilities are described in \pcref{Special Combat Abilities}.
...
\section{Circumstances and Debuffs}\label{Circumstances and Debuffs}
  ...
  \debuffdef{climbing} A creature that is climbing without a \glossterm{climb speed} takes a \minus2 penalty to its \glossterm{accuracy}, Armor defense, and Reflex defense.
  ...
  \buffdef{empowered} An empowered creature rolls damage twice whenever it deals damage and keeps the higher result.
  This only affects damage dealt directly by the creature, including conditions it inflicts.
  For example, an empowered creature that uses an ability which causes \glossterm{knockback} does not roll the falling damage from the knockback twice.
  ...
  \debuffdef{flying} A creature that is flying takes a \minus4 penalty to its Armor and Reflex defenses (see \pcref{Aerial Movement}).

  \buffdef{focused} A focused creature rolls attack rolls twice and keeps the higher result.
  This only affects attack rolls, not \glossterm{miss chances} or \glossterm{checks}.

  \buffdef{fortified} A fortified creature gains a \plus2 bonus to its Fortitude and Mental defenses.
  ...
  \debuffdef{gliding} A creature that is gliding takes a \minus4 penalty to its Armor and Reflex defenses (see \pcref{Aerial Movement}).
  ...
  \debuffdef{immobilized} An immobilized creature takes a \minus4 penalty to its Armor and Reflex defenses and can't use any of its movement modes.
  This can cause flying and gliding creatures to fall (see \pcref{Aerial Movement}).
  ...
  This only affects damage dealt directly by the creature, including conditions it inflicts.
  For example, a maximized creature that uses an ability which causes \glossterm{knockback} does not maximize the falling damage from the knockback.
  ...
  \debuffdef{slowed} A slowed creature has a \minus10 foot penalty to its speed with all movement modes and takes a \minus2 penalty to its Armor and Reflex defenses.
  ...
  \debuffdef{submerged} A creature that is in water without a \glossterm{swim speed} takes a \minus2 penalty to its \glossterm{accuracy}, Armor defense, and Reflex defense.
