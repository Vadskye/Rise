...
It treats all targets as if they had \glossterm{concealment} and it has a 50\% miss chance with all attacks and vision-related checks, even if it knows the location of its target.
...
\debuffdef{immobilized} An immobilized creature takes a \minus4 penalty to Armor and Reflex defenses and can't use any of its movement speeds.
...
In addition, it takes a \minus2 accuracy penalty with \glossterm{strikes}.
...
Subtle spells can still be identified with the Spellsense skill (see \pcref{Spellsense}), but the \glossterm{difficulty rating} is 20 higher than normal.
