...
\section{Ability Tags}\label{Ability Tags}
  ...
  Covering or plugging your ears makes you \resistant to Auditory attacks.
  Objects and \deafened creatures are immune to Auditory abilities.
  Auditory abilities only require a DV 0 Awareness check to notice.
  ...
  \abilitytagdef{Blood} Blood abilities manipulate the target's blood.
  They only work on \trait{blooded} creatures.

  \abilitytagdef{Brawling} Brawling abilities rely on brute force in hand to hand combat.
  Many Brawling abilities use your \glossterm{brawling accuracy} instead of your normal \glossterm{accuracy}.
  They always require a \glossterm{free hand} or \weapontag{Maneuverable} weapon unless otherwise noted in the ability's description.
  Common Brawling abilities are described in \pcref{Universal Combat Abilities}.

  \abilitytagdef{Clinch} You gain a \plus2 accuracy bonus with this ability against creatures that are \grappled by you or are grappling you.
  Clinch is also a \glossterm{weapon tag}.

  \abilitytagdef{Cold} Cold abilities draw energy out of their targets to cause their effects.
  ...
  They cannot be \glossterm{dismissed}, but can be removed with the \ritual{dispel curse} ritual.

  \abilitytagdef{Detection}\label{Detection} Detection abilities magically reveal information.
  Unless otherwise specified, they can penetrate up to 1 foot of solid material of any composition.
  However, they cannot penetrate lead.
  ...
  \abilitytagdef{Emotion} Emotion abilities alter a creature's opinions or personality, but do not necessarily affect their actions.
  They have no effect on anything without a mind.

  \abilitytagdef{Fire} Fire abilities tap into the elemental force of fire.
  Their effects typically create \glossterm{bright illumination} in a \smallarea radius that lingers \briefly.
  Damage from Fire abilities can set flammable objects on fire.
  Fire abilities only require a DV 0 Awareness check to notice.

  \abilitytagdef{Impact} When you miss by 5 or less with a damaging Impact ability, you still deal half damage.
  The attack is still treated as a miss, and it has no effects other than immediate damage.
  For example, the target does not suffer any conditions that would normally result from a successful hit with the ability.
  Impact is also a \glossterm{weapon tag}.
  ...
  Keen is also a \glossterm{weapon tag}.

  \abilitytagdef{Life} Life abilities only affect living things.
  Undead and nonliving objects are immune to Life abilities.

  \abilitytagdef{Manifestation} Manifestation abilities create temporary creatures and objects formed from raw magical energy.
  Something created with a Manifestation ability seems real on the surface, but it has no internal structure.
  ...
  When something created by a Manifestation ability is destroyed or killed, or when the duration of the ability that created it ends, it disappears without a trace.

  % Abilities are tagged as Physical if it seems like they shouldn't have any effect on incorporeal creatures.
  % Note that abilities that make strike are *not* generally tagged as physical.
  % Strikes are normally tagged as Physical themselves, but some special abilities make strike not be Physical, so we rely on the strike itself to be tagged.
  \abilitytagdef{Physical} Physical abilities affect physical objects and creatures.
  They do not affect and cannot be used by \trait{incorporeal} creatures.
  ...
  You cannot create a sensor in a location blocked by lead sheeting, and you sense when the effect is blocked in this way.
  ...
  \abilitytagdef{Soul} Soul abilities manipulate souls.
  They have no effect on \trait{soulless} creatures and objects.
  ...
  Subdual is also a \glossterm{weapon tag}.

  \abilitytagdef{Subtle} Subtle abilities normally have no visual or auditory effects.
  If a Subtle ability also has the \atVisual or \atAuditory tags, it can still be seen or heard.
  It's difficult to recognize that you are being affected by a Subtle ability (see \pcref{Analysis}, and \pcref{Awareness}).
  ...
  If it also includes ``attunable'', you can choose to \glossterm{attune} to the effect instead of sustaining it every turn.
  ...
  Visual abilities do not determine their \glossterm{line of effect} like other abilities.
  ...
  Visual abilities only require a DV 0 Awareness check to notice.

  Anything that cannot see a Visual effect is immune to it.
  Simply closing your eyes only makes you \resistant to Visual effects.
  Completely covering your eyes, such as with your hands or with a blindfold, makes you immune.
  Special vision abilities that replace normal vision entirely, such as \sense{blindsight} and \sense{tremorsense}, are unable to perceive Visual effects.
...
\section{Circumstances and Debuffs}\label{Circumstances and Debuffs}
  ...
  It has a 50\% \glossterm{miss chance} when trying to attack targets it cannot see.
  If a blinded creature can perceive targets with other effects that do not require eyes, such as \sense{blindsight}, it can ignore these detrimental effects.

  \buffdef{braced} A braced creature gains a \plus2 bonus to all defenses.

  \debuffdef{bleed} A bleeding creature takes damage at the end of each of its turns.
  It can stop bleeding if it makes a \glossterm{difficulty value} 8 Medicine check as a \glossterm{standard action}.
  Applying bandages as part of this action gives a +5 bonus to this check.
  You automatically stop bleeding if you take damage from a \atFire ability.

  If a creature stops bleeding at the end of its turn, it still takes damage from the bleed that turn.
  When an ability that inflicts bleeding gets a \glossterm{critical hit}, the damage of the bleed is doubled normally from the critical unless the ability has a different critical hit effect.

  \debuffdef{burn} A burning creature takes damage at the end of each of its turns.
  A burning creature stops burning if it makes a \glossterm{difficulty value} 8 Dexterity check as a \glossterm{standard action}.
  Dropping \prone as part of this action gives a +5 bonus to this check.
  You automatically stop burning if you are affected by a \atCold or \atWater ability, or are submerged in a non-flammable liquid like water.

  If a creature stops burning at the end of its turn, it still takes damage from the burn that turn.
  When an ability that inflicts burning gets a \glossterm{critical hit}, the damage of the burn is doubled normally from the critical unless the ability has a different critical hit effect.
  ...
  This penalty does not stack with the \dazed effect.
  When a creature becomes confused, and at the start of each of its turns, it randomly decides to have one of two behaviors that turn: attack its \glossterm{enemies}, or protect itself and its \glossterm{allies} without attacking.
  ...
  \debuffdef{corrode} A corroding creature takes damage at the end of each of its turns.
  It can stop corroding if it makes a \glossterm{difficulty value} 8 Dexterity check as a \glossterm{standard action}.
  Dropping \prone as part of this action gives a +5 bonus to this check.
  You automatically stop corroding if you are affected by a \atCold or \atFire ability, or are submerged in a non-corrosive liquid like water.

  If a creature stops corroding at the end of its turn, it still takes damage from the corrosion that turn.
  When an ability that inflicts corrosion gets a \glossterm{critical hit}, the damage of the corrosion is doubled normally from the critical unless the ability has a different critical hit effect.
  ...
  It loses the benefits of special vision traits that require eyes, such as \sense{darkvision} and \sense{low-light vision}.
  In addition, it treats everything it sees as if it had \glossterm{concealment}.
  Among other effects, this gives its \glossterm{targeted} attacks a 20\% \glossterm{miss chance}.
  If the dazzled creature can perceive targets with other effects that do not require eyes, such as \sense{blindsight}, it can ignore this miss chance.

  \debuffdef{deafened} A deafened creature cannot hear.
  It automatically fails at actions that depend on hearing, and is immune to Auditory abilities.
  ...
  It obeys the commands of the creature that dominated it unquestioningly, as an automaton.
  If it does not understand the language of the creature that dominated it, it still attempts to obey as much as possible, and simple commands (such as ``attack'' or ``follow'') can usually be communicated successfully.
  ...
  \debuffdef{enraged} An enraged creature must spend a \glossterm{standard action} to make an attack during its turn.
  ...
  \debuffdef{exposed} An exposed creature takes a \minus4 penalty to all defenses.
  This effect automatically ends after the creature is next attacked, or at the start of its next turn, whichever comes first.
  ...
  \debuffdef{goaded} A goaded creature takes a \minus2 penalty to \glossterm{accuracy} against creatures other than the creature that goaded it as long as it is within \medrange of that creature.
  ...
  \begin{raggeditemize}
    \item One of your hands cannot be used for any purpose.
  ...
  These penalties do not stack with the penalties for being unsteady and either partially or fully unaware.
  In addition, it cannot use any action that would require motion.
  Creatures that are helpless due to bindings can still attempt to free themselves, such as with a Flexibility check, but can take no other actions.
  Bound and unconscious creatures are helpless.
  ...
  \buffdef{immune} A creature that is immune to an attack is completely unaffected by it.
  Creatures and objects can be immune to a specific ability tag or debuff.
  ...
  For example, if you are immune to being \dazed, you still take full damage from an attack that deals damage and stuns you.
  ...
  The penalty from this effect does not stack with the \frightened effect.
  ...
  \debuffdef{partially unaware} Creatures take a \minus2 penalty to all defenses against attacks that they are partially unaware of.
  They have a 50\% miss chance with \glossterm{targeted} attacks against creatures and objects that they are partially unaware of, and they can only attempt to target creatures and objects that they know the location of.
  These penalties do not stack with the penalties for being \unaware or \unsteady.

  A creature that knows that it is in danger and is attempting to defend itself, but does not know the exact location or nature of its attackers, is partially unaware of attacks against it.
  ...
  A creature can stand up from being prone as part of a \glossterm{movement}.
  ...
  \buffdef{resistant}
  % Are there any other things you can be resistant to?
  A creature can be resistant to a particular ability tag.
  Creatures gain a \plus4 bonus to all defenses against attacks that they are resistant to.
  In addition, they take no damage if the attack misses, even if the attack would normally deal half damage on a miss.
  If an attack has multiple tags, a creature is resistant to the attack if it is resistant to any of the attack's tags.
  % There shouldn't be any conditional tags like this
  % For attacks with random effects, such as the \spell{chromatic orb} spell, determine the random effect before determining if the creature is resistant.

  \buffdef{shielded} A shielded creature gains a \plus2 bonus to its Armor and Reflex defenses.

  \debuffdef{sickened} A sickened creature cannot \glossterm{explode} on attack rolls and has a \glossterm{damage penalty} equal to its \glossterm{character rank}.

  \debuffdef{slowed} A slowed creature has a \minus10 foot penalty to its \glossterm{speed} and takes a \minus2 penalty to its Armor and Reflex defenses.

  \debuffdef{squeezing} A squeezing creature is trying to move through an area too small for it to fight in normally.
  ...
  \debuffdef{stasis} A creature in stasis is frozen in place.
  While in stasis, it is \glossterm{unconscious} and cannot be awakened by any means. 
  It cannot be targeted, moved, damaged, or otherwise affected in any way.

  \buffdef{steeled} A steeled creature takes no additional damage from \glossterm{critical hits}.
  Any other special effects from critical hits, such a condition that must be removed multiple times, still function normally.
  ...
  If a creature would be both vulnerable and \resistant to an ability, it is treated as vulnerable and not resistant.
  However, an immune creature is still immune.
...
\newpage
\section{Senses}\label{Senses}

  \sensedef{Blindsense}{blindsense}
  ...
  \sensedef{Blindsight}{blindsight}
  ...
  It does not need to use its eyes to gain this benefit.
  This allows the creature to ignore all \glossterm{miss chances} against anything it can see with its blindsight.
  Blindsight always has a specific range limit, and provides no benefit beyond that range.
  It also does not mitigate \glossterm{cover} or otherwise allow sensing through objects that block \glossterm{line of effect}.

  \sensedef{Darkvision}{darkvision}
  ...
  As long as a creature with darkvision is in \glossterm{bright illumination} or \glossterm{brilliant illumination}, its darkvision stops working.
  ...
  \sensedef{Lifesense}{lifesense}
  Lifesense functions like \sense{blindsense}, except that it only grants knowledge of the location of living things.
  This includes both creatures and non-creatures, such as plants.

  \sensedef{Lifesight}{lifesight}
  Lifesight functions like \sense{blindsight}, except that it can only see living things.
  This includes both creatures and non-creatures, such as plants.

  \sensedef{Low-light Vision}{low-light vision}
  ...
  Unless otherwise specified, telepathy has a \glossterm{limit} of one communication channel at a time.
  ...
  That channel remains open until the telepathic creature \glossterm{dismisses} it.
  ...
  This does not generally grant the ability to detect any other thoughts, though creatures with limited self-control may accidentally broadcast their private thoughts.
  ...
  \sensedef{Tremorsense}{tremorsense}
  Tremorsense functions like \sense{blindsense}, except that it requires an uninterrupted path through solid objects instead of \glossterm{line of effect}.
  ...
  \sensedef{Tremorsight}{tremorsight}
  Tremorsight functions like \sense{blindsight}, except that it requires an uninterrupted path through solid objects instead of \glossterm{line of effect}.
...
\section{Traits}\label{Traits}
  ...
  \traitdef{Blooded}{blooded}
  A blooded creature has blood.
  It is affected normally by \atBlood abilities.

  \traitdef{Bloodless}{bloodless}
  A bloodless creature does not have blood.
  It is immune to \atBlood abilities.

  \traitdef{Corporeal}{corporeal}
  A corporeal creature has a physical body.
  If its body is sufficiently damaged, it will die or otherwise cease functioning.

  \traitdef{Dynamic}{dynamic}
  A dynamic creature needs to eat, drink, and sleep.
  It heals normally when it rests, and is affected normally by the Medicine skill.

  \traitdef{Ensouled}{ensouled} Ensouled creatures have \glossterm{souls}.
  When an ensouled creature dies, the soul native to its body typically goes to an appropriate afterlife.
  ...
  Being incorporeal has the following effects:
  \begin{itemize}
    \item Always \trait{floating}
    \item \plus5 bonus to Stealth 
    \item An average \glossterm{fly speed} with no \glossterm{height limit}, but no other movement modes
    \item Cannot be \glossterm{touched}
    \item Cannot be detected by \sense{tremorsense} or \sense{tremorsight}
    \item Immune to \squeezing and \glossterm{difficult terrain}
    \item Immune to \atBrawling, \atCreation, \atManifestation, \atPhysical, and all \glossterm{mundane} abilities
    \item Unable to use any \glossterm{mundane} \atPhysical abilities
  \end{itemize}

  Incorporeal creatures ignore any other effects which would only make sense for creatures with a physical body, such as setting off pressure plates for traps.
  They can also enter or pass through solid objects.
  They must remain adjacent to the object's exterior at all times.
  ...
  \traitdef{Mortal}{mortal} Mortal creatures eventually die of old age.
  Their bodies deteriorate over time.

  \traitdef{Multipedal}{multipedal}
  A multipedal creature typically uses more than four legs to move.
  Multipedal creatures gain a \plus10 foot bonus to their \glossterm{speed}, a \plus5 bonus to the Poise skill, and are immune to the \ability{trip} ability.

  \traitdef{Nonliving}{nonliving}
  A nonliving creature or object is neither alive nor undead.
  It is immune to \atLife effects and \atPoison effects.
  It is unaffected by the Medicine skill, even if it is \trait{dynamic}.

  % \traitdef{Planebound}{planebound}
  ...
  % Most planebound creatures are \creaturetype{soulforged} creatures from that plane.

  \traitdef{Quadrupedal}{quadrupedal}
  A quadrupedal creature typically uses four legs to move.
  Quadrupedal creatures gain a \plus10 foot bonus to their \glossterm{speed} and are \resistant to the \ability{trip} ability.
  ...
  \traitdef{Sighted}{sighted}
  A sighted creature uses light to see, typically using eyes.

  \traitdef{Sightless}{sightless}
  A sightless creature does not use light to see, making it permanently \blinded.
  Most sightless creatures have special senses, such as \sense{blindsight}, that allow them to attack and defend themselves normally.

  \traitdef{Soulless}{soulless}
  A soulless creature does not have a soul.
  It is immune to \atSoul abilities.
  ...
  If it dies, it cannot be \glossterm{resurrected}.

  \traitdef{Static}{static}
  A static creature does not need to eat, drink, or sleep.
  It does not regain hit points or remove vital wounds when it rests, and is unaffected by the Medicine skill.
  Any damage it takes must be physically repaired like a damaged object using a relevant Craft skill, as described below.
  The \glossterm{difficulty value} for these Craft checks is equal to 5 \add twice the creature's rank (minimum DV 2).
  Unlike when creating new, the time required for these repairs is not reduced by beating the Craft DV by 10 or more.
  \begin{itemize}
    \item A short rest requires 10 minutes of work and a Craft check.
    \item A long rest requires 8 hours of work and a Craft check.
    \item Removing a vital wound requires 8 hours of work, a Craft check, and raw materials with a value one rank lower than the creature's rank (minimum rank 0, or 1 gp). This also grants the benefits of a long rest.  
  \end{itemize}

  \traitdef{Swarm}{swarm}
  A swarm is a collection of smaller creatures that are mechanically treated as a single larger creature.
  Swarms can be targeted more than once by \glossterm{targeted} abilities that can choose multiple targets, such as \glossterm{chain} abilities and strikes using weapons with the \weapontag{Sweeping} weapon tag.
  They gain a \plus4 bonus to all defenses against targeted abilities that target them only once.
  However, they take a \minus4 penalty to all defenses against area abilities.
  Swarms can freely share space with other non-swarm creatures, and this does not cause the swarm or the creature in its space to be \squeezing.

\newpage
\section{Creature Origins}\label{Creature Origins}
  % Affected by origin:
  % mortal/immortal
  % living/nonliving
  ...
  Each creature has exactly one origin.
  Unlike traits, which are defined above, an origin is an intrinsic property of a creature that cannot be granted or removed temporarily by any means.
  Unless otherwise specified, all creatures have the following default traits: \trait{blooded}, \trait{corporeal}, \trait{dynamic}, \trait{ensouled}, \trait{living}, \trait{mortal}, and \trait{sighted}.

  \creatureorigindef{Artificial}{artificial}
  Artificial creatures are composed of matter that would not ordinarily support life, such as metal, water, or \glossterm{essentia}.
  Common artificial creatures include constructs, elementals, and demons.

  Artificial creatures normally have the following traits: \trait{bloodless}, \trait{immortal}, and \trait{nonliving}.

  \creatureorigindef{Natural}{natural}
  Natural creatures are composed of biological matter that naturally supports life. 
  Most creatures are natural, including humans, animals, and dragons.

  \creatureorigindef{Undead}{undead}
  ...
  Although undead are \trait{nonliving}, they are damaged by some forms of healing.
  Undead can be targeted as if they were living \glossterm{allies} by \magical \atLife effects.
  If a \magical \atLife ability would cause an undead to regain hit points, it instead takes damage equal to the hit points that it would have regained. 
  ...
  Undead creatures normally have the following traits: \trait{bloodless}, \trait{immortal}, \trait{nonliving}, and \trait{static}.
...
\subsection{Creature Types}\label{Creature Types}

  Each creature has one or more creature types.

  \creaturetypedef{Aberration}{aberration}
  Aberrations have souls that are owned by the Precursors in the Eternal Void.
  When an aberration dies, its soul drifts through the Astral Expanse for centuries until it finally returns to the Eternal Void.

  \traitdef{Animal}{animal}
  ...
  \traitdef{Beast}{beast}
  Beasts have animalistic characteristics, but they are smarter, more magical, or otherwise more complex than animals.
  Pure beasts are typically found in the wilderness, and seem to naturally avoid civilized areas.

  % TODO: should constructs regain hit points when they rest?
  \traitdef{Construct}{construct} A construct is a creature that is made of non-sapient matter.
  ...
  However, they lack agency and cannot make independent decisions.
  Instead, they simply follow the instructions they were given when created.
  A construct's Intelligence reflects the complexity and nuance of the instruction it was given.

  Constructs normally have the following traits: \trait{mindless}, \trait{nonliving}, \trait{soulless}, \trait{static}.
  ...
  This functions like \ability{accelerate recovery} ability from the Medicine skill, except that it uses an appropriate Craft skill and raw materials appropriate to the construct's construction.

  \creaturetypedef{Dragon}{dragon} A dragon is a powerful magical creature with four legs and two wings.
  Dragons can feed on magic and valuable metals, including gold. 
  ...
  Dragons normally have the following traits: \trait{immortal}.

  % TODO: What makes fey unique? They are defined as chaotic in the votive section, but why?
  \traitdef{Fey}{fey} Fey are chaotic creatures that are highly magical.

  Fey normally have the following traits: \trait{immortal}.

  \creaturetypedef{Ghost}{ghost}
  A ghost is the soul of a deceased creature that lingered after death instead of proceeding to its proper afterlife.
  Ghosts are \trait{incorporeal} and \resistant to \atCold and \atEarth abilities.
  However, they are \vulnerable to \atFire abilities.
  Despite being incorporeal, they are affected by even \glossterm{mundane} Fire effects.

  Ghosts normally have the following traits: \trait{bloodless}, \trait{immortal}, \trait{incorporeal}.

  \creaturetypedef{Humanoid}{humanoid}
  Humanoid creatures have two arms, two legs, and one head.
  Most civilized creatures and societies are predominantly humanoid.

  \traitdef{Indwelt}{indwelt}
  An indwelt is a creature that is made of non-sapient matter.
  Its inanimate body was awakened to a semblance of life by connection to an external soul.
  Unlike constructs, indwelt have agency and true intelligence.

  The soul of an indwelt did not originally belong to to the matter that composes its body.
  ...
  If the body of an indwelt is made of organic matter, it is \trait{dynamic} and \trait{living}.
  Otherwise, it is \trait{static} and \trait{nonliving}.

  \creaturetypedef{Insect}{insect}
  Insects are multi-legged invertebrates that often have a tough exoskeleton.
  They generally have highly organized social structures.
  Even \trait{simple-minded} insects can be capable of highly coordinated behavior in groups.

  \creaturetypedef{Ooze}{ooze}
  Oozes are amorphous, gelatinous creatures that lack defined organs or skeletons.

  Oozes normally have the following traits: \trait{bloodless}, \trait{simple-minded}, \trait{soulless}.

  \traitdef{Plant}{plant}
  Plants are alive, but do not have flesh or blood.
  They are \vulnerable to \atFire abilities.
  Some plants are creatures, but most plants are living \glossterm{objects}.

  Plants normally have the following traits: \trait{bloodless}, \trait{soulless}.

\traitdef{Soulforged}{soulforged}
  A soulforged is a creature made from the essentia of a Spiritual Plane.
  They are called soulforged because they are made of essentia, which comes from souls.
  ...
  Soulforged normally have the following traits: \trait{bloodless}, \trait{immortal}, \trait{nonliving}, \trait{soulless}.
