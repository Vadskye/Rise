...
\glossdef{ability rank} Every ability has a \glossterm{rank}.
An ability's rank is typically equal to the minimum \glossterm{character rank} you would need to have learn or use the ability.
...
An ally is any creature you consider an ally, and who also considers you an ally, excluding yourself.
...
\glossdef{area} An area ability affects multiple targets within an area.
...
All attacks require an \glossterm{attack roll}.
An ability that requires an attack roll is an attack, even if used in a non-harmful way.
...
\glossdef{briefly}[brief] An effect that lasts briefly, or a brief effect, lasts through the end of the next round after the effect was applied.
...
\glossdef{character rank} Your character rank is a general categorization of your character's power.
It is based on your level, as defined in \tref{Character Advancement and Gaining Levels}.
Your character rank is also listed in the base class table for each class, since it determines your maximum \glossterm{archetype rank} (see \pcref{Archetype Ranks}).
...
\glossdef{immune} A creature that is immune to an effect treats it as if the effect did not exist.
% TODO: wording
...
For more long-term consequences of receiving damage in combat, see \pcref{Vital Wounds}.
...
It has no effect while suppressed.
% TODO: This seems designed to support antimagic fields. Is that meaningful?
% Only \magical abilities can be suppressed.
% Mundane results of magical abilities (e.g., water from \ritual{create water}) cannot be suppressed and do not disappear in suppression areas.
