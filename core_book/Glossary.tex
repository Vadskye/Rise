...
\glossdef{acid}[Acid] A kind of \glossterm{energy}. Acid damage is very effective against most objects. For the Acid spell tag, see \pcref{Ability Tags}.
...
\glossdef{attack}[attacks] Anything that affects another creature in a potentially harmful way. There are two kinds of attacks: \glossterm{physical attacks} and special attacks.
...
\glossdef{immediate action} Each round, you can take a single swift or immmediate action.
...
\glossdef{physical attack}[physical attacks] A physical attack is an attack with a weapon, fist, or other physical object.
Examples include swinging a longsword, punching, and using a combat maneuver (see \pcref{Combat Maneuvers}).
Attacks that are not physical in nature, such as spells and many class abilities, are called \glossterm{special attacks}.
...
\glossdef{special attack}[special attacks] A special attack is an attack that is not physical in nature.
Examples include casting a spell, using a cleric's invocation (see Domain Invocation, page \pref{Clr:Domain Invocations}), and many other supernatural abilities.
Attacks that are physical in nature are called \glossterm{physical attacks}.
...
\glossdef{swift action} Each round, you can take a single swift or immmediate action.
