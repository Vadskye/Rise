...
\glossdef{ability rank} An ability's \glossterm{rank} is typically the minimum \glossterm{character rank} required to learn or use the ability.
...
\glossdef{ally}[allies] An ally is any creature you consider an ally, and who also considers you an ally, excluding yourself.
...
\glossdef{archetype rank} Your rank in an archetype determines which abilities from that archetype you have access to, and the power of those abilities.
Your rank in any given archetype cannot exceed your \glossterm{character rank}.
For details, see \pcref{Archetype Ranks}.

\glossdef{area} An area ability has a \glossterm{point of origin} and affects all valid targets within some distance from that location.
There are six standard area sizes: \tinyarea, \smallarea, \medarea, \largearea, \hugearea, and \gargarea.
...
\glossdef{attached}{attach} A creature can become attached to another creature in its space.
The attached creature can no longer move using its movement modes.
Whenever the creature it is attached to moves, the attached creature moves with it in the same way.

Each ability that attaches creatures specifies how that attachment can be broken.
In addition, the attachment is broken if either creature \glossterm{teleports} or become \glossterm{incorporeal}.

\glossdef{attack}[attacks] Any ability that requires an \glossterm{attack roll} is an attack, even if it is used in a non-harmful way.

\glossdef{attack result} An attack result is the total you get on an \glossterm{attack roll}, after taking into account any bonuses or penalties that apply to the roll.

\glossdef{attack roll}[attack rolls] To make an attack roll, roll 1d10 \add your \glossterm{accuracy} with the attack.
If the result equals or exceeds the target's \glossterm{defense}, the attack succeeds.
...
\glossdef{attribute} An attribute represents a character's capacity in a wide range of areas.
There are six attributes: \glossterm{Strength}, \glossterm{Dexterity}, \glossterm{Constitution}, \glossterm{Intelligence}, \glossterm{Perception}, and \glossterm{Willpower}.

\glossdef{attunable} An effect is attunable if it can be attuned to.
There are two kinds of attunable effects: effects with the \atAttune tag, and effects with the \atSustain (attunable) tag.

\glossdef{attune}[attunement] Some abilities last as long as you attune to them.
Attuning to an ability costs an \glossterm{attunement point} that you cannot recover as long as you maintain that attunement.
...
\glossdef{available movement} Your available movement is the maximum distance you can move during the current turn.
At the start of your turn, it becomes equal to your \glossterm{speed}.
For details, see \pcref{Movement Speed}.

\glossdef{barding} Armor designed for non-humanoid creatures is called barding.
The Armor defense bonus provided by barding is 1 lower than normal.
...
\glossdef{briefly}[brief] An effect that lasts briefly is called a brief effect.
A brief effect lasts until the end of the next turn of the creature that created it.
If a brief effect is not created by a creature, it lasts until the end of the next turn.
...
Any effect which creates bright illumination in an area also creates enough light for \glossterm{dim illumination} in twice that area.
...
Any effect which creates brilliant illumination in an area also creates enough light for \glossterm{dim illumination} in twice that area.
For details, see \pcref{Vision and Light}.
...
\glossdef{character rank} A character's rank is a broad categorization of its capabilities, ranging from 0 to 7.
Character rank is based on level, as defined in \tref{Character Advancement and Gaining Levels}.
It is also listed in the base class table for each class, since it determines a character's maximum \glossterm{archetype rank} (see \pcref{Archetype Ranks}).
...
That target must be within 15 foot \glossterm{range} of the previous target in the chain and must not already be a target of the ability.
You must have \glossterm{line of sight} to each chained target.
However, you do not need \glossterm{line of effect} to them, and they can be beyond the ability's original range.
...
\glossdef{check}[checks] A check is a roll that you make to try to accomplish a task.
If the result of your roll, including your modifier, is high enough, you succeed.
Otherwise, you fail.
For details, see \pcref{Making Checks}.

\glossdef{class}[classes] A character's class determines their fundamental source of power and many of their abilities.
For details, see \pcref{Classes}.

\glossdef{class skill}[class skills] Each \glossterm{class} has an associated set of class skills that members of that class often know.
Your \glossterm{base class} automatically grants you training with a specific number of skills from this list.
For details, see \pcref{Skills}.

\glossdef{climb speed} A creature with a climb speed can move at that speed while climbing, and does not suffer penalties while doing so.
For details, see \pcref{Movement Modes}.
...
For details, see \pcref{Maneuver Lists}.
...
\glossdef{condition}[conditions] A condition is an effect that lasts on a creature until it is removed.
Most conditions are standard \glossterm{debuffs}.
Player characters can remove conditions with a \glossterm{short rest}, or with special abilities like \ability{recover} (see \pcref{Recover}).
...
\glossdef{corpse} A corpse is the inanimate, deceased body of a once-living creature.
...
It grants a \plus2 bonus to Armor and Brawn defenses.
In addition, attacks that miss you hit the obstacle instead.
For details, see \pcref{Cover}.

\glossdef{critical hit}[critical hits] An attack that beats the target's defense by 10 or more gets a critical hit.
...
\glossdef{critical success} A \glossterm{check} that beats the \glossterm{difficulty value} by 10 or more gets a critical success.
Some abilities have special effects on a critical success.

\glossdef{damage} Taking damage reduces a creature's \glossterm{hit points}.
For details, see \pcref{Taking Damage}.

\glossdef{damage penalty} If you have a damage penalty, you reduce all damage you deal by that amount, to a minimum of 0.
For details, see \pcref{Damage Penalties}.
...
\glossdef{deep attunement} A deep attunement ability is an \abilitytag{Attune} ability with two additional restrictions.
First, it costs an extra \glossterm{attunement point} to \glossterm{attune} to.
Second, these attunement points are not recovered until you take a \glossterm{short rest}, even if the attunement is released.
For details, see \pcref{Deep Attunement}.

\glossdef{defeated}[defeat] A creature is defeated if it dies or is incapacitated for an extended period of time (such as by being knocked unconscious).
...
\glossdef{dim illumination} In an area with dim illumination, it is more difficult to see clearly.
...
% TODO: more details about diseases
\glossdef{disease} An affliction of the body, causing a steady deterioration over time.
Diseases are not mechanically defined in Rise, though they still exist in the narrative of the world.

\glosssynonym{dismisses}
\glossdef{dismiss}[dismissed] When an ability is dismissed, it ends and all of its lingering effects are removed.
Any \magical ability with a duration can normally be dismissed as a \glossterm{free action}, but \glossterm{mundane} abilities cannot be dismissed unless they have a \glossterm{limit}.
For details, see \pcref{Dismissal}.
...
\glossdef{elite action}[elite actions] Elite monsters can take a special extra action every turn called an elite action.
...
\glossdef{enemy}[enemies] An enemy is any creature you consider to be an enemy, excluding yourself.
For details, see \pcref{Allies and Enemies}.

\glossdef{enhancement bonus}[enhancement bonuses] Enhancement bonuses do not stack with other enhancement bonuses, even if they are from different sources.
For details, see \pcref{Stacking Rules}.

\glossdef{essentia} Essentia is the physical substance left behind when a soul decomposes into nonexistence.
This typically happens after the soul has spent centuries in its afterlife and finally loses the will to maintain its existence.
It is the foundational material of the Spiritual Planes (see \pcref{Cosmology}).
...
\glossdef{extended check} An extended check is a \glossterm{check} that represents your character taking some action over a prolonged period of time.
...
\glossdef{everything} When "everything" is used in the description of an ability, it means "all creatures and objects".

\glossdef{extra damage} Extra damage is additional damage added on top of an attack's normal damage.
For details, see \pcref{Extra Damage}.

\glossdef{failure chance}[failure chance] If you have a failure chance with an ability, you have a random chance to fail.
Failure chances are rolled before \glossterm{miss chances} and \glossterm{attack rolls}.
...
\glossdef{falling damage} When a creature or object falls 10 or more feet, it takes falling damage based on its \glossterm{weight category}.
...
There are two types of forced movement: \glossterm{push} and \glossterm{fling}.
...
You can take any number of free actions during each of your turns.
...
You cannot use the same hand for two different purposes in the same turn.
...
\glossdef{hardness} Whenever a creature or object with hardness takes damage, it reduces that damage by an amount equal to its hardness.
Normally, only objects have hardness.
For details, see \pcref{Breaking Objects}.
...
\glossdef{height limit} A height limit defines your maximum distance directly above an object at least two size categories larger than you that is free-standing and capable of supporting your weight.
...
\glossdef{ice crystal} Ice crystals improve the effects of some spells from the \sphere{Cryomancy} mystic sphere.
...
\glossdef{immune} You treat effects you are immune to as if they did not exist.
You cannot have or gain \glossterm{conditions} or similar effects from sources you are immune to.
For example, if you become immune to poison, you would remove all poisons affecting yourself.
...
\glossdef{initiative} Initiative determines the order in which creatures take their turns in combat.
Allies with no enemies between them in the initiative order form an \glossterm{initiative group}.
For details, see \pcref{Initiative}.

\glossdef{initiative group}[initiative groups] Allied creatures with no enemies between them in the initiative order are bundled into initiative groups.
Each initiative group gets a single shared turn.
Within that turn, creatures can take their actions in any order.
For details, see \pcref{Initiative Groups}.

\glosssynonym{injured}
\glossdef{injury}[injuries] If your remaining hit points are at or below your \glossterm{injury point}, you are \glossterm{injured} (see \pcref{Injury}).
For more long-term consequences of taking damage, see \pcref{Vital Wounds}.
...
For example, Athletics is a Strength-based skill, so Strength is its key attribute.

\glossdef{fling}[flung] A fling is a type of \glossterm{forced movement}.
It represents being thrown backwards by a single large impact.
% TODO: Should this reference falling damage and scale with size?
If a creature or object being flung encounters an obstacle, it and the obstacle each take 1d8 damage per 10 feet of movement remaining.
For details, see \pcref{Fling Effects}.
...
\glossdef{limit} Some abilities have a defined limit on how many times they can be active simultaneously.
When you would exceed that limit, another instance of that ability is immediately \glossterm{dismissed}.
Unless otherwise specified, you can choose which instance is dismissed.
...
It allows you to remove all of your \glossterm{stamina} and make progress towards healing a \glossterm{vital wound}.
...
\glossdef{maneuver} A maneuver is a type of \glossterm{mundane} ability that some classes grant access to through particular combat styles.
For details, see \pcref{Maneuver Lists}.
...
\glossdef{maximum stamina}[Maximum stamina] Your maximum stamina measures the peak amount of energy you can exert before you begin to suffer from exhaustion.
For details, see \pcref{Maximum Stamina}.
...
\glossdef{metallic} A creature is metallic if it is wearing metal armor or otherwise carrying a significant amount of exposed metal.
...
\glossdef{midair} A creature or object is midair if it is in the air unsupported by any other creature or object.
While midair, land-based creatures are \unsteady, even if they have a fly or glide speed.
...
You can take one minor action during each of your turns.
For details, see \pcref{Actions}.
...
You can use one move action during your turn.
Almost all move actions change your location on the battlefield.
For details, see \pcref{Basic Movement}.
...
\glossdef{mundane}[Mundane] A mundane ability does not originate from a magical source.
...
For details, see \pcref{Spell Lists}.
...
\glossdef{object} An object is something with physical substance that is incapable of meaningful motion or agency.
Some objects, such as ordinary plants, are \trait{living}, but most objects are \trait{nonliving}.
...
\glossdef{poison}[poisoned] Poisons attack Fortitude defense each turn to cause debilitating effects or damage.
...
\glossdef{power} Your power increases your damage, and can have other effects on specific abilities.
For details, see \pcref{Power}.
...
\glossdef{proficient}[proficiency] A creature must be proficient with weapons and armor to use them effectively.
Using a weapon without proficiency imposes a \minus2 accuracy penalty.
Wearing armor without proficiency halves its defense bonus and applies its Armor defense bonus as a penalty to \glossterm{accuracy}.

\glossdef{projectile} A projectile is an object fired from a weapon at a target.
Arrows and bolts are projectiles.

\glossdef{push}[pushed] Push is a type of \glossterm{forced movement}.
...
\glossdef{rank} A rank is a general categorization of how powerful something is.
There are several specific types of rank: \glossterm{ability rank}, \glossterm{archetype rank}, \glossterm{character rank}, and \glossterm{item rank}.
...
\glossdef{reactive attack} A reactive attack is an \glossterm{attack} that happens outside of your control.
...
If you would make multiple reactive attacks with the same ability against multiple targets during the same turn, use the same attack roll for each target.
...
% TODO: move this detail to a core book section
...
% TODO: why would it have +2 accuracy? A more specific example would be helpful
For example, imagine a creature had a \plus2 accuracy bonus when it initially used a repeating ability and then was knocked unconscious before the repeat occurs.
...
% TODO: It seems like there are two concepts here:
% * repeating the ability *usage* from the original creature
% * repeating the ability *effect* on specific targets
...
\glossdef{resource} A resource is something that a character can expend to gain a benefit.
...
In addition, there are two resources that are used during gameplay: \glossterm{attunement points} and \glossterm{stamina}.
...
Some abilities cause a scrying sensor to see and act autonomously.
A scrying sensor's visual acuity is the same as that of a normal human, giving it a \plus0 bonus to the Awareness skill and similar checks.
When you look through a scrying sensor, you use your own Awareness modifier, but any special vision abilities like \trait{darkvision} and \trait{low-light vision} do not function.
...
\glossdef{shapeable} Abilities with shapeable areas can customize the exact area they affect.
For details, see \pcref{Shapeable Areas}.
...
For example, the Poise skill represents how skilled you are at maintaining your balance or controlling a mount.
For details, see \pcref{Skills}.
...
\glossdef{speed} Your speed indicates how far you can move during a typical turn.
For details, see \pcref{Movement Speed}.

\glossdef{spell}[Spell] A spell is a discrete \magical ability with combat-relevant effects.
...
\glossdef{stamina} You can spend stamina for special abilities.
If your current stamina falls below 0, you take a \glossterm{stamina penalty}.
For details, see \pcref{Stamina}.

% Does "exhausted" need to exist as a term?
\glossdef{stamina penalty} You take a penalty to \glossterm{accuracy} and \glossterm{checks} equal to the amount your current \glossterm{stamina} is below 0.
When your current stamina reaches \minus5, you fall \unconscious until your stamina is restored above \minus5.
For details, see \pcref{Stamina Penalty}.

\glossdef{standard action} A standard action is one of the four action types (see \pcref{Actions}).
You can take one standard action during each of your turns.
For details, see \pcref{Actions}.
...
You can make a strike as a \glossterm{standard action}.
For details, see \pcref{Strikes}.

\glossdef{strike damage} Your strike damage is the amount of damage you deal with a normal \glossterm{strike}.
It's normally equal to your \glossterm{weapon damage} plus half your relevant \glossterm{power}.
For details, see \pcref{Strike Damage}.

\glossdef{subdual damage} Subdual damage is a special kind of damage that can't kill you.
If you would gain a \glossterm{vital wound} from subdual damage, you reduce your \glossterm{stamina} by three instead.
For details, see \pcref{Subdual Damage}.

\glossdef{suppressed}[suppress] Effects have no effect while they are suppressed.

\glosssynonym{sustaining}
...
At the start of your turn, the ability is dismissed unless you take the action to sustain the ability that turn.
For details, see \pcref{Sustained Abilities}.

\glossdef{swim speed} A creature with a swim speed can move at that speed while swimming, and being underwater does not make it \unsteady.
For details, see \pcref{Movement Modes}.
...
\glossdef{targeted}[Targeted] A targeted ability allows you to individually choose which creatures or objects are affected.
Area abilities are not targeted, since you choose the area rather than individual targets.
A \glossterm{strike} is a targeted ability, as is any ability that causes you to immediately make a strike.
% TODO: It seems like this means you can chain strike abilities, which is confusing. Should strike abilities not be targeted?
...
Unless otherwise specified, teleportation requires \glossterm{line of sight}, \glossterm{line of effect}, and an unoccupied destination on stable ground.
...
\glossdef{touch}[touches] You can generally touch an adjacent creature with a \glossterm{free hand} as a \glossterm{free action}, even an enemy.
...
\glossdef{turn}[turns] Combat takes place in a series of turns, which represent about six seconds of action.
You can take actions during your turn.
For details, see \pcref{Combat Time}.
...
% TODO: do you need to be able to target items carried by allies as if they were unattended? Seems complicated
...
\glossdef{weapon damage} Each weapon defines the damage die or dice that it deals when you hit with it.
Typically, weapon damage is dealt by \glossterm{strikes} (see \pcref{Strikes}).
For details about the damage dealt by specific weapons, see \pcref{Weapons}.

\glossdef{weapon dice increment} A bonus to weapon dice increment increases your \glossterm{weapon damage}.
