...
That target must be within 15 foot \glossterm{range} of the previous target in the chain, it must not already be a target of the ability.
...
\glossdef{defeat} A creature is defeated when it stops being able to continue fighting, such as when it is killed.
...
This typically happens after the soul has spent centuries in its afterlife and finally loses the will to maintain its existencecl.
...
\glossdef{immune} Creatures treat effects they are immune to as if they did not exist.
An creature cannot have or gain \glossterm{conditions} or similar effects from sources it is immune to.
