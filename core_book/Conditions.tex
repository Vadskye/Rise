...
\condition{frightened} A frightened creature is \severelyimpaired with attacks and checks as long as it is within 100 feet of the source of its fear.
...
\condition{incorporeal} Having no physical body. Incorporeal creatures are immune to \glossterm{physical} abilities. They can be harmed only by other incorporeal creatures and \glossterm{magical} abilities.

\condition{ignited} An ignited creature has been set on fire. It takes 1d6 fire damage at the end of each round, and is \impaired with attacks and checks. As a move action, an ignited creature can make a DR 15 Dexterity check to put out the flames. This action requires a free hand. Dropping prone as part of the action gives a \plus5 bonus on this check.
...
\condition{prone} The creature is lying on the ground, rather than standing normally. A prone creature takes a \minus4 penalty to accuracy with physical melee attacks and physical defenses. It gains a \minus4 bonus to physical defenses against ranged attacks.
...
While squeezing, a creature moves at half speed and takes a \minus4 penalty to physical accuracy, physical checks, and physical defenses.
...
If that creature is rendered \helpless, surrenders, or is otherwise unable to fight, this effect immediately ends.
The \conditionlink{goaded} condition is a more severe version of this effect.
