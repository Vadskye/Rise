...
\section{Ability Tags}\label{Ability Tags}
    ...
    \parhead{Creation} Creation abilities create physical objects. Unless otherwise specified, the objects created are temporarily formed from raw magical energy, and disappear without a trace at the end of the effect, as the magic sustaining them fades.
    ...
    \parhead{Destructive} Destructive abilities can blast through intervening obstacles.
    If a Destruction effect deals enough damage to a barrier to destroy it, the area of the effect continues past the barrier up to its normal limits.
    ...
    \parhead{Figment} Figment abilities create a false sensation shared by everyone viewing or otherwise perceiving the figment.
    Figments cannot remove real sensations present in their area, but they can add additional false sensations.
    ...
    \parhead{Fog} Fog abilities create physical fog in an area.
    They have no effect underwater.
    If at least 5 points of fire damage are dealt in a square within the area of effect, the fog is destroyed in that area.
    Unless otherwise specified, a moderate wind (11\add mph) disperses the fog in 5 rounds, and a strong wind (21\add mph) disperses the fog in 1 round.

    \parhead{Glamer} Glamer abilities alter sensations from everyone viewing or otherwise perceiving the glamer.
    ...
    \parhead{Imbuement} Imbuement abilities grant a target new abilities, or improve its existing abilities.
    ...
    \parhead{Physical} Physical abilities create or manipulate physical objects rather than having a directly magical effect. They do not allow spell resistance.
    ...
    \parhead{Unreal} Unreal abilities have effects that are not completely real.
    Their effects can be disbelieved once inconsistencies with reality are noticed.
    All Unreal abilities are also either \glossterm{Figment} or \glossterm{Glamer} abilities.

    When you use an Unreal ability, you make a check with a bonus equal to 10 \add your \glossterm{power} with the ability.
    Creatures who observe its effect can recognize the effect as unreal by interacting with it physically, or by making an Awareness check (see \pcref{Awareness}).
    The DR of the Awareness check is equal to your check result when using the ability.
    A creature gets a \plus10 bonus on this Awareness check for each sense it uses that should be present in the effect, but which is missing.

    A creature that recognizes an unreal ability as false can treat it as if it did not exist. Its effects can still be observed if desired, but they are mere shadows of the full effect: visual effects appear translucent outlines, sounds can be heard as ghostly echoes, and so on.
