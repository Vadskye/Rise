...
\section{Saying What Your Character Does}
  ...
  \subsection{Using Specific Abilities}
    ...
    Abilities such as the \ritual{create water} ritual or \spell{distant hand} spell can be used to solve other kinds of problems entirely.
...
\section{Making Checks}\label{Checks}\label{Making Checks}
  ...
  \subsection{Standard Difficulty Values}\label{Standard Difficulty Values}
    ...
    \begin{raggeditemize}
      \item Easy (DV 0): Only an exceptionally incompetent or impaired person could possibly fail a DV 0 check. For example, this includes walking on rough ground without tripping (Poise) or noticing that a yelling, red-faced person is angry (Social Insight).
      \item Average (DV 5): A typical human with no relevant skills should still succeed at a DV 5 check without much issue. However, it would be possible to fail in a stressful situation where time is limited if the person had no relevant training. For example, this includes climbing a ladder (Athletics) or hearing the topic of a nearby conversation in a crowded bar (Awareness).
      \item Hard (DV 10): A typical human with no relevant skills might succeed at a DV 10 check, but only if they were very lucky or had a lot of time on their hands. An experienced practicioner might fail infrequently in stressful circumstances, but a world-class expert would never fail. For example, this includes swimming in fast-moving water (Athletics) or providing first aid to mitigate a barely lethal wound (Medicine).
      ...
      \item Almost Impossible (DV 20): A world-class expert like an Olympic medalist could succeed at a DV 20 check if they were lucky or patient. Succeeding consistently at tasks of this difficulty requires superhuman capabilities. For example, this includes climbing a weathered natural rock wall without equipment (Athletics) or squeezing through a space with a diameter of only half a foot (Flexibility).
      \item Impossible (DV 25\add): No real-world human can succeed at a DV 25 check. This sort of feat is only possible for high-level Rise characters who have explicitly surpassed ordinary limitations. For example, this includes running at full speed along a slack rope (Poise) or climbing a sheer glass pane (Athletics).
    \end{raggeditemize}
  ...
  \subsection{Opposed Checks}
    ...
    For example, a climbing race up a wall might involve each participant rolling a Athletics check, or you might make a Might check to hold a door closed while another creature tries to shove it open.
