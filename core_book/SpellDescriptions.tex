...
\begin{spellsection}{Black Tentacles}[5]
    ...
    \begin{spellcontent}
        ...
        \end{spelltargetinginfo}
        \begin{spelleffects}
            \spelleffect Ground within the area is considered difficult terrain. At the end of every round, make a Spellpower vs. Fortitude defense against all creatures in the area that are within 5 feet of the ground. Success against a creature means it takes \spelldamage{bludgeoning}[d8]. A critical success causes the target to be \immobilized for the duration of the spell.
            \spelldur \durshort
        \end{spelleffects}
    \end{spellcontent}
    \begin{spellfooter}
...
\begin{spellsection}{Create Image}[2]
    ...
    \begin{spellfooter}
        ...
        \spellnotes When you cast this spell, you make a check with a bonus equal to 10 \add your spellpower. Creatures can recognize the figment as unreal by interacting with it physically, or by making a Awareness check against a DR equal to your check result when casting the spell. A creature gets a \plus10 bonus on this Awareness check when using senses which should be present in the figment, but which are missing.

        A creature that disbelieves the effect can treat it as if it did not exist. Its effects can still be observed if desired, but they are mere shadows of the full effect: visual effects appear translucent outlines, sounds can be heard as ghostly echoes, and so on.
        \miscastexplode
    \end{spellfooter}
    \begin{spellaugments}
