...
\section{Analysis}
  ...
  Analysis is a broad skill that can serve multiple purposes, which can make it one of the most dynamic skills to resolve as a GM.
  Some players specifically dislike solving in-game puzzles, and they want to use Analysis to allow their character to be skilled in that area even if they don't like doing that themselves.
  ...
  When puzzles do arise, allowing a sufficiently high Analysis check to basically solve the puzzle on its own will only increase that player's overall fun.

  On the other hand, some players really like puzzle-solving, and they want to use the Analysis skill to give their characters more opportunities to do that.
  For those players, you should generally use their Analysis checks to give them additional clues and allow them to identify evidence that is relevant and discard red herrings.
  However, you should still require  them to fully solve the puzzle on their own.
  ...
  One reliable approach is to say that a successful Analysis check will solve a problem on its own eventually, but the players can try to figure it out first if they want.
  ...
  If they don't solve it in that time, a successful Analysis check either provides a key hint to get the players unstuck, or simply solves the puzzle completely, depending on whether the players seem to want to spend more time on the puzzle.

  As another example, you can let a character spend an in-game week making an Analysis check to identify the murderer, assuming the party has access to the crime scene and suspects to interrogate.
  The party can accelerate that time frame and lower the difficulty value of the Analysis check by making specific investigations on their own to eliminate suspects or gather additional evidence.
...
\section{Endurance}\label{Endurance}
  ...
  However, you should not allow players to use Endurance to ignore core game mechanics like conditions or stamina costs.
...
\section{Profession}
  ...
  For example, a player with Profession (sailor) shouldn't be able to use that skill to completely replace the Athletics, Awareness, and Devices skills.
...
\section{Poise}
  Mounts can be logistically challenging, though little of that complexity comes from the Poise skill itself.
  Characters without the Poise skill are unlikely to ride mounts into combat.
  On the other hand, characters trained in the Poise skill can generally ride mounts well within their skill level with no risk of randomly falling off.
  This means that Poise checks for riding are rare in practice.
...
\section{Swim}
  ...
  Feel free to decide that a creature who fails a Athletics check by a large enough amount sinks underwater if it seems appropriate.
