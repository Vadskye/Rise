...
\cf{Brd}{Bardic Knowledge} Bardic Knowledge: A bard may make a special bardic knowledge check with a bonus equal to his bard level \add his Intelligence to see whether he knows some relevant information about local notable people, legendary items, or noteworthy places.
...
When a cleric channels energy, it affects all creatures in a \areamed radius burst centered on him, including himself. The cleric may choose to exclude a number of creatures from the effect equal to 1 \add his Wisdom value. The amount of damage dealt (if negative energy is channeled) or healed (if positive energy is channeled) is equal to 1d6 damage per two cleric levels. Each affected creature can make a Will save to halve the damage. The DC of this save is equal to 10 \add half the cleric's level \add the cleric's Charisma.
...
\cf{Mnk}{Diamond Soul (Ex)} At 12th level, a monk gains spell resistance 10. In order to affect the monk with a spell, a spellcaster must make a spell penetration check (d20 \add half caster level \add casting attribute) that equals or exceeds the monk's spell resistance \add relevant saving throw modifier.
