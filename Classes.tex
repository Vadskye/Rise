...
\subcf{6th -- Terrifying Rage (Su)} Whenever the barbarian makes a physical attack, he may also make a (barbarian level \add Charisma vs. Will) attack against all enemies he threatens. A successful attack makes a creature \shaken for 5 rounds. This ability can only affect any individual creature once per 24 hours.
...
\cfl{Bbn}{4th}{Grit (Ex)} The barbarian's resilience allows him to shrug off magical effects. If he resists a attack against his Fortitude that deals half damage on a failed attack, he instead takes no damage.
...
\cfl{Bbn}{16th}{Improved Grit (Ex)} The barbarian's fortitude knows no bounds. If he fails to resist an attack against his Fortitude that deals half damage on a failed attack, he takes only half damage from the attack.
...
When a cleric channels energy, he affects all creatures in a \areamed radius burst centered on him, including himself if he desires. The cleric may choose to exclude a number of other creatures from the effect equal to 1 \add half his Wisdom. The amount of damage dealt (if negative energy is channeled) or healed (if positive energy is channeled) is equal to 1d6 damage per two cleric levels. The cleric must make a (cleric level \add Charisma) attack against the Fortitude of unwilling targets. The energy burst deals or heals half damage if the attack fails.
...
\cfl{Drd}{8th}{Commanding Wild Speech (Su)} As a standard action that consumes a use of her wild speech ability, the druid can attempt to charm a creature she is speaking with using her wild speech ability. If she succeeds at a (druid level \add Charisma) attack against its Will, the target is charmed. The effect lasts for the duration of the conversation, and for 1 hour thereafter. This ability is not mind-affecting, and can affect creatures or even objects of any kind that the druid can use her wild speech to converse with. The attack automatically succeeds against non-intelligent objects.
...
\cfl{Drd}{16th}{Dominating Wild Speech (Ex)} When the druid uses her commanding wild speech ability, she can dominate the subject (as the \spell{dominate person spell) instead of charming it. This ability is not mind-affecting, and can affect creatures of any kind that the druid can converse with. The attack automatically succeeds against non-intelligent objects.
...
\cfl{Mnk}{2nd}{Ki Power (Su)} The monk gains the ability to channel her \ki energy to temporarily enhance her abilities. She may use any combination of \ki powers she knows a number of times per day equal to half her monk level \add her Wisdom (minimum 1), but no more than once per round.
...
\subcf{Stunning Fist} As part of an unarmed attack, the monk can strike a weak point to interfere with her foe's \ki. If the attack deals damage, the monk makes a (monk level \add Wisdom) attack against the Fortitude of the struck creature to stagger it for 1 round. If the creature is bloodied, it is stunned for 1 round instead.
...
\subcf{Bewildering} The paladin's smite challenges her foe's mind. If the paladin succeeds on an attack vs. Will, the struck creature is \bewildered for 5 rounds.
...
\subcf{5th -- Blinding} The paladin's smite manifests as a bright light. If the paladin succeeds on an attack vs. Will, the struck creature is blinded for 1 round (see \pcref{Blinded}). Creatures vulnerable to light (such as vampires) take extra damage equal to twice her paladin level.
...
The paladin can lay on hands a number of times per day equal to 1 \add half her Charisma (minimum 1). When using lay on hands on an unwilling target, the paladin must make a touch attack. If she hits, she must make a (paladin level \add Charisma) attack against the touched creature's Fortitude. A failed attack heals or inflicts half damage.
...
\parhead{1st -- Lesser Reduction} The spellwarped makes an attack against the Fortitude of a creature within \rngclose range. A successful attack causes the creature to become one size category smaller for 2 rounds. This has the following effects:
  \begin{itemize*} 
...
\parhead{20th -- Immolate} The spellwarped makes an attack against the Fortitude of a foe within \rngclose range to consume it in flames from the inside out. This attack deals d6 points of fire damage per spellwarped level, and if the creature is bloodied after it takes this damage, it immediately dies. A failed attack deals half damage and leaves a bloodied creature with 0 hit points.

\subsubsection{Telekinesis Powers}
\parhead{1st -- Lesser Crush} As a standard action, the spellwarped can crush a creature or object within \rngclose range with telekinetic force. If he succeeds at an attack against Fortitude, he deals 1d6 points of physical damage \add 1 per spellwarped level. A failed attack deals half damage.
...
\parhead{4th -- Crush} This power functions like the lesser crush attack, except that it deals 1d6 points of physical damage per spellwarped level. If the attack against Fortitude succeeds, the target is also \sickened for 5 rounds.
...
\parhead{12th -- Strangle} As a standard action, the spellwarped can make an attack against the Fortitude of a creature within \rngclose range to crush its windpipe. This attack deals 1d6 damage per spellwarped level. If the target is bloodied after the damage is dealt, it is nauseated for 1 round. A failed attack deals half damage, and prevents the foe from being nauseated. This power costs two spellwarp points.
\parhead{14th -- }
\parhead{16th -- } 
\parhead{18th -- }
...
\parhead{1st -- Lesser Timetheft} As a standard action, the spellwarped can attempt to steal time. He makes an attack against the Will of an adjacent creature to force it to skip an action. If it is bloodied, it skips a standard action, while if it is healthy, it skips a move action.
\parhead{4th -- Slow} As a standard action, the spellwarped makes an attack against the Will of a creature within \rngclose range, making it \slowed for 5 rounds.
...
\parhead{6th -- Disjointed Time} As a standard action, the spellwarped makes an attack against the Fortitude of a single creature within \rngmed range to chaotically disrupt its local flow of time. If the attack succeeds, the creature takes a \minus4 penalty to attacks, defenses, and checks for 5 rounds.
...
\parhead{8th -- Pause Time} As a standard action, the spellwarped makes an attack against the Will of a single creature within \rngclose range to completely stop time for it for 5 rounds. The affected creature can take no actions and cannot be moved, damaged, or even affected in any way until the effect ends. The spellwarped may dismiss the effect as a swift action.
