...
\cf{Clr}{Spells}  A cleric casts divine spells granted to him by his deity or by the power of his beliefs. All clerics share access to some general cleric spells. The cleric uses Charisma as his casting attribute for these spells. To learn or cast a spell, a cleric must have a casting attribute at least equal to half the spell's level. 

A cleric's selection of spells is limited, as indicated on \trefnp{Cleric Spells Known}. Two spells at every spell level must be drawn from the cleric's domains; sometimes these spells are normal spells on the cleric's spell list, but often they are only accessible by the domain. A cleric may also choose spells from his domain lists with his normal spells known.

A cleric can cast any spell he knows at any time, assuming he has spell slots remaining. For example, at 1st level, the cleric Cadius can cast four 1st-level spells per day. He knows three 1st-level spells: one of his choice, and two from his domains. Thus, on any given day, he can cast any combination of the three spells a total of four times. A cleric may use a higher-level slot to cast a lower-level spell if he so chooses.

At each cleric level, a cleric can choose to learn a new spell in place of one he already knows. In effect, the cleric ``loses'' the old spell in exchange for the new one. The new spell's level must be the same as that of the spell being exchanged, and the spells must be of the same type; regular spells cannot be exchanged with domain spells, and the spells a cleric knows from his alignment cannot be exchanged in this way at all. A cleric may swap only a single spell at any given level, and must choose whether or not to swap the spell at the same time that he gains new spells known for the level.
...
\end{tabularx}
\end{dtable!*}
\cf{Clr}{Deity, Domains, and Domain Spells} A cleric's deity influences his alignment, what magic he can perform, his values, and how others see him. A cleric chooses two domains from among those belonging to his deity. If a cleric is not devoted to a particular deity, he still selects two domains to represent his spiritual inclinations and abilities.
...
Each domain also has an associated domain attribute which is used for the domain's spells and abilities. The cleric may choose to use Charisma or his domain attribute to learn and cast spells granted by a domain. The domain attributes for each domain are listed below.

\subcf{Air} Dexterity
\subcf{Chaos} Charisma
\subcf{Death} Constitution
\subcf{Destruction} Strength
\subcf{Earth} Constitution
\subcf{Evil} Charisma
\subcf{Fire} Wisdom
\subcf{Good} Wisdom
...
\subcf{Protection} Wisdom
\subcf{Strength} Strength
\subcf{Travel} Dexterity
\subcf{Trickery} Charisma
\subcf{Vitality} Constitution
\subcf{War} Strength
\subcf{Water} Dexterity
...
\subcf{Air} The cleric adds Athletics to his cleric class skill list and gains electricity damage reduction equal to his cleric level.
...
\par To learn or cast a spell, a druid must have a Wisdom score at least equal to half the spell's level. The Difficulty Class for a saving throw against a druid's spell is 10 \add the spell level \add the druid's Wisdom.
...
\subcf{Flurry of Blows} When the monk makes a full attack, she can make an additional attack at a \minus5 penalty.
...
\cf{Mnk}{Ki Power (Su)} At 5th level, the monk gains an additional \ki power ability. She adds the abilities below to the list of abilities she can choose.
...
\cf{Wiz}{Spells}  A wizard casts arcane spells, which are drawn from the sorcerer/wizard spell list. To learn or cast a spell, the wizard must have an Intelligence score at least equal to half the spell's level. The Difficulty Class for a saving throw against a wizard's
