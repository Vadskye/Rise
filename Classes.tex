...
\begin{dtable}
    ...
    \begin{tabularx}{\columnwidth}{>{\ccol}p{\levelcol} >{\ccol}p{\babcolgood} *{2}{>{\ccol}p{\savecol}} >{\lcol}X}
        ...
        5th & \plus3 & \plus6 & \plus4 & Mighty surge \\
        6th & \plus4 & \plus7 & \plus5 & Manifest power \\
        7th & \plus5 & \plus8 & \plus6 & Manipulate magic, spellwarped aspect \\
        8th & \plus6/\plus1 & \plus9 & \plus7 & Manifest mighty attack \\
        9th & \plus6/\plus1 & \plus10 & \plus7 & Improved spellwarped surge \\
        10th & \plus7/\plus2 & \plus11 & \plus8 & Spell resistance \\
        11th & \plus8/\plus3 & \plus12 & \plus9 & Rapid surge, spellwarped aspect \\
        12th & \plus9/\plus4 & \plus13& \plus10 & Manifest mighty power \\
        ...
        18th & \plus13/\plus8/\plus3 & \plus20& \plus14 & Manifest ultimate power \\
...
\cf{Spl}{Spellwarp Pool (Su)} A spellwarped has the ability to tap into the latent magic within his body to generate magical effects. He has a number of spellwarp points equal to half his spellwarped level \add his Constitution (minimum 1 point). As long he has at least one spellwarp point remaining, he gains a minor ability based on his choice of magic.
...
\cf{Spl}{Manifest Attack (Su)} A spellwarped can spend a spellwarp point to attack a foe within \rngclose range as a standard action. If a saving throw is allowed, the DC is equal to 10 \add the spellwarped's class level \add his key attribute.
\subcf{Alteration -- Reduce Foe} The subject becomes one size category smaller for 2 rounds unless it makes a Fortitude save. It takes a \minus2 penalty to Strength, decreases its weapon damage dice by one size, and takes a \minus4 penalty to CMA and CMD. However, it gains a \plus2 bonus to Stealth checks and a \plus1 bonus to attack rolls and armor class. This is a size-affecting effect.
\subcf{Pyromancy -- Ignite} The subject takes 1d6 points of fire damage per spellwarped level. A successful Reflex save halves the damage.
\subcf{Temporal -- Slow} The subject is slowed for 2 rounds unless it makes a Will save. A slowed creature can take a standard action or a move action each round, but not both. It cannot take full-round actions, but it may take swift actions. Additionally, it takes a \minus2 penalty to attack rolls, Strength and Dexterity-based checks, and armor class.
...
\cf{Spl}{Mighty Surge (Su)} At 5th level, the spellwarped can spend a spellwarp point to call upon his innate magic to generate a powerful but brief effect.
...
\cf{Spl}{Manifest Mighty Attack (Su)} At 8th level, the spellwarped can spend a spellwarp point as a standard action to channel his innate magic into a powerful attack.
\subcf{Alteration -- } 
\subcf{Pyromancy -- Combustion} As a standard action, the spellwarped can release a powerful explosion of flame. All creatures within a \areamed radius spread of him take 1d8 fire damage per two spellwarped levels. A successful Reflex save against a DC of 10 \add spellwarped level \add Charisma halves the damage.
\subcf{Temporal -- Pause Time} As a standard action, the spellwarped can completely stop time for a single creature for 5 rounds. A successful Will save negates the effect. The affected creature can take no actions and cannot be moved or even affected in any way until the effect ends.
...
\subcf{Pyromancy -- Fire Shield} Any creature that hits the spellwarped with its body or a melee weapon takes 1d6 fire damage per two spellwarped levels. Each individual creature can take this damage only once per round.
...
\cf{Spl}{Rapid Surge (Su)} At 11th level, the spellwarped gains the ability to use his mighty surge ability as a swift action.

\cf{Spl}{Manifest Mighty Power (Su)} At 12th level, the spellwarped gains the ability to manifest his innate magic more powerfully.
