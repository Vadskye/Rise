...
Your character must have the indicated attribute score, class feature, feat, skill, base attack bonus, or other quality designated in order to select or use that feat.
...
All effects dependent on spell level (such as the ability to penetrate a \spell{lesser globe of invulnerability}) are calculated according to the spell's modified level.

\parhead{Applying Metamagic Feats} Spellcasters apply metamagic feats on the spot.
...
\parhead{Multiple Metamagic Feats on a Spell} A spellcaster can apply multiple metamagic feats to a single spell.
Changes to its level are cumulative.
You can't apply the same metamagic feat more than once to a single spell.

\parhead{Magic Items and Metamagic Spells} With the right item creation feat, you can store a metamagic version of a spell in a scroll, potion, or wand.
...
\begin{longtabu}{>{\lcol}p{10em} >{\lcol}p{15em} >{\lcol}X >{\lcol}p{3em}}
    ...
    \tind \featref{Spell Specialization} & 4th level spells, Spell Focus &  \plus2 caster level with specific type of magic, \minus2 penalty with other types.
...
white}
    \begin{longtabu}{>{\lcol}p{10em} >{\lcol}p{15em} >{\lcol}X >{\lcol}p{6em} >{\lcol}p{3em}}
        ...
        \featref{Point Blank Shot} & \x &  \plus2 bonus to ranged attacks within half range increment.
& Precision, Style & \featpref{Point Blank Shot} \\
...
\parhead{Prerequisite} A minimum attribute score, another feat or feats, a minimum base attack bonus, a minimum number of ranks in one or more skills, or a class level that a character must have in order to acquire this feat.
This entry is absent if a feat has no prerequisite.
A feat may have more than one prerequisite.
\parhead{Benefit} What the feat enables the character (``you'' in the feat description) to do.
If a character has the same feat more than once, its benefits do not stack unless indicated otherwise in the description.
\par In general, characters cannot gain the same feat twice.
\parhead{Normal} What a character who does not have this feat is limited to or restricted from doing.
...
\featben You may gain the benefits of the \spell{haste} spell for up to 5 rounds each day.
...
\parhead{Special} You may take this feat multiple times.
...
\parhead{Benefit} You reduce your armor check penalty by 2 and your arcane spell failure by 5\% when using your chosen armor.
...
\parhead{Normal} A character who is wearing armor with which she is not proficient applies its armor check penalty to physical attacks and to all checks that involve moving, including Ride.
...
\featpre Two attacks.
\featben When you make a physical attack, you may take a \minus4 penalty to hit.
...
\parhead{Benefit} In melee, every time you miss because of being unable to see your opponent, you can reroll your miss chance one time to see if you actually hit.
...
\parhead{Benefit} As a standard action, you can bless your allies, as the \spell{bless} spell.
...
\parhead{Prerequisites} Nonevil alignment, Celestial Heritage.
\parhead{Benefit} You gain physical damage reduction 2/evil.
...
\parhead{Prerequisites} Nonevil alignment, Celestial Heritage.
\parhead{Benefit} Your smite attacks with your Celestial Heritage ability gain an attack bonus equal to the number of celestial bloodline feats you possess.
...
\parhead{Prerequisites} Nonevil alignment, Celestial Heritage.
\parhead{Benefit} You gain a \plus2 bonus to caster level with Evocation (Channeling) spells and spells from the Good domain.
If you have four or more celestial bloodline feats, this bonus increases to \plus4.

\feat{Celestial Wings}{Bloodline, Celestial}
\parhead{Prerequisite} Any three celestial bloodline feats.
\parhead{Benefit} You gain feathery wings that sprout from your back.
...
\parhead{Benefit} While in this style, you gain a \plus2 bonus to physical attack and damage against creatures that moved towards you during the current round.
...
\parhead{Benefit} If you make a melee attack that knocks a creature unconscious or kills it, you can immediately make an extra melee attack against another creature you threaten.
...
In exchange, you take a \minus2 penalty to physical defenses against creatures you don't threaten.
\stylereq Wield a melee weapon.
You must take a full attack, charge, or total defense action each round.

\feat{Combat Casting}{Magic}
\parhead{Benefit} You gain a \plus2 bonus to Concentration checks made to cast a spell or use a spell-like ability.
...
\featben When you make a physical attack, you may take a \minus6 penalty to hit.
...
A foe confused by this effect does not automatically attack you on its next turn, even though you attacked it.
You cannot make more than one strike each round.
...
\parhead{Benefit} At the start of each phase, you may choose a creature you can see.
You ignore that creature when determining whether you are \engaged in combat.

\feat{Covering Fire}{Combat, Style}
\parhead{Benefit} While in this style, if you hit a creature with a physical ranged attack, it takes a \minus2 penalty to physical attacks for 1 round.
In exchange, you take a \minus2 penalty to damage rolls with physical ranged attacks.
...
\parhead{Special} Metamagic feats and similar spell augmentations are ignored for the purpose of countering spells.
...
\featben Whenever a foe misses you with a melee attack, you gain a cumulative \plus2 bonus to damage on your next physical melee attack against that creature.
...
\parhead{Prerequisites} Dex 3.
\parhead{Benefit} While in this style, you gain a \plus2 bonus to physical ranged damage rolls.
In exchange, you take a \minus2 penalty on physical ranged attacks.
\stylereq Wield a ranged weapon.
You must take a full attack action each round.
...
\featben You may gain the benefits of the \spell{death ward} spell for up to 5 rounds each day.
Activating this ability is a swift action, and it lasts until you will it to stop (a free action) or until the duration expires.

\feat{Defensive Fighting}{Combat, Style}
\parhead{Benefit} While in this style, you gain a \plus2 bonus to your physical defenses.
In exchange, you take a \minus2 penalty to physical attacks.
\stylereq Wield a melee weapon.
...
\parhead{Benefit} You must have at least one hand free (holding nothing) to use this feat.
...
You can deflect one additional attack at base attack bonus \plus5, \plus10, \plus15, and \plus20, to a maximum number of arrows equal to your Dexterity.
...
\featpre Four attacks.
\featben When you make a physical attack, you may take a \minus8 penalty to hit.
...
\parhead{Benefit} When you have critical damage, you suffer a penalty to attacks, checks, and defenses equal to half the amount of critical damage you have.
See \pcref{Dying}, for details.
...
\parhead{Benefit} Choose a number of Knowledge skills equal to your Intelligence.
...
\parhead{Benefit} While in this style, foes you threaten take a \minus4 penalty to Concentration checks.
In exchange, you take a \minus2 penalty to physical defenses against melee attacks.
\stylereq Wield a melee weapon.
You must take a full attack or charge action each round.
...
In addition, you gain a \plus1 bonus to caster level with spells from the chosen domain.

\feat{Draconic Breath}{Bloodline, Dragon}
\parhead{Prerequisites} Con 3, any three dragon bloodline feats.
\parhead{Benefit} You gain a breath weapon based on the type of dragon you chose for the Draconic Heritage feat.
The shape of the breath weapon is given on \trefnp{Dragon Types}: either a \arealarge, 5 ft.
...
\parhead{Benefit} You gain low-light vision, allowing you to treat sources of light as if they had double their normal illumination range.
...
\parhead{Prerequisite} Draconic Heritage.
\parhead{Benefit} You gain a \plus1 bonus to caster level with all spells and spell-like abilities.
If you have four or more dragon bloodline feats, this bonus increases to \plus2.

\feat{Draconic Voice}{Bloodline, Dragon}
\parhead{Prerequisite} Draconic Heritage.
\parhead{Benefit} You gain a \plus2 bonus to Intimidate and Persuasion checks.
If you have four or more dragon bloodline feats, this bonus increases to \plus4.

\feat{Draconic Weapons}{Bloodline, Dragon}
\parhead{Prerequisite} Draconic Heritage.
\parhead{Benefit} You gain a bite natural attack that deals d8 damage for a Medium creature.
...
\parhead{Benefit} You gain leathery wings that sprout from your back.
These allow you to slow your fall so you fall no more than 60 feet per round, preventing you from taking falling damage.
...
\parhead{Benefit} When casting a heightened spell, you gain a \plus2 bonus to caster level.
...
\featpre Con 3.
\parhead{Benefit} You are not staggered when you have no hit points remaining.
...
\parhead{Normal} A character without this feat who has no hit points remaining is staggered.
...
\parhead{Prerequisite} Ability to cast 2nd level spells.
\parhead{Benefit} When casting a substituted spell, you can choose what kind of energy damage it deals: cold, fire, or electricity.
...
\parhead{Prerequisite} Ability to cast 2nd level spells.
\parhead{Benefit} An extended spell has its range doubled.
...
\parhead{Benefit} You are proficient with exotic weapons from weapon groups that you are otherwise proficient with.
This allows you to defend yourself normally with melee weapons, and to attack with exotic ranged weapons without penalty.
\parhead{Normal} You are not normally proficient with exotic weapons.
You cannot defend yourself with melee weapons you are not proficient with, and you take a \minus4 penalty to physical attacks with ranged weapons you are not proficient with.
...
\parhead{Benefit} You learn a new arcane invocation.
...
\parhead{Benefit} You can smite three more times per day.
If you have more than one smite ability, you choose which ability this applies to.
\parhead{Special} You can take more this once if you have more than one smite ability.
...
\featben As a standard action, you can create sound or light, as the \spell{create sound} or \spell{dancing light} spells.
...
\parhead{Benefit} When you use a projectile weapon, such as a bow, its range increment increases by one-half (multiply by 1-1/2).
...
\featpre Two attacks.
\featben While in this style, during the movement phase, you may choose to be \defenseless against one opponent threatening you.
If the creature attacks you that round, you gain a \plus3 bonus to damage on your next physical attack against it.
...
\parhead{Benefit} You gain a \plus2 bonus to Will defense.
...
This creates an effect similar to the \spell{break enchantment} spell, with a caster level equal to your Perform ranks.

\feat{Freeing Surge}{Surge}
\featpre 16th level, Dex 7.
\featben You may gain the benefits of the \spell{freedom} spell for up to 5 rounds each day.
Activating this ability is a swift action, and it lasts until you will it to stop (a free action) or until the duration expires.
...
\parhead{Prerequisite} Two attacks.
\parhead{Benefit} While in this style, you double your critical threat range with any weapon you wield.
In exchange, you take a \minus4 penalty to all physical attacks which are not critical threats or threat confirmations.
...
The second attack has a \minus10 penalty.

\feat{Imbue Magic}{Item Creation}
\parhead{Prerequisite} Caster level 2nd or Craft (any) 6 ranks.
\parhead{Benefit} You can imbue items with magic using your spells or crafting ability.
...
\parhead{Prerequisite} Ability to cast 2nd level spells.
\parhead{Benefit} As part of casting an imbued spellstrike spell, you can make a single attack with a weapon in your hand.
...
\parhead{Benefit} When you shove a creature, you can move it outside of your reach.
You also gain a \plus2 bonus to shove attacks.

\feat{Improved Dirty Trick}{Combat, Maneuver}
\parhead{Benefit} When you perform a dirty trick on a cretaure successfully, the creature is \vulnerable for 1d4 rounds.
You also gain a \plus2 bonus to dirty trick attacks.

\feat{Improved Disarm}{Combat, Finesse}
\parhead{Benefit} When you disarm a creature, you can make the disarmed item land up to 15 feet away in a random direction.
You also gain a \plus2 bonus to disarm attacks.

\feat{Improved Feint}{Combat, Finesse}
\parhead{Benefit} When you feint against a creature, if you beat its Armor defense by 5 or more, you deal damage with your weapon normally.
You also gain a \plus2 bonus to feint attacks.

\feat{Improved Imbued Spellstrike}{Metamagic}
\parhead{Prerequisite} Ability to cast 3rd level spells, Imbued Spellstrike.
\parhead{Benefit} This metamagic functions like Imbued Spellstrike, except that the imbuement lasts for 5 minutes if you have not struck a foe.
...
\parhead{Benefit} Grappling does not make you \vulnerable against other creatures.
You also gain a \plus2 bonus to grapple attacks.
...
\parhead{Benefit} When you attempt to overrun a foe, the creature may not choose to avoid you unless you let it.
You also gain a \plus2 bonus to overrun attacks.
...
\parhead{Benefit} When you successfully trip a foe, you get a free melee attack against it.
This attack is made as the creature is being tripped, so it does not have penalties for being prone.
You also gain a \plus2 bonus to trip attacks.

\feat{Improved Unarmed Strike}{Combat}
\parhead{Benefit} You treat your unarmed strike as if it were a lethal weapon.
...
\parhead{Normal} Attacking unarmed is difficult.
See \pcref{Unarmed Combat}.
\parhead{Special} A monk automatically gains Improved Unarmed Strike as a bonus feat at 1st level.
...
In exchange, you take a \minus4 penalty to physical defenses against ranged attacks.
\stylereq Wield a melee weapon.
You must take a full attack or charge action each round.
...
You can affect one creature per two Perform ranks you have.
An affected creatures gains a \plus2 enhancement bonus to physical attacks and defenses against fear.
This bonus increases by 1 at 8 Perform ranks and every 6 Perform ranks thereafter.
You can sustain the performance as a swift action.
...
You can affect one creature per two Perform ranks you have.
All affected creatures gain a \plus3 enhancement bonus to their caster level with spells and spell-like abilities.
...
\featben When you make a physical attack, you may take a penalty to hit of \minus4 or greater.
If you deal damage and the struck creature is within \rngmed range, you may make an Intimidate check as an immediate action to demoralize it (see \pcref{Demoralize}).
...
\parhead{Benefit} When gaining levels in either of your chosen classes, you increase your spellcasting ability with the other class as if your class was nonmagical.
See \pcref{Spellcasters and Multiclassing}.

\feat{Manyshot}{Combat, Style}
\parhead{Prerequisites} Dex 5, three attacks.
\parhead{Benefit} While in this style, when you attack with a light thrown weapon or projectile weapon, you may attack with two projectiles or weapons at once.
If the attack hits, you roll damage twice and take the higher of the two roll, as with two-weapon fighting (see \pcref{Two-Weapon Fighting}).
...
\parhead{Benefit} Once per round when your mount is hit in combat, you may attempt a Ride check as an immediate action to negate the hit.
...
\parhead{Benefit} When you attempt to overrun an opponent while mounted, your target may not choose to avoid you.
Your mount may make one attack with an appropriate natural weapon (hoof, claw, or other leg-based attack) against any target you knock down, gaining the normal \plus4 bonus to physical melee attacks against prone targets.
...
\parhead{Benefit} You gain two skill points.
You may spend these skill points immediately.

\feat{Overpowering Assault}{Combat, Style}
\featpres Str 3.
\parhead{Benefit} While in this style, you gain a \plus2 bonus to maneuver attacks.
In exchange, you take a \minus2 penalty to physical defenses.
\stylereq Wield a melee weapon.
You must take a full attack or charge action each round.

\feat{Overwhelming Fire}{Combat, Reaction, Style}
\parhead{Benefit} While in this style, you contribute to overwhelm penalties against all creatures within a single range increment of you.
You do not contribute to overwhelm penalties against creatures with cover from you.
In exchange, you take a \minus2 penalty to your physical defenses.
\stylereq Wield a ranged weapon.
You must take a full attack action each round.
...
Each parry attempt after the first takes a cumulative \minus5 penalty, just like your attack rolls do.
...
\parhead{Benefit} While in this style, your ranged attacks ignore cover and concealment, except total cover and total concealment.
...
\featpre Two attacks.
\featben When you make a physical attack with a ranged weapon, you may take a \minus4 penalty to hit.
If you deal damage, you make a Fortitude attack against the struck creature.
...
\parhead{Benefit} While in this style, you gain a \plus2 bonus to physical attack rolls with ranged weapons when attacking targets within half of your range increment.
...
\featpres Str 3.
\parhead{Benefit} While in this style, you gain a \plus2 bonus to physical melee damage rolls.
In exchange, you take a \minus2 penalty to physical melee attacks.
...
\parhead{Benefit} While in this style, you can ignore a number of creatures equal to your Perception (minimum 1) when determining whether targets have active cover from you when making ranged attacks.
In exchange, you must spend a full-round action to make a full attack.
\stylereq Wield a ranged weapon.
You must take a full attack action each round.
...
\parhead{Benefit} You can draw light weapons as a free action and medium weapons as a swift action.
...
\parhead{Benefit} Casting a quickened spell is a swift action.
...
\parhead{Special} All spellcasters cast a quickened spell as a swift action, even if they would normally increase the casting time of spells with metamagic applied.
...
\parhead{Prerequisite} Ability to cast 3rd level spells.
\parhead{Benefit} When casting a reach spell, you can use a spell with a range of touch on a target within \rngclose range.
...
In exchange, you gain a \plus3 bonus to physical damage with melee attacks.
\stylereq Wield a melee weapon.
You must take a full attack or charge action each round.
...
\parhead{Benefit} As part of the action to counter a spell, you may expend an Abjuration (Negation) spell of 4th level or higher.
If you do, the counterspelled spell is turned back on the caster as if it were affected by the \spell{spell turning} spell.
...
\parhead{Benefit} While in this style, any creature you deal damage to with a physical attack takes a \minus2 penalty to physical defenses for 1 round.
In exchange, you take a \minus2 penalty to physical attacks and damage.
\stylereq Wield a weapon.
You must take a full attack or charge action each round.

\feat{Ride-By Attack}{Combat}
\featpre Ride 8 ranks.
\parhead{Benefit} When you are mounted and use the charge action, you may move and attack as if with a standard charge and then move again (continuing the straight line of the charge).
...
The creature takes a \minus4 penalty to physical attacks against you, but is not otherwise encumbered by your presence unless it cannot support your weight.
...
\parhead{Prerequisite} Ability to cast 3rd level spells.
\parhead{Benefit} When casting a shaped spell, you can exclude any number of 5-foot cubes within the spell's area.
...
\featben Three times per day, when you successfully attempt to disarm an object, you may cause it to be affected as if by the targeted version of the \spell{shatter} spell with a caster level equal to your level.
Activating this ability is an immediate action.
...
Reducing a shield's encumbrance category causes tower shields to be treated as heavy shields (and no longer impose a \minus2 penalty to physical attacks), heavy shields to be treated as light shields, and both light shields and bucklers to be treated as being unarmored (allowing you to freely use other items in that hand).
...
\parhead{Special} Barbarians, clerics, druids, fighters, paladins, rangers, and spellwarped autogmatically have Shield Proficiency as a bonus feat.
...
\parhead{Prerequisite} Ability to cast 2nd level spells.
\parhead{Benefit} A silent spell can be cast with no verbal components.
...
Choose a skill.
\parhead{Benefit} You gain a \plus3 bonus to all checks involving that skill.
In addition, once per day you may reroll a skill check made with that skill.
\parhead{Special} You can gain this feat multiple times.
Its effects do not stack.
...
\parhead{Benefit} When using the Deflect Arrows feat, you may catch the weapon instead of just deflecting it.
...
\featben You gain a \plus2 bonus to your caster level when casting spells and using spell-like abilities from the school of magic or with the spell descriptor you select.
In exchange, you take a \minus2 penalty to caster level with all other spells and spell-like abilities.
...
\subcf{Alteration} \x
\subcf{Pyromancy} You can use any combustable item as a ritual component.
...
\parhead{Benefit} As a full-round action, you can move up to your speed and make your full attack.
...
\parhead{Prerequisite} Ability to cast 2nd level spells.
\parhead{Benefit} A stilled spell can be cast with no somatic components.
...
\parhead{Prerequisite} Dwarf.
\parhead{Benefit} You gain a \plus2 bonus to Craft and Awareness checks related to stone or metal.
...
This functions as the \spell{suggestion} spell, with an attack bonus equal to your Perform modifier.
...
\featben You may gain the benefits of the \spell{expeditious retreat} spell for up to 5 rounds each day.
Activating this ability is a swift action, and it lasts until you will it to stop (a free action) or until the duration expires.
...
This only affects spell duration, and has no effect on spells with special effects based on concentration, such as \spell{call lightning}.
...
\parhead{Benefit} You can attempt to identify the strengths and weaknesses of creatures based on your combat experience.
As a swift action, you can make a special check with a bonus equal to your base attack bonus \add your Intelligence \add 2 per round you have seen the creature fight.
...
\parhead{Benefit} You can attempt to predict what your opponent will do.
As a swift action, you can make a special check with a bonus equal to your base attack bonus \add your Intelligence \add 2 per round you have seen the creature fight.
...
This does not remove any other penalties for fighting at close range, such as the danger of being \defenseless.
...
\feat{Track}{Skill}
\parhead{Benefit} To find tracks or to follow them for 1 mile requires a successful Survival check.
You must make another Survival check every time the tracks become difficult to follow.
You move at half your normal speed (or at your normal speed with a \minus5 penalty on the check, or at up to twice your normal speed with a \minus20 penalty on the check).
...
\par \emph{Firm Ground:} Most normal outdoor surfaces (such as lawns, fields, woods, and the like) or exceptionally soft or dirty indoor surfaces (thick rugs and very dirty or dusty floors).
...
\par \emph{Hard Ground:} Any surface that doesn't hold footprints at all, such as bare rock or an indoor floor.
...
\parhead{Normal} Without this feat, you can use the Survival skill to find tracks, but you can follow them only if the DC for the task is 10 or lower.
...
\parhead{Special} A ranger automatically has Track as a bonus feat.
...
\parhead{Benefit} You learn how to make items from any school of magic, including universal spells.
...
\parhead{Benefit} You gain a special ability based on the weapon group chosen.
...
In addition, armor spikes no longer impose a penalty to your physical defenses.
    ...
    \item Blades, light: If you draw a light blade and attack with it in the same round, you gain a \plus2 bonus to attack.
The creature you attack is unaffected by this ability for the rest of the encounter.
    \item Blunt weapons: When you deal damage to a creature, it takes a \minus2 penalty to Will defense for 1 round.
...
Choose one weapon group.
\parhead{Benefit} You are proficient with weapons from your chosen weapon group.
This allows you to defend yourself normally with melee weapons, and to attack with exotic ranged weapons without penalty.
...
\parhead{Special} You can gain Weapon Proficiency multiple times.
...
\parhead{Benefit} You can alter a burst, emanation, line, or spread shaped spell to increase its area.
