---
trigger: glob
description: Applied when editing LaTeX (.tex), Rust (.rs), or TypeScript (.ts) files to ensure adherence to Rise project coding and formatting standards.
...
* **Multiline Strings**: Start with `r"` followed by a linebreak. Indent text inside the string by one level.
