...
\sectiongraphic*{Making Attacks}{width=\columnwidth}{combat/making attacks}
  ...
  \subsection{Attack Rolls}\label{Attack Rolls}
    ...
    The resulting value is called your \glossterm{attack result}.
    You compare your attack result to one or more \glossterm{defenses} to see if you hit.
  ...
  \subsection{Attack Outcomes}
    There are three possible outcomes for an attack: a miss, a critical hit, and a regular hit.
    ...
    \subsubsection{Critical Hits}\label{Critical Hits}
      ...
      For example, if you get a double critical with an attack that has a critical hit effect of ``The condition must be removed an additional time'', the condition must be removed two additional times.
...
\section{Calculating Damage}\label{Calculating Damage}
  Most damaging abilities tell you how much damage they deal in their description, such as "\damagerankone".
  This section explains some common damage calculations.

  \subsection{Strike Damage}\label{Strike Damage}
    When you hit with a \glossterm{strike}, you deal damage equal your \glossterm{weapon damage} plus half your relevant \glossterm{power}.
    This is called your \glossterm{strike damage}.

    If a \magical ability causes you to make a strike, you use your \glossterm{magical power}.
    Otherwise, you use your \glossterm{mundane power}.
    If you are using a \weapontag{Heavy} or \weapontag{Versatile Grip} weapon in two hands, you add your full power instead (see \pcref{Weapon Tags}).

    % TODO: can any ability modify weapon damage?
    \subsubsection{Weapon Damage}\label{Weapon Damage}
      Your \glossterm{weapon damage} is normally equal to your weapon's damage die or dice, as defined in the Equipment chapter (see \pcref{Weapons}).
      Some abilities multiply your weapon damage with a particular strike.
      That does not multiply other damage dealt by the strike, such as the damage from your \glossterm{power} or any \glossterm{extra damage}.

    \subsubsection{Weapon Dice Increment}
      A bonus to weapon dice increment increases your \glossterm{weapon damage}.
      Each \plus1 bonus to your weapon dice increment increases your weapon damage by one step in the order below:
      \begin{raggeditemize}
        \item 1d2
        \item 1d3
        \item 1d4
      ...
      Likewise, a \minus1 penalty to your weapon dice increment reduces your weapon damage by one step in that order.
      Modifiers to weapon dice increment are rare.
  ...
  \subsection{Extra Damage}\label{Extra Damage}
    ...
    It is doubled on a critical hit, and so on.
    ...
    For example, the \spell{caustic grasp} spell deals damage immediately, and again at the end of the target's next turn.
    ...
    Extra damage is only applied to damage directly caused by your attacks.
    It does not apply to damaging \glossterm{conditions} you inflict, \glossterm{poisons}, or other special effects.
    For example, an empowered creature that uses an ability which \glossterm{flings} a creature does not apply the extra damage to the impact from the fling, if any.

  \subsubsection{Damage Penalties}\label{Damage Penalties}
    Some effects cause you to suffer a \glossterm{damage penalty}.
    Damage penalties reduce the damage you deal.
    Like extra damage, damage penalties apply before any doubling or halving effects on the attack, and they only apply once per attack roll to any individual creature.
    For example, if you get a \glossterm{critical hit} while \sickened, you would subtract twice your rank from the result instead of half your rank.
...
\section{Taking Damage}\label{Taking Damage}
  ...
  \subsection{Injury}\label{Injury}
    ...
    When you take damage from an attack, if you are injured, the attack inflicted an \glossterm{injury} on you.
  ...
  \subsection{Negative Hit Points}\label{Negative Hit Points}
    You can have negative hit points, but only temporarily.
    ...
    At the start of your turn, your hit points are reset to 0 if they are still negative.
    This also resets your vital wound thresholds, which means you'll suffer a vital wound if your hit points go negative again afterwards.
  ...
  \subsection{Subdual Damage}\label{Subdual Damage}
    ...
    If an attack that deals subdual damage would inflict a vital wound, the target reduces its \glossterm{stamina} by three instead.
    Whenever you make a \glossterm{strike}, you can choose to deal subdual damage instead of normal damage.
    If you do, the strike deals half damage.

\sectiongraphic*{Vital Wounds}{width=\columnwidth}{combat/vital wounds}
  ...
  \subsection{Vital Rolls}\label{Vital Rolls}
    ...
    \begin{columntable}
      \columncaption{Vital Wound Effects}
      \begin{dtabularx}{\columnwidth}{l X}
        ...
        1               & You are unconscious while you are \glossterm{injured}     \\
        2               & You take a \minus1 penalty to \glossterm{accuracy}               \\
        3               & You have a \minus5 foot penalty to your \glossterm{speed}        \\
        4               & You take a \minus2 penalty to your \glossterm{maximum stamina} \\
...
\section{Combat Time}\label{Combat Time}
  ...
  \subsection{Turns}\label{Turns}
    ...
    Unless otherwise specified, if an ability functions ``per turn'', it is counting your turns, not every turn.

    Turn order in combat is determined by rolling \glossterm{initiative}.

  \subsection{Initiative}\label{Initiative}
    When combat starts, you can act independently, or you can choose an \glossterm{ally} that you can see and hear to lead you.
    If you act independently, you roll \glossterm{initiative}.
    Initiative is neither an attack nor \glossterm{check}.
    You just roll 1d10 and keep the result.
    A few special abilities can give you a bonus to your initiative.

    If you choose an ally to lead you, you use the result of their initiative check instead of rolling yourself.
    Either way, this determines your initiative result.

    Initiative order proceeds from the highest initiative result to the lowest.
    If two creatures have the same initiative result, the higher initiative modifier wins.
    If they are still tied, they independently roll initiative against each other to determine who acts first.

    If you lead your allies during initiative, you must clearly and obviously communicate with them.
    This generally requires motion and noise which make your presence known to enemies.
    A \glossterm{difficulty value} 5 Awareness check is enough to recognize your presence and identify that you are acting as a leader.
    Similarly, you can identify the leader in enemy groups with a DV 5 Awareness check.

    \subsubsection{Initiative Groups}\label{Initiative Groups}
      Once everyone in a combat has determined their initiative order, allies with no enemies between them in the initiative order are bundled into initiative groups.
      Each initiative group gets a single shared turn.
      Within that turn, each creature in the initiative group can take actions in any order.

      Once you have started an action, you must complete it before any creature in the initiative group can take another action.
      As a special case, you can use \glossterm{free actions} and \glossterm{minor actions} during the resolution of your own \glossterm{move actions}.
      For example, you can walk up to a door, open it, and continue through the door (see \pcref{Manipulating Objects}).
      However, you can't move up to a creature, use a standard action to attack it, and move away with the same move action.

      After all creatures in an initiative group taken all of their intended actions, the next initiative group can act.
      Since your whole initiative group shares a turn, effects that last "until the end of your turn" last until the end of your full initiative group's turn, regardless of when you take your actions during that turn.

    \subsubsection{Initiative Example}
      Alice, Bob, Carol, and Dave are player characters in an adventuring party.
      They come across a group of goblins led by a troll.
      Alice and Dave decide to act independently, while Bob declares Carol as his leader.
      The goblins all declare the troll as their leader.

      When initiative is rolled, Alice rolls an 4, Dave rolls a 6, the goblin shaman rolls a 7, and Carol rolls a 9.
      Bob and the individual goblins don't roll initiative, since they designated leaders.
      Bob and Carol go first as a group, followed by the troll and goblins as a group.
      Alice and Dave are grouped together, since there are no enemies between them.
      In addition, Bob and Carol's \textit{next} turn joins that group, since there are no enemies between them.
      This means that after Bob and Carol's first turn together, there are only two alternating turns for the rest of combat.

      \begin{enumerate}
        \item Bob and Carol (once)
        \item The troll and goblins (every time)
        \item Alice, Bob, Carol, and Dave (every time)
      \end{enumerate}

  \subsection{Surprise and Pre-Combat Actions}
    In some circumstances, one or more groups may be aware that combat will happen before it actually starts.
    For example, a creature at a tense negotiation may secretly drink a potion before a fight breaks out.
    This can also cover scenarios where one group actively initiates combat, such as by breaking down a door or attempting an assassination.

    In such cases, everyone who is pre-aware of combat rolls initiative, ignoring creatures not yet aware of the combat.
    If only one allied group is pre-aware of combat and they can communicate effectively, you can skip rolling initiative, since they'll all be joined into a single turn anyway.
    Once all pre-aware creatures have taken their turns, decide whether the other creatures are aware that combat has started.
    If they are, they roll initiative and join the combat, with the winner of that initiative roll taking their turn immediately.
    Otherwies, they take a turn doing whatever they would normally do outside of combat, such as talking amongst themselves, and this process repeats.

  \subsection{Deciding Action Order}\label{Deciding Action Order}
    Try not to spend too much time on the exact order of everyone's actions within your initiative group.
  ...
  \subsection{Delaying Your Turn}
    You can freely delay your turn to join a later initiative group.
    However, you can't join in the middle of an enemy initiative group to try to break it up.
    You can only join allied initiative groups or create a new initiative group after an already existing enemy initiative group.

    When you delay your turn, any effects that would have happened at the start or end of your turn still happen at your original initiative order.
    You can't extend the duration of \brief effects by delaying your turn.

  % TODO: move to a more relevant location
  \subsection{Simultaneous Damage and Healing}\label{Simultaneous Damage and Healing}
    If you regain hit points and take damage simultaneously, add together all of the modifiers to your hit points before checking if you gained a vital wound or exceeded your maximum hit points.
    For example, suppose you end your turn with 5 hit points out of a maximum of 10.
    At the end of your turn, you regenerate to heal 20 hit points and burn for 10 damage.
    You would end up at 10 hit points, having not gained a vital wound.

  % TODO: solve the peasant railgun in a sequential initiative system
  % \subsection{Group Size Limits}\label{Group Size Limits}
  %   There is a limit to how many creatures can coordinate in combat.
  %   In general, no more than twenty creatures can constitute an initiative group.
  %   Larger groups than that are still allies, but they must resolve actions within their group without perfect knowledge of or coordination with other allied groups.

% TODO: Clean up this wording, this whole section is awkward with the movement -> move action change
    \sectiongraphic*{Basic Movement}{width=\columnwidth}{combat/basic movement}
  ...
  \subsection{Movement Speed}\label{Movement Speed}
    Your \glossterm{speed} is how far you can normally move during your turn.
    Your speed is normally equal to your \glossterm{base speed}, which is derived from your size (see \pcref{Size in Combat}).
    Some abilities can modify your speed.
    At the start of your turn, your available movement becomes equal to your speed.
    Any movement you make subtracts from your available movement until it reaches 0, which prevents you from moving farther.
    Some abilities can increase your available movement, including above your speed.
    ...
    \subsubsection{Shared Movement Limits}\label{Shared Movement Limits}
      Any non-forced movement that moves you subtracts from your available movement, even if you aren't the one causing the movement.
      If your available movement reaches zero, you can't be moved any farther by non-forced movement, even if you're being carried by an ally.
      For example, you can't move your full speed onto a mount and then have that mount move with you in the same turn.
      You can still be moved by \glossterm{push}, \glossterm{fling}, and teleportation effects.

  % Pure movement abilities don't have Physical if they can be used by incorporeal creatures
  \subsection{Move Action Abilities}\label{Move Action Abilities}
    Each of these abilities requires a \glossterm{move action} to use.
    ...
    \begin{activeability}{Block}{Move action}
      \abilitytags \atPhysical
      \rankline
      When you use this ability, choose a creature you can see.
      Until your next turn, the next time the target attempts to move out of a space adjacent to you, you attempt to block its movement.
      When you do, make an opposed \glossterm{initiative} check against it.
      If you beat it on the initiative check, it must spend additional movement equal to your available movement in order to move from its space.
      This consumes all of your available movement.
      If it cannot, it stops moving.
      This represents you automatically repositioning yourself to block its movement.

      If a creature has the ability to move through your space, or if you stop being able to see it and react to its movements, it is unaffected by this additional movement cost.
    ...
    % TODO: weird wording
    \begin{activeability}{Hustle}{Move action}
      \rankline
      Choose a path that you want to travel.
      You travel that path, up to the limit of your available movement.
    \end{activeability}

  \subsection{Measuring Distance}

    For simplicity, all distances in combat are measured in five-foot increments.
    ...
    \parhead{Diagonals}\nonsectionlabel{Diagonals} When measuring distance, the first diagonal counts as five feet of movement, and the second counts as ten feet of movement.
    ...
    \subsubsection{Vertical Distance}
      Vertical distance is measured differently from horizontal distance.
      When measuring distance between two points, use the maximum of the vertical distance and the horizontal distance.
      Don't calculate an appropriate diagonal distance, like you would in real life.
      This is unrealistic, but it's a useful simplification.

      For example, a spell with 30 foot range can target something that is 30 feet away horizontally \textit{and} 30 feet away vertically.
      However, it could not target something that is 35 feet away horizontally or 35 feet away vertically.
      Similarly, moving in three dimensions (such as flying or jumping) consumes \glossterm{available movement} equal to the maximum of the horizontal distance and vertical distance moved.
      Abilities that affect an area are generally shaped to affect vertical and horizontal dimensions similarly (see \pcref{Area Shapes}).
  ...
  \subsection{Movement Impediments}
    ...
    Enemies and large solid objects like walls completely block your movement, preventing you from entering the space they occupy.
    Some abilities can allow you to move through spaces occupied by enemies, which means you do not treat them as obstacles.

    \parhead{Undergrowth}\nonsectionlabel{Undergrowth} Vines, roots, bushes, and similar plants can obstruct sight and impede movement.
    ...
    Heavy undergrowth provides \glossterm{concealment} and is \glossterm{difficult terrain}.
...
\section{Advanced Movement}
  ...
  \subsection{Movement Modes}\label{Movement Modes}
    ...
    Each of your movement modes has an associated \glossterm{speed category} (see \pcref{Speed Categories}).
    ...
    It must still make an Athletics check to move in challenging conditions, such as slippery walls (see \pcref{Athletics}).
    ...
    It must still make an Athletics check to move in challenging conditions, such as stormy seas (see \pcref{Athletics}).
    A creature that is in water without a \glossterm{swim speed} is \unsteady.
  ...
  \subsection{Speed Categories}\label{Speed Categories}
    ...
    \begin{raggeditemize}
      \item Fast: Moving 10 feet with this movement mode costs 5 feet from your speed.
      ...
      \item Average: Moving 5 feet with this movement mode costs 5 feet from your speed.
        When moving on diagonals, the first diagonal costs 5 feet of movement and the second costs 10.
      \item Slow: Moving 5 feet with this movement mode costs 10 feet from your speed.
        When moving on diagonals, each diagonal costs 15 feet of movement.
    \end{raggeditemize}

  \subsection{Combining Movement Modes}
    You can move using multiple different movement modes in the same turn in any order.
    Each distance moved subtracts from your available movement during that turn, using the appropriate modifiers for fast or slow movement modes.

    For example, assume you have a 40 foot base speed, an average walk speed, and a slow swim speed.
  ...
  \subsection{Jumping}\label{Jumping}
    ...
    A jump's maximum horizontal distance is normally equal to a quarter of your \glossterm{base speed}.
    If your Strength is positive, you add 5 feet per 2 Strength to your jump distance.
    If you are trained with the Athletics skill, the distance you add from your Strength is instead equal to 5 feet per Strength, to a minimum of \plus5 feet.
    Modifiers to your available movement, such as the \ability{sprint} ability, do not affect your jumping distance.

    A jump's maximum vertical height is equal to half your maximum horizontal distance.
    Note that if you jump 20 feet into the air or more, you may take falling damage when you land (see \pcref{Falling Damage}).
    % TODO: minimum jump height? too complicated?

    When you jump intentionally, you do not start falling or become \unsteady until the end of your turn.
    This allows you to jump using a \glossterm{move action} and then take a \glossterm{standard action} while midair without penalty.

    \subsubsection{Jumping Speed Limits}
      Movement while jumping subtracts from your \glossterm{available movement} just like any other movement.
      ...
      This limits both your maximum horizontal distance and your maximum vertical height, using the maximum of the horizontal and vertical distance jumped.
      For example, if your speed is thirty feet and you start by walking ten feet, you have twenty feet of available movement left, allowing you to jump up to twenty feet horizontally and vertically.
      If your jump distance is extremely high and your speed is low, you may need to use the \ability{run} or \ability{sprint} abilities to make use of your full jumping potential (see \pcref{Sprint}).
  ...
  \subsection{Squeezing}\label{Squeezing}
    ...
    Squeezing is only checked at the start of each turn.
    If you temporarily enter a creature's space during a turn and leave it during that same turn, neither you nor that creature are ever squeezing.
  ...
  \subsection{Forced Movement}\label{Forced Movement}
    ...
    There are two kinds of forced movement: \glossterm{push} effects and \glossterm{fling} effects.
    ...
    \subsubsection{Push Effects}\label{Push Effects}
      ...
      When a creature would fall while being pushed, it automatically makes a \glossterm{reactive check} with the Athletics skill to avoid falling (see \pcref{Athletics}).

    \subsubsection{Fling Effects}\label{Fling Effects}
      A creature affected by a \glossterm{fling} effect is thrown backwards by a single point of impact.
      It always travels in a single straight line.
      If it encounters an \glossterm{obstacle}, the fling ends.
      Both the creature and the obstacle take 1d8 damage per 10 feet of movement remaining.

      Unless otherwise noted, a fling effect can move a creature in any direction, including upwards or directly into the ground.
      ...
      If you fling a creature vertically, it is typically \unsteady until it lands at the end of its turn.

\section{Aerial Movement}\label{Aerial Movement}
  Creatures with a fly speed or glide speed can move in midair.
  To use a fly or glide speed, a creature must not exceed its maximum \glossterm{carrying capacity} (see \pcref{Weight Limits}).

  While \glossterm{midair}, land-based creatures are \unsteady, even if they have a fly or glide speed.
  ...
  If you jump into the air intentionally, you don't become unsteady until the end of your turn (see \pcref{Jumping}).

  \parhead{Falling and Catching Falls}
  If a flying or gliding creature becomes \helpless or chooses to fall, it falls like any other creature (see \pcref{Falling Damage}).
  \trait{Floating} creatures do not fall in this way.

  A flying or gliding creature can catch itself while falling as a \glossterm{move action}.
  This consumes all of its \glossterm{available movement} for the turn, but does not cause any \glossterm{falling damage}.
  If it enters the air willingly, such as by jumping, starting to fly or glide does not require a move action or reduce its available movement.

  All \glossterm{mundane} flight abilities require air to push against, so they do not function in places without an atmosphere.
  Magical flight abilities function without air.

  \subsection{Gliding}\label{Gliding}
    A creature with a glide speed can move horizontally, but not vertically, in midair using normal movement abilities.
    At the end of each of its turns, it descends 10 feet.
    This descent does not count against its \glossterm{available movement} during its turn.

    In order to glide, a creature must be able to fall.
    Since \trait{floating} creatures cannot fall, they also cannot glide.
    Gliding is impossible in areas without significant gravity, such as in space.
  ...
  \subsection{Flight}\label{Flight}
    ...
    Moving in three dimensions consumes \glossterm{available movement} equal to the maximum of the vertical distance and horizontal distance moved (see \pcref{Vertical Distance}).
...
\section{Strikes}\label{Strikes}
  ...
  Strikes have all tags from the weapon or weapons you use to make the strike.
  ...
  \begin{activeability}{Basic Strike -- Melee}{Standard action}
    ...
    You add your weapon's accuracy modifier to this attack.
    You must have \glossterm{line of effect} to the target.
    % TODO: wording
    \hit You deal the target your \glossterm{strike damage} (see \pcref{Strike Damage}).
  \end{activeability}
  ...
  \subsection{Dual Strikes}\label{Dual Strikes}
    ...
    When you make a dual strike, you take a \minus3 accuracy penalty.
    ...
    You still only add the damage bonus from your \glossterm{power} once to the combined weapon, not once for each weapon.
    ...
    For example, using two magical Flaming weapons would provide 2d4 \glossterm{extra damage}, and using two Keen weapons would give you a \plus4 accuracy bonus for the purpose of getting a critical hit.
    However, \glossterm{enhancement bonuses} still do not stack.
    ...
    \begin{raggeditemize}
      ...
      \item \abilitytag{Impact}: If both weapons are Impact, you deal half damage when you would miss by 8 or less.
      \item \weapontag{Light}: If both weapons are Light, you gain a \plus3 accuracy bonus and the combined weapon is considered Light.
        If only one weapon is Light, you gain a \plus2 accuracy bonus and the combined weapon is not considered Light.
      \item \weapontag{Long}: Only functions if both weapons are Long.
      \item \weapontag{Parrying}: The bonus stacks, even though it does not stack with the benefits of using a shield.
      ...
      \item \abilitytag{Subdual}: Only functions if both weapons are Subdual.
    ...
    \begin{raggeditemize}
        % = +2a both weapons, +2a light, -3a base
      \item Broadsword and smallsword: \plus1 accuracy, 1d6\plus1d4 damage, Versatile Grip (irrelevant unless you have three hands)
        % = +2a both weapons, +3a light, -3a base
      \item Two kukris: \plus2 accuracy, 2d4 damage, Sweeping (2)
        % = +3a light, -3a base
      \item Two kama: \plus0 accuracy, 2d4 damage, Keen+ (\plus4 accuracy with critical hits)
        % = -2a both weapons, -3a base
      \item Two totokias: \minus5 accuracy, 2d10 damage, Impact+ (half damage if miss by 8 or less)
        % = +2a light, -3a base
      \item Throwing axe and handaxe: \minus1 accuracy, 1d6\plus1d4 damage, Thrown (30/60)
        % = -1a warhammer, +2a light, -3a base
      \item Warhammer and light hammer: \minus2 accuracy, 1d8\plus1d4 damage, Impact+ (half damage if miss by 8 or less), Versatile Grip (irrelevant unless you have three hands)
        % = +2a both weapons, +3a light, -3a base
      \item Two claws: \plus2 accuracy, 2d4 damage
    \end{raggeditemize}
    ...
    \subsubsection{Split Strikes}
      ...
      This does not reduce damage that is not directly part of the strike, such as the triggered damage from the \ability{quivering palm} maneuver.
...
\section{Universal Combat Abilities}\label{Universal Combat Abilities}
  ...
  \begin{dtable!*}
    \lcaption{Universal Combat Abilities}
    \begin{dtabularx}{\textwidth}{l l X l}
      \tb{Ability}       & \tb{Usage Time}  & \tb{Short Description}                        & \tb{Tags}                              \tableheaderrule
      Desperate Exertion & Non-action       & Reroll an attack or check with a \plus2 bonus & \tdash                                 \\
      Escape Grapple     & Standard         & Stop being grappled                           & \atBrawling, \atPhysical               \\
      Grapple            & Standard         & Start grappling with a foe                    & \atBrawling, \atPhysical, \atSizeBased \\
      Ready Reaction     & Standard         & Ready an action                               & \tdash                                 \\
      Recover            & Standard         & Regain HP, remove conditions                  & \tdash                                 \\
      Run                & Standard         & Briefly double your available movement        & \tdash                                 \\
      Shove              & Move or standard & Move a foe                                    & \atBrawling, \atPhysical               \\
      Sprint             & Free             & Double your available movement                & \tdash                                 \\
      Trip               & Standard         & Trip a foe                                    & \atBrawling, \atPhysical                \\
    \end{dtabularx}
  \end{dtable!*}

  \begin{triggeredability}{Desperate Exertion}{Triggered (see text)}
    \rankline
    You \glossterm{reroll} any \glossterm{attack roll} or \glossterm{check} you just rolled, keeping the better result.
    You gain a \plus2 bonus with the reroll.
    ...
    After you use this ability, you reduce your \glossterm{stamina} by two.
    This is not a cost, and it applies after you resolve the attack or check.
  ...
  \begin{activeability}{Escape Grapple}{Standard action}
    \abilitytags \atBrawling, \atPhysical
    \rankline
    Make a \glossterm{brawling attack} vs. Brawn against any number of creatures that you are grappled by.
  ...
  \begin{activeability}{Grapple}{Standard action}
    \abilitytags \atBrawling, \atPhysical, \atSizeBased
    \rankline
  ...
  \begin{activeability}{Ready Reaction}{Standard action}
    ...
    If your intended action ever becomes possible, between the end of your current turn and the start of your next turn, it happens immediately.
    ...
    Your reaction resolves as soon as it becomes possible, interrupting other actions partway through their execution.
    This means that if you ready to hit an enemy as soon as it runs up to you, and it moves up to attack you, you will attack before it does.

    % TODO: you can become immune to reactions if you ready *all* of your actions
    You cannot react to the readied reaction of another creature.
    If your reaction becomes possible only as a result of the readied reaction of another creature, you do nothing.

    If multiple creatures ready reactions in response to the same trigger, all of those readied reactions occur.
    They resolve in the order that the creatures readied their reactions, so the creature that readied its reaction first goes first.
    This can mean that the readied reaction of the last creature is wasted, such as if the previously resolved readied reactions already killed the triggering creature.
  \end{activeability}
  ...
  \begin{activeability}{Recover}{Standard action}
    ...
    Then, you remove all \glossterm{conditions} affecting you.
  \end{activeability}

  \begin{activeability}{Run}{Standard action}
    \rankline
    Add your \glossterm{speed} to your \glossterm{available movement}.
    At the start of your next turn, add your speed to your available movement again.
  \end{activeability}

  \begin{activeability}{Shove}{Standard action}
    \abilitytags \atBrawling, \atPhysical
    \rankline
    If you are grappling or being grappled by a creature, you can use this ability as a \glossterm{move action}, and it affects all creatures grappling with you (see \pcref{Grappling}).
    ...
    % Can't use crit for extra movement here to limit your ability to push party members in excess of their normal speed.
    \hit You can move up to half your \glossterm{speed}, reducing your \glossterm{available movement} as normal.
    ...
    When pushing a target off of a ledge, if you move with the target over the edge to fall with them, it cannot make the normal reactive Athletics check to grab the ledge (see \pcref{Push Effects}).
  \end{activeability}

  \begin{activeability}{Sprint}{Free action}
    \abilitycost One \glossterm{stamina}. You can only use this ability once per turn.
    \rankline
    You add your \glossterm{speed} to your \glossterm{available movement}.
  \end{activeability}
  ...
  \subsection{Grappling}\label{Grappling}
    ...
    \begin{raggeditemize}
      \item One of your hands cannot be used for any purposes other than using \abilitytag{Brawling} abilities, including maintaining the grapple.
        This prevents humanoid creatures from taking any actions that would require having two free hands, such as attacking with \weapontag{Heavy} weapons.
      ...
      \item You can use the \ability{escape grapple} ability to stop grappling (see \pcref{Universal Combat Abilities}).
      \item You can attempt to release the grapple as a \glossterm{free action}. If each other creature you are grappling with used \ability{escape grapple} ability or attempted to release the grapple in this way since your last turn, the grapple immediately ends for all participants. Otherwise, it continues.
    \end{raggeditemize}
    ...
    \subsubsection{Controlling a Grapple}\label{Controlling a Grapple}
      ...
      Second, you can release the grapple as a \glossterm{free action} regardless of whether the other participants want to leave the grapple.
...
\sectiongraphic*{Vision and Light}{width=\columnwidth}{combat/vision and light}
  ...
  Some creatures have \sense{darkvision} or other extraordinary senses, but most creatures need light to see by.
  In an area of \glossterm{bright illumination}, all characters can see clearly.

  Creatures can see only dimly into areas that have \glossterm{dim illumination}.
  ...
  In areas of total darkness, creatures without \sense{darkvision} or some other form of supernatural vision are \blinded.

  \subsection{Emitting Light}
    Some items and abilities emit light.
    Any effect which creates bright or brilliant illumination in an area also creates enough light for \glossterm{dim illumination} in twice that area.

    For example, a simple torch emits \glossterm{bright illumination} in a \smallarea radius.
    That means it also creates \glossterm{dim illumination} between 15 feet and 30 feet from the torch in a dark room.
  ...
  \subsection{Concealment}\label{Concealment}
    Concealment represents anything which makes it more difficult to see your target, such as \glossterm{dim illumination}.
...
\section{Obstacles and Cover}\label{Obstacles and Cover}
  Obstacles can impede attacks by blocking \glossterm{line of sight} or \glossterm{line of effect}, or by providing \glossterm{cover}.
  This section explains how to deal with these limitations.

  % TODO: should this be "use an ability" instead of "make an attack"?
  % It also seems like this belongs in a different section.
  \subsection{Point of Origin}\label{Point of Origin}
    ...
    % This has to not just be emanations; consider effects like Powderkeg, which shouldn't be forced to choose a single point of origin.
    Cylinders without a range are unusual, since they radiate from your whole body instead of a single point.
    When determining their total size, treat every grid intersection touching the source creature or object's space as a point of origin.
  ...
  \subsection{Cover}\label{Cover}
    ...
    A creature or object behind cover gains a \plus2 bonus to Armor and Brawn defenses.
    If an attack against any of those defenses would miss the defense of a creature or object behind cover, the attack is applied to the obstacle instead of the intended target.
    ...
    This can protect creatures behind cover from attacks that deal damage on a miss.
    ...
    \subsubsection{Measuring Cover}
      ...
      For creatures that occupy multiple 5 ft. squares, these must be corners where the target's space ends, not just grid intersections touching squares the target occupies.
    ...
    \subsubsection{Improved Cover}
      ...
      However, exceptionally well covered creatures, such as a creature behind an arrow slit in a castle, may gain a \plus4 benefit to Armor and Brawn defenses rather than the normal \plus2 at the GM's discretion.
  ...
  \subsection{Line of Effect}\label{Line of Effect}
    ...
    To measure line of effect for a particular attack, draw a line between the attack's \glossterm{point of origin} and any grid intersection touching the target's space.
    If you're targeting a particular point, you would naturally draw the line to that point instead.
...
\section{Awareness and Surprise}\label{Awareness and Surprise}
  ...
  For example, a creature that is already in combat who is attacked by a previously unseen foe is partially unaware of the attack.
  ...
  Like other exertions, this time can be extended with the Endurance skill (see \pcref{Endurance}).
  ...
  \subsection{Surprise Attacks}\label{Surprise Attacks}
    ...
    It is usually \unaware of all attacks until that point, though creatures expecting imminent attack may be \partiallyunaware.
...
\section{Special Combat Rules}
  ...
  \subsection{Fighting In Water}\label{Fighting In Water}
    Any creature that is immersed or submerged in water and does not have a \glossterm{swim speed} is \unsteady.
    As long as the creature has a defined swim speed, it does not suffer this penalty, even if its movements are restricted or if it fails a Athletics check to navigate the water.
    ...
    In clear water, depths below 120 feet have only \glossterm{dim illumination}, and depths below 500 feet are completely dark.
  ...
  \subsection{Mounted Combat}\label{Mounted Combat}
    ...
    At the start of your turn, you must make a \glossterm{difficulty value} 10 Poise check to control such a horse.
    Success means you can act normally that turn, directing the horse's movements as if it was trained for combat.
    Failure means that the horse acts of its own volition that turn, usually fleeing in panic.
    ...
    The \glossterm{difficulty value} for all Poise checks on a mount using a fly speed is increased by 10.

    \parhead{Combat while Mounted} You must make a \glossterm{difficulty value} 5 Poise check at the start of your turn while riding a mount.
    Success means you can guide and stay on your mount without using your hands.
    Failure means you must use one \glossterm{free hand} to control your mount during that turn, or you fall off if you cannot free a hand.
  ...
  \subsection{Allies and Enemies}\label{Allies and Enemies}
    ...
    You can choose how you consider each creature at the start of your turn.
...
\section{Using Abilities}\label{Using Abilities}
  ...
  \subsection{Magical and Mundane Abilities}\label{Magical and Mundane Abilities}
    ...
    That strike would use your \glossterm{mundane power}, and it would be unable to affect \trait{incorporeal} creatures.
  ...
  \subsection{Ability Choices}\label{Ability Choices}
    Most abilities require choices, such as which creatures or areas are affected.
    Some abilities also require choices unique to the ability, such as the weather pattern for \ritual{control weather}.
    You make all of these choices when you start using the ability, before resolving any part of its effects.
    For \glossterm{rituals}, you make these choices when you start performing the ritual, not when you finish performing it.
  ...
  \subsection{Touch Range}
    ...
    Some creatures cannot be touched, such as \trait{incorporeal} creatures.
  ...
  \subsection{Ability Targeting}\label{Ability Targeting}
    ...
    \subsubsection{Invalid Targets}
      ...
      For example, you can try to cast a healing spell on a rock if you want, even if the healing spell has the \atLife tag and says it can only target creatures.
    ...
    \subsubsection{Targeting Proxies}\label{Targeting Proxies}
      ...
      Unless otherwise stated, you must have both \glossterm{line of sight} and \glossterm{line of effect} to a targeting proxy to use it.
  ...
  \subsection{Area Shapes}\label{Area Shapes}
    ...
    \parhead{Cone} A cone extends from the point of origin in a quarter-cylinder, up to the given length.
    Unless otherwise specified, a cone's height is equal to its length, and the cone extends upwards from the point of origin.
    ...
    \parhead{Cylinder} A cylinder extends out from the point of origin horizontally in a circle, up to the given radius.
    Cylinders also have a specific height.
    Unless otherwise specified, a cylinder's height is the same as its radius.
    Bursts and zones extend upwards from the point of origin.
    Emanations extend both upwards and downwards from each point of origin.
    Any ability which only specifies a radius for its area is cylinder-shaped.
    ...
    A wall's height is equal to its length, to a minimum of 5 feet high.
    ...
    All squares affected by a wall must be contiguous, so every square is adjacent to another affected square, treating diagonals as adjacent.
    ...
    Many walls have effects on things that pass through them.
    Creatures pass through a wall when moving through it and when making melee \glossterm{strikes} through it using non-\weapontag{Long} weapons.
  ...
  \subsection{Area Types}\label{Area Types}
  ...
  \subsection{Shapeable Areas}\label{Shapeable Areas}
    ...
    In all cases, a shapeable area must still be a single contiguous shape that includes the area's \glossterm{point of origin}.
    \begin{raggeditemize}
      ...
      \item Wall: The wall can bend at 45 degree or 90 degree angles at any number of points during its path.
    \end{raggeditemize}

    For example, you could convert a shapeable cylinder into a cube or square, as long as the cube or square would fit entirely within the original area affected by the cylinder.
  ...
  \subsection{Impossible Abilities}
    ...
    It can also happen if the effects are impossible, such as enlarging a creature that is already in a confined space that leaves it no room to expand.
  ...
  \subsection{Impossible Consequences of Ending Abilities}
    Sometimes, abilities can end in a way that creates normally impossible circumstances.
    Since this results from an ability \textit{ending} instead of being used, the ability must still end, and the rules for using impossible abilities don't apply.
    Instead, everything involved in the impossible situation immediately takes the minimum amount of damage required to make the situation possible again.

    For example, if you are in a confined space and your \spell{shrink} spell ends, you may find yourself in a space too small to contain your normal body.
    You and the containing object would both take the amount of damage required to either kill you or destroy the containing object, whichever amount of damage is smaller.
  ...
  \subsection{Dismissal}\label{Dismissal}
    ...
    Any \magical ability with a duration can normally be dismissed, but \glossterm{mundane} abilities cannot be dismissed.
    You can dismiss an ability as a \glossterm{free action} that requires only mental effort.

\section{Ability Durations}\label{Ability Durations}
  ...
  \subsection{Attuned Abilities}\label{Attuned Abilities}
    ...
    \parhead{Attunement Time Limits} After you spend a week attuned to an effect, that effect ends on you.
  ...
  \subsection{Sustained Abilities}\label{Sustained Abilities}
    ...
    They last as long as you take an action to sustain them each turn.
    ...
    At the start of your turn, the ability is dismissed unless you take the appropriate action to sustain the ability.
    ...
    Some sustained abilities include ``attunable'' in the tag before the action type.
...
% TODO: move this to a better location, figure out edge cases and object destruction.
\section{Simultaneous Duplicate Effects}
  Sometimes, multiple identical effects will happen simultaneously.
  For example, a creature might drink a liquid containing multiple doses of the same type of poison, or a pile of firebombs may explode simultaneously.
  In all such cases, the effects don't simply stack.
  Instead, any relevant attack gains a \plus2 accuracy bonus for each time that the number of sources doubles.
  For example, a pile of eight firebombs that all go off at once would deal the damage of a single firebomb, but with a +6 accuracy bonus to the attack.
