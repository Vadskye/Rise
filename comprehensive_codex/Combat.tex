...
\section{Universal Abilities}\label{Universal Abilities}
  ...
  \subsection{Dual Strikes}\label{Dual Strikes}
    ...
    All magical properties and special material bonuses from both weapons apply to the combined weapon if possible.
    Numeric bonuses like \glossterm{extra damage} stack, even if you have the same magical property on both weapons.

    Some weapon tags have special rules for they work in a dual strike.
    \begin{raggeditemize}
      \weapontag{Ammunition}: No special effect. Only the Ammunition weapon breaks after making the dual strike.
      \weapontag{Bow}: You cannot make dual strikes with Bow weapons.
      \weapontag{Clinch}: If you use two Clinch weapons, the accuracy bonus stacks.
      \weapontag{Compact}: No special effect.
      \weapontag{Heavy}: You take a \minus1 accuracy penalty for each Heavy weapon.
      \weapontag{Impact}: If you use two Impact weapons, you get a glancing blow when you would miss by 8 or less.
      \weapontag{Keen}: If you use two Keen weapons, the accuracy bonus stacks.
      \weapontag{Light}: You gain a \plus1 accuracy bonus for each Light weapon.
      \weapontag{Long}: Only functions if both weapons are Long.
      \weapontag{Maneuverable}: Only functions if both weapons are Maneuverable.
      \weapontag{Mounted}: If you use two Mounted weapons, the accuracy bonus stacks.
      \weapontag{Parrying}: If you use two Parrying weapons, the accuracy bonus stacks.
      \weapontag{Projectile}: You can only make dual strikes with Projectile weapons if both weapons have the Projectile tag. Use the lower \glossterm{range limits} of the two weapons.
      \weapontag{Resonating}: If you use two Resonating weapons, the defense penalty stacks.
      \weapontag{Subdual}: Only functions if both weapons are Subdual.
      \weapontag{Sweeping}: If you use two Sweeping weapons, the number of additional targets stacks.
      \weapontag{Thrown}: Only functions if both weapons are Thrown. Use the lower \glossterm{range limits} of the two weapons.
      \weapontag{Versatile Grip}: No special effect.
    \end{raggeditemize}

    % Assuming 4 Dex:
    % 2x rapier: +3a 2d4 = +4a 2d4, +2a Light, -3a light base
    % 2x Broadsword: -2a 2d6 Sweeping (2) = +2a 2d6, Sweeping (2), -4a base
    % broadsword/smallsword: +1a 1d6+1d4 Sweeping (1) = +3a 1d6+1d4, Sweeping (1), +1a Light, -3a Light base
    % 2x sickle: +1a 2d4 Sweeping (2) = +2a 2d4 Sweeping (2), +2a Light, -3a Light base

    % Exotics:
    % War scythe: +0a 1d10 Sweeping (2)
    % 2x Kukri: +2a 2d4 Sweeping (2) = +4a 2d4 Sweeping (2), +2a Light, -4a Light exotic base
    Normally, you take a \minus5 accuracy penalty when making a \glossterm{dual strike}.
    If you are using at least one \glossterm{exotic weapon}, this accuracy penalty is increased by 1.
    If you are using at least one \weapontag{Light} weapon, this accuracy penalty is reduced by 1.
    If you have at least 4 Dexterity, this accuracy penalty is reduced by 1.

    Putting this all together, here are some examples for a creature with less than 4 Dexterity:
    % Assuming 1-3 Dex
    \begin{raggeditemize}
      \item Broadsword and smallsword: \plus0 accuracy, 1d6\plus1d4 damage, Sweeping (1)
      \item Two kukris: \plus1 accuracy, 2d4 damage, Sweeping (2).
      % = +2a 2d4, +2a light, -4a light base
      \item Two rapiers: \plus0 accuracy, 2d4 damage, Keen+ (\plus4 accuracy with critical hits).
      % = +0a 2d10, -5a base
      \item Two totokias: \minus5 accuracy, 2d10 damage, Impact+ (glance if miss by 8 or less)
      % = +1a 1d8+1d4, +1a light, -4a light base
      \item Throwing axe and handaxe: \minus2 accuracy, 1d8\plus1d4 damage, Thrown (30/60)
      % = +0a 1d8+1d6, +1a light, -4a light base
      \item Warhammer and light hammer: \minus3 accuracy, 1d8\plus1d6 damage, Resonating+ (-4 Armor on hit), Versatile Grip (usually irrelevant)
    \end{raggeditemize}
