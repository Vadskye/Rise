  ...
  \subsection{Sample Animal Hybrids}
    ...
    \begin{raggeditemize}
      ...
      \itemhead{Winged Agility} A hybrid bee has wings that are not strong enough to help it fly.
        ...
        While it is able to use its wings, it gains a \plus3 bonus to the Poise skill and a \plus5 foot bonus to its maximum horizontal jump distance (see \pcref{Jumping}).
    \end{raggeditemize}
...
\section{Automaton}\label{Automaton}
  ...
  However, its artificial body is inhabited by a true soul, making it a \creaturetype{indwelt} (see \pcref{Indwelt}).

  \parhead{Creature Type} Unlike most other playable species, an automaton is a \creatureorigin{artificial} \creaturetype{humanoid} \creaturetype{indwelt} instead of a \creatureorigin{natural} humanoid.
  \parhead{Size} Medium.
  \parhead{Attributes} \plus1 Constitution or Intelligence, \minus1 Dexterity.
  \parhead{Special Abilities}
  \begin{raggeditemize}
    \itemhead{Artificial Life} Automatons are \trait{nonliving}, so they are immune to \atLife effects and \atPoison effects.
    ...
    \itemhead{Mechanical Body} Automatons are \trait{static}, so they do not need to eat, drink, or sleep. 
      They do not regain hit points or remove vital wounds when they rest, and they are unaffected by the Medicine skill.
      Any damage it takes must be physically repaired like a damaged object using a relevant Craft skill.
...
\section{Awakened Animal}
  ...
  Awakened animals are animals that have been granted sentience by the \ritual{awaken} ritual.
  ...
  \subsection{Sample Awakened Animals}
    ...
    \subparhead{Special Abilities}
    \begin{raggeditemize}
      \itemhead{Claws} A cat's paws end in claws, which it can use to attack (see \pcref{Natural Weapons}). A cat's claws have a \plus1 accuracy bonus and deal 1d4 damage.
      \itemhead{Feline Senses} Cats have \sense{low-light vision} and \sense{scent} (see \pcref{Scent}).
      \itemhead{Quadrupedal} A cat is \trait{quadrupedal}, which gives it a \plus10 foot bonus to its \glossterm{speed}.
    \end{raggeditemize}

\section{Catfolk}

  Catfolk are humanoid creatures with cat-like traits.
  Almost all catfolk have fur.
  Their ears are above their heads, like cats, rather than on the sides of their head.
  They tend to be chaotic and fickle in nature.

  \parhead{Size} Medium.
  \parhead{Attributes} \plus1 Dexterity, \minus1 Strength or \minus1 Constitution.
  \parhead{Special Abilities}
  \begin{raggeditemize}
    \itemhead{Adorable} A catfolk with fur gains a \plus2 bonus to the Persuasion skill.
    \itemhead{Alien Appearance} A catfolk without fur gains a \plus2 bonus to the Intimidate skill.
    \itemhead{Claws} A catfolk's hands end in claws, which it can use to attack (see \pcref{Natural Weapons}). A catfolk's claws have a \plus1 accuracy bonus and deal 1d4 damage.
    \itemhead{Feline Agility} Catfolk gain a \plus2 bonus to the Poise and Stealth skills.
    \itemhead{Feline Senses} Catfolk have \sense{low-light vision} and \sense{scent} (see \pcref{Scent}).
  \end{raggeditemize}
  \parhead{Automatic Languages} Common.
...
\section{Changeling}
  ...
  \parhead{Special Abilities}
  \begin{raggeditemize}
    \itemhead{Alter Shape} A changeling can change its body using the \ability{alter shape} ability.
...
\section{Chimera}
  A chimera is a unique creature that combines the characteristics of three different species.
  This fusion results in a creature that possesses the archetypes of all three of its component species.
  Chimeras are magical in nature, but their exact origin is unclear.
  All attempts to intentionally create chimeras have failed.

  All of a chimera's component species must either be \creatureorigin{natural} or have the same non-natural creature origin.
  This means that you can't be an automaton vampire chimera.

  \parhead{Creature Type} A chimera has all creature types of its component species. It is \creatureorigin{natural} unless one of its component species changes that.
  \parhead{Size} Medium.
  \parhead{Attributes} A chimera gains the attribute modifiers of any one of its component species.
  \parhead{Special Abilities}
  \begin{raggeditemize}
    \itemhead{Chimera Archetypes} A chimera chooses three different species that have a dedicated species archetype.
      The chimera treats those three species archetypes as its three archetypes.
      It does not normally gain any class archetypes.
    \itemhead{Species Abilities} A chimera gains some abilities from each of its three component species, as described in \tref{Chimera Species Abilities}.
    It gains all listed benefits and drawbacks of each species, except that any numeric bonuses from these abilities do not stack with each other.
    If multiple species would modify the same body part, the chimera must choose to keep only one of those abilities and ignore the others.

    For example, an oozeborn harpy chimera must choose between the Amorphous Form ability and the Harpy Wings ability, since they both modify the chimera's arms.
    A dragon harpy chimera would always have wings on their back, since since the Draconic Wings ability doesn't remove any other body parts.
    However, they would have to choose between the Draconic Weapons ability and the Harpy Wings ability, since those both modify the chimera's arms.
  \end{raggeditemize}
  \parhead{Automatic Languages} Common.

  \begin{dtable}
    \lcaption{Chimera Species Abilities}\label{Chimera Species Abilities}
    \begin{dtabularx}{\columnwidth}{l >{\lcol}X}
      \tb{Species} & \tb{Abilities} \tableheaderrule
      Automaton    & Artificial Life, Mechanical Body (see \pcref{Automaton}) \\
      Dragon       & Draconic Scales, Draconic Weapons, Draconic Wings, Dragon Type, Limited Equipment (see \pcref{Dragon}) \\
      Dryad        & Tree Bond, Fey Vulnerability, Verdant Flourishing (see \pcref{Dryaidi}) \\
      Harpy        & Harpy Wings, Limited Equipment, Prehensile Talons, Winged Agility (see \pcref{Harpy}) \\
      Incarnation  & Essence Infusion, Essence Vulnerability (see \pcref{Incarnation}) \\
      Naiadi       & Aquatic Essence, Enchanting Appearance, Fey Vulnerability, Water Affinity (see \pcref{Naiadi}) \\
      Oozeborn     & Acidic Body, Amorphous Form (see \pcref{Oozeborn}) \\
      Tiefling     & Demonic Prejudice, Hellfire Tolerance, Infernal Presence (see \pcref{Tiefling}) \\
      Sapling      & Barkskin, Ingrain, Limited Equipment, Made of Wood, Unhurried and Unfaltering (see \pcref{Sapling}) \\
      Troll        & Bite, Fungal Resilience, Fungal Vulnerabilities, Hard to Kill (see \pcref{Troll}) \\
      Vampire      & Climb Speed, Fangs, Undead, Vampire Weaknesses (see \pcref{Vampire}) \\
    \end{dtabularx}
  \end{dtable}
...
\section{Dragon}\label{Dragon}
  ...
  \parhead{Creature Type} Unlike most other playable species, a dragon is a \creaturetype{dragon} instead of a \creaturetype{humanoid}.
  ...
  \parhead{Attributes} \minus1 Strength, \plus1 to any of Constitution, Intelligence, or Willpower.
  \parhead{Special Abilities}
  \begin{raggeditemize}
    ...
    % +2 instead of +1 because this is compensating for both not having body armor and not being able to use a shield
    ...
    \itemhead{Immortal} Dragons are \trait{immortal}.
      They cannot die of old age.
    \itemhead{Limited Equipment} A dragon's claws are not able to effectively wield shields or manufactured weapons, but can still use ordinary objects and implements.
...
\section{Drow}
  ...
  \begin{raggeditemize}
    ...
    \itemhead{Poison Tolerance} Drow are \resistant to poison.
...
\section{Dryaidi}\label{Dryaidi}
  ...
  \begin{raggeditemize}
    ...
    \magicalitemhead{Tree Bond} A dryaidi must be bonded with a specific tree.
      ...
      As long as the bonded tree remains healthy and intact, the dryaidi gains a \plus1 bonus to Mental defense and a \plus1 bonus to its \glossterm{maximum stamina}.
    ...
    \magicalitemhead{Verdant Flourishing} Dryaidi can use the \spell{rapid growth} \glossterm{cantrip} from the \sphere{verdamancy} sphere.
      If they already have access to that sphere, they can sustain that cantrip as a \glossterm{free action} instead of as a minor action.
  \end{raggeditemize}
...
\section{Eladrin}
  ...
  \begin{raggeditemize}
    ...
    \itemhead{Fae Season} Eladrin respond strongly to their emotions, and change their abilities to reflect the season they currently represent.
...
\section{Harpy}\label{Harpy}
  ...
  \begin{raggeditemize}
    ...
    \itemhead{Harpy Wings} Harpies have no arms or hands.
      Instead, they have feathered wings that sprout from their shoulders.
      These wings grant them an average \glossterm{glide speed} (see \pcref{Aerial Movement}).
    \itemhead{Limited Equipment} Harpy talons are not able to effectively wield shields or manufactured weapons, but can still use ordinary items and implements.
    Harpies can wear armor, but it is treated as \glossterm{barding} instead of normal armor, reducing its effectiveness (see \pcref{Barding}).
    ...
    % +2 instead of +1 because this is compensating for both not having body armor and not being able to use a shield
    \itemhead{Winged Agility} While a harpy is able to use its wings, it gains a \plus2 bonus to Armor defense, a \plus3 bonus to the Poise skill, and a \plus5 foot bonus to its maximum horizontal jump distance (see \pcref{Jumping}).
  \end{raggeditemize}
  \parhead{Automatic Languages} Common.
...
\section{Incarnation}\label{Incarnation}
  ...
  \parhead{Creature Type} Unlike most other playable species, an incarnation is a \creatureorigin{artificial} \creaturetype{humanoid} \creaturetype{indwelt} instead of a \creatureorigin{natural} humanoid.
  \parhead{Size} Medium.
  ...
  \begin{raggeditemize}
    \magicalitemhead{Essence Infusion} Each incarnation chooses one of the following tags as its essence infusion: \atAcid, \atAir, \atAuditory, \atCold, \atCompulsion, \atEarth, \atEmotion, \atElectricity, \atFire, \atVisual, or \atWater.
      All of the incarnation's \glossterm{strikes} gain that tag, and it is \resistant to attacks with that tag.
    ...
    \itemhead{Poison Resistance} An incarnation is \resistant to \atPoison attacks.
  ...
  Untethered incarnations have no arms or legs, and are not \creaturetype{humanoid}.
  ...
  Most untethered incarnations have some sort of face, as they can speak and see normally, but the appearance of that face can be highly unusual.
  ...
  \begin{raggeditemize}
    ...
    \magicalitemhead{Flight} The incarnation's only movement mode is an average \glossterm{fly speed} with a 5 foot height limit (see \pcref{Aerial Movement}).
      ...
      It is also \trait{floating}, so it does not need to fly every turn to avoid falling.
...
\section{Kit}
  ...
  \parhead{Special Abilities}
  \begin{raggeditemize}
    \itemhead{Foxlike Agility} Kit gain a \plus2 bonus to the Poise and Stealth skills.
...
\section{Naiadi}\label{Naiadi}
  ...
  \parhead{Special Abilities}
  \begin{raggeditemize}
    \itemhead{Aquatic Essence} Naiadi are \resistant to \atFire and \atWater attacks.
      However, they are \vulnerable to \atCold and \atElectricity attacks.
    \itemhead{Create Water} A naiadi can cast the \ritual{create water} cantrip.
...
\sectiongraphic*{Oozeborn}{width=\columnwidth}{classes/oozeborn}\label{Oozeborn}
  ...
  \parhead{Creature Type} Unlike most other playable species, oozeborn are an \creaturetype{ooze} instead of a \creaturetype{humanoid}. They are \trait{bloodless}, but unlike most oozes, they are not \trait{simple-minded} or \trait{soulless}.
  \parhead{Size} Medium.
  \parhead{Attributes} \minus1 Intelligence, \plus1 Constitution.
  \parhead{Special Abilities}
  \begin{raggeditemize}
    \itemhead{Acidic Body} Oozeborn are \resistant to \atAcid and \atPoison attacks. However, they are \vulnerable to \atEarth attacks.
...
\section{Sapling}\label{Sapling}
  ...
  \parhead{Special Abilities}
  \begin{raggeditemize}
    \itemhead{Barkskin} A sapling gains a \plus1 bonus to Armor defense.
    ...
    \itemhead{Unhurried and Unfaltering} Saplings have a \minus10 penalty to their \glossterm{speed}.
      However, a sapling's \glossterm{speed} cannot be more than 10 feet slower than its \glossterm{base speed}, even while \slowed or under similar effects, except with the \ability{ingrain} ability.
...
\sectiongraphic*{Tiefling}{width=\columnwidth}{optional rules/tiefling}\label{Tiefling}
  ...
  \begin{raggeditemize}
    ...
    \itemhead{Hellfire Tolerance} Tieflings are \resistant to \atFire attacks.
  ...
  \subsection{Tiefling Archetype}
    ...
    \begin{magicalactiveability}{Abyssal Hop}{Standard action}
        ...
        \rankline
        \rank{2} The damage increases to \damagerankone.
        \rank{3} The damage increases to \damageranktwo.
        \rank{4} The damage increases to \damagerankthree.
    ...
    \magicalcf{Tif}[3]{Abysswalker} Once per turn, you can teleport horizontally instead of moving using your \glossterm{walk speed}.
    ...
    \magicalcf{Tif}[4]{Infernal Ancestry+} The benefits of your \textit{infernal ancestry} ability improve.
      \begin{raggeditemize}
        ...
        \item Hellfire Conduit: If your \ability{abyssal hop} ability also hits a target's Fortitude defense, you can \glossterm{repeat} the attack against that target at the start of your next turn.
        \item Tempting Allure: You can charm creatures.
          \begin{magicalactiveability}{Charming Temptation}{Standard action}
            ...
            You take a \minus10 penalty to \glossterm{accuracy} with this attack against creatures that have made an attack or been attacked since the start of your last turn.
    ...
    In addition, you can teleport in this way any number of times per turn.
    ...
    \magicalcf{Tif}[7]{Infernal Ancestry++} The benefits of your \textit{infernal ancestry} ability reach their peak.
      ...
      %         \item A \plus2 bonus to your \glossterm{maximum stamina}, which makes it easier for you to use powerful abilities that drain your stamina (see \pcref{Stamina}).
      ...
      %     \begin{raggeditemize}
      %         \item \subparhead{Strength} Athletics.
...
\section{Troll}\label{Troll}
  ...
  \begin{raggeditemize}
    ...
    \itemhead{Fungal Resilience} Trolls are \resistant to \atPoison attacks.
    ...
    \itemhead{Subspecies} Every troll has a particular subspecies with specific effects, listed below.
      ...
      They gain a \plus3 bonus to the Athletics and Endurance skills, and they can hold their breath for ten times the normal limit (see \pcref{Common Endurance Tasks}).
...
\section{Vampire}\label{Vampire}
  ...
  \parhead{Creature Type} Unlike most other playable species, a vampire is an \creatureorigin{undead} \creaturetype{humanoid} instead of a \creatureorigin{natural} humanoid.
  \parhead{Size} Medium.
  \parhead{Attributes} \plus1 Strength and Dexterity, \minus1 Constitution.
  \parhead{Special Abilities}
  \begin{raggeditemize}
    \itemhead{Climb Speed} Vampires have a slow \glossterm{climb speed}.
    ...
    \itemhead{Unnatural Life} Unlike most undead creatures, vampires are \trait{blooded}, \trait{dynamic}, and can be affected by the Medicine skill. 
