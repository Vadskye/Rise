...
\section{Skill Overview}
  ...
  \subsection{Class Skills}\label{Class Skills}
    ...
    \begin{dtable!*}
      ...
      \begin{dtabularx}{\textwidth}{l *{11}{>{\ccol}X} >{\ccol}p{4em}}
        \tb{Skill}        & \tb{Bbn} & \tb{Clr} & \tb{Drd} & \tb{Ftr} & \tb{Mnk} & \tb{Pal} & \tb{Rgr} & \tb{Rog} & \tb{Sor} & \tb{Vot} & \tb{Wiz} & \tb{Key Ability} \tableheaderrule
        Athletics         & C        & \tdash   & C        & C        & C        & \tdash   & C        & C        & \tdash   & \tdash   & \tdash   & Str          \\
        Might             & C        & \tdash   & \tdash   & C        & C        & C        & C        & \tdash   & \tdash   & \tdash   & \tdash   & Str          \\
        Flexibility       & C        & \tdash   & \tdash   & C        & C        & \tdash   & \tdash   & C        & \tdash   & \tdash   & \tdash   & Dex          \\
        Poise             & C        & \tdash   & C        & C        & C        & C        & C        & C        & \tdash   & C        & \tdash   & Dex          \\
        ...
        Analysis          & \tdash   & C        & C        & \tdash   & C        & C        & C        & C        & C        & C        & C        & Int          \\
  ...
  \subsection{Tasks}\label{Tasks}
    Each skill contains a short description of how the skill is usually used.
  ...
  \subsection{Improvising}
    ...
    Skills are primarily intended to be useful out of combat, not to replace the power of maneuvers and spells.

\newpage
\skill{Athletics}{Str}
  The Athletics skill represents your ability to move with power in challenging circumstances.
  This includes climbing, jumping, swimming, and other forms of athletic full-body movements. 

  The distance you can jump changes depending on whether you are trained with the Athletics skill.
  For details, \pcref{Jumping}.

  \subsection{Common Athletics Tasks}

    \parhead{Athletic Movement} While you are climbing, swimming, or otherwise moving athletically, you must make an Athletics check whenever you move.
    This generally requires two \glossterm{free hands}, or one free hand if you take a \minus5 penalty to the check.
    The \glossterm{difficulty value} is based on the environment you are moving in (see \pcref{Athletics Difficulty Values}).
    Success means you can move at one quarter speed, so moving 5 feet costs 20 feet of your \glossterm{available movement}.
    Critical success means you can move at half speed instead.
    If you have a relevant speed, such as a \glossterm{climb speed}, you use that speed instead of these multipliers.
    \parhead{Creature Climb}\nonsectionlabel{Creature Climb} As a standard action, you can make an Athletics vs. Reflex attack against a creature you touch.
    ...
    You must repeat this attack at the end of each of your subsequent turns to stay climbing on the creature.
    For each consecutive turn that you climb on a non-ally in this way, you take a cumulative \minus2 penalty to this attack.
    \parhead{Grab Surface} You can make an Athletics check as part of movement to grab a surface that you are passing by.
    You automatically attempt this as a \glossterm{reactive check} if you would fall while being \glossterm{pushed}.
    ...
    \parhead{Maintain Hold} Whenever you take damage while climbing on a surface, suddenly acquire significantly more weight (such as by catching a falling character), or otherwise are significantly distracted, you must make an Athletics check based on the surface.
    Failure means you fall off of the surface, and are \prone when you land.
    \parhead{Wall Jump} You can make a \glossterm{difficulty value} 10 Athletics check as part of movement to jump off of a wall you are adjacent to.
    ...
    \parhead{Wallrun} You can make an Athletics check as part of your movement while you are touching a solid vertical surface.
    The \glossterm{difficulty value} is 10 higher than normal to climb for the surface, but this does not require \glossterm{free hands}.
    Success means you can move using your \glossterm{walk speed} along the wall this turn.
    You move at half speed while going up.
    Failure means you fall.
    For every turn in which you use this ability on the same wall without reaching a stable stopping point, the DV increases by 10.

  \subsection{Common Athletics Modifiers}
    Slippery and mobile surfaces make it more difficult to climb.
    ...
    Creatures that are native to water, such as fish and monsters with a swim speed but no walk speed, gain a \plus10 bonus to Athletics checks made to swim.

    \subsection{Athletics Difficulty Values}
      \begin{columntable}
        \begin{dtabularx}{\columnwidth}{l X X}
          \multicolumn{3}{l}{Climbing} \\
          ...
          \multicolumn{3}{l}{Swimming} \\
          5       & Calm water                                   & Pond or lake with little wind, wide and slow river \\
          10      & Rough water                                  & Lake on a windy day, ocean on a mild day, fast-moving but steady river \\
          10      & Viscous liquid                               & Muddy swamp \\
          15      & Stormy water                                 & Lake during a severe storm, ocean during a mild storm, river rapids \\
          20      & Extremely stormy water                       & Ocean during a severe storm \\          
        \end{dtabularx}
      \end{columntable}

\newpage
\skill{Analysis}{Int}
  The Analysis skill represents your ability to analyze information in front of you.
  This includes sifting through information to identify relevant details, making logical deductions based on evidence, and identifying suspicious flaws.
  ...
  \subsection{Common Analysis Tasks}
    \parhead{Identify Disguise} If you succeed at an opposed Analysis vs. Disguise check, you can determine whether a creature or object is disguised.

    \parhead{Identify Forgery} If you succeed at an opposed Analysis vs. Craft check, you can determine whether an object is a forgery.

    \parhead{Identify Relevant Details} You can make an Analysis check to understand what aspects of your environment are important and why.
    This can be used to scan a room, or to sift through information like a long book.
    If important details are concealed, this may require a successful Awareness check to notice them.

    Correctly identifying relevant details generally takes ten times longer than it would take to skim the relevant information.
    That means it takes about a minute to scan a typically sized room, but hours or days to sift through books.
    You can automatically attempt to identify relevant details as part of seeing information for the first time, but you take a \minus10 penalty to this check.

    \parhead{Reach Conclusion} You can combine information that you know to reach a specific conclusion.
    This may require other checks, such as Knowledge or Analysis checks, to ensure that you have enough information to work with.
    ...
    In general, sifting through a mixture of helpful and misleading evidence increases the difficulty of the Analysis check and the time required to complete it.
    
    \parhead{Understand Personal Effect} When an ability affects you, you can make an Analysis check to understand its effects.
    You can make another check when the ability ends if you didn't understand it initially.
    The \glossterm{difficulty value} of this check is 5 \add twice the \glossterm{rank} of the ability.
    If the ability is \atSubtle, the difficulty value increases by 10.
\newpage
\skill{Awareness}{Per}
  ...
  \subsection{Common Awareness Tasks}
    ...
    \parhead{Notice Ability Usage} If you succeed at an Awareness check, you can notice a creature using an ability.
    Unless otherwise noted in an ability's description, any active ability has some combination of visual and auditory effects that require a \glossterm{difficulty value} 0 Awareness check to notice.
    If you beat the DV by 10, you can also determine which creature used the ability.
    If the ability requires \glossterm{verbal components} or \glossterm{somatic components}, you only have to beat the DV by 5 to identify the ability's source.
    ...
    \parhead{Notice Personal Effect} Whenever an ability is used on you, you can make an \glossterm{difficulty value} 5 Awareness check to notice it.
    This doesn't tell you what the ability does, which requires an Analysis check (see \pcref{Analysis}).
    You can make this check even if the ability has no effect on you, such as if it fails to beat your defenses.
    If the ability is \atSubtle, the difficulty value increases to 15 \add twice the ability's \glossterm{rank}.
    
    This check is only necessary for non-damaging abilities.
    If you take damage from an ability, you automatically notice it.
...
\skill{Craft}{Int}
  ...
  \subsection{Crafting Items}\label{Crafting Items}
    ...
    The \glossterm{difficulty value} to craft an item is normally equal to 5 \add three times its rank.
    You can't use the Craft skill to craft permanent magic items unless you have a special ability granting you that ability.

    Creating items always requires an appropriate crafting kit or workstation.
    Crafts with alchemy and metal are more difficult to work with, and their workstations are larger.
    For details, see \pcref{Tools, Goods, and Mounts}.

    \subsubsection{Crafting Time}
      The time required to craft an item depends on its complexity.
      Generally, it takes more time and effort to craft things that are large or require precision.
      Larger objects are intrinsically more complex to construct, and the GM should use their discretion to decide which complexity is most appropriate for a given task.
      Extraordinarily large or complex objects, such as an entire castle, may require creating any number of smaller component pieces with their own complexity.

      The base time required is given below:
      \begin{raggeditemize}
        \itemhead{Basic} 10 minutes. Basic items include most poisons, potions, and alchemical items.
        \itemhead{Standard} 8 hours. Standard items include mundane tools, non-exotic weapons, and common apparel.
        \itemhead{Advanced} 40 hours, or one working week. Advanced items include exotic weapons, leather armor, and magical tools.
        \itemhead{Masterwork} 160 hours, or one working month. Masterwork items include metal armor, siege engines, and architecture.
      \end{raggeditemize}

      When you make a Craft check to create an item, you make progress on the item based on how long you spend crafting.
      For every 10 points by which you beat the Craft check, you accomplish twice as much work in the same amount of time.
...
\skill{Creature Handling}{Per}
  ...
  \subsection{Common Creature Handling Tasks}
    ...
    \begin{sustainability}{Command}{Standard action}
      ...
      \hit The target is unable to take any \glossterm{standard actions}.
  ...
  \subsection{Teaching Tricks}
    ...
    \parhead{Track (DV 10)} The creature tracks the scent presented to it. (This requires the creature to have the \sense{scent} ability)
  ...
  \subsection{Training Non-Domesticated Creatures}
    ...
    While acting at the behest of another creature using Creature Handling, elite monsters can only take one standard action or one elite action during of their turns, not both.
...
\skill{Deception}{Per}
  ...
  \subsection{Common Deception Tasks}
    ...
    This generally makes the distracted creature \briefly \partiallyunaware of you, which can allow you to hide or backstab them.
...
\skill{Disguise}{Int}
  ...
  \subsection{Common Disguise Tasks}
    ...
    You can make an opposed Disguise vs. Analysis check using a disguise kit to change a creature's appearance.
    ...
    This functions like the \textit{change appearance} task, except that the result of your Disguise check can't exceed the result of an Analysis check you or someone helping you made to observe the creature you are trying to emulate.
...
\skill{Endurance}{Con}
  ...
  \subsection{Common Endurance Tasks}\label{Common Endurance Tasks}
    ...
    While holding your breath, you must make an Endurance check at the end of every 5 turns that you spend without taking any actions, or at the end of any turn in which you take an action.
    ...
    Essentially, you can fight while holding your breath for a number of turns equal to your Endurance modifier with no risk of failure.
    ...
    If you fail the check, you cannot continue the activity until you rest.
    If the activity took less than an hour in total, you can restart the activity after a \glossterm{short rest}.
    Otherwise, you can only restart the activity after a \glossterm{long rest}.

    For example, a human can normally run for ten minutes without rest.
    If they pass a DV 10 Endurance \glossterm{extended check}, they could sprint for an additional five minutes.
    After that time, they would need to make a DV 15 Endurance \glossterm{extended check} for the next five minutes, then DV 20, and so on.
    ...
    Failure means you reduce your \glossterm{stamina} by three.
    You must make another \glossterm{extended check} every 8 hours as long as you are still beyond your normal sleep limits.

  \subsection{Maintain Exertion Durations}
    This is a reference for how long a typical creature can maintain a particular form of exertion without needing to make an Endurance check.

    \begin{raggeditemize}
      \item Running: Ten minutes.
      \item Martial combat: Ten minutes.
      \item Spellcasting: Ten minutes.
      \item Ritual participation: Eight hours.
      \item Walking: Eight hours.
    \end{raggeditemize}

    % This is thematically appropriate, but it breaks the rule about not introducing ``shadow'' resources
    % \subsection{Ignore Stamina Penalty}\label{Ignore Stamina Penalty}
    %     You can temporarily ignore your stamina penalty to try to accomplish your objectives.
    %     Ignoring your stamina penalty requires a \glossterm{minor action}.
    %     The \glossterm{difficulty value} starts at 10.
    %     If you succeed, you reduce your \glossterm{stamina penalty} by 1 until the end of the turn.
    %     For every 10 points by which you succeed, you reduce your stamina penalty by an additional 1.
...
\skill{Intimidate}{Varies}
  ...
  However, depending on how you are trying to intimidate creatures, you can add any attribute to your Intimidate check.
  For example, if you intimidate a creature by smashing a table and threatening to smash its head in, you could add your Strength to the Intimidate check.
  On the other hand, if you intimidate a creature by staring it down in a cold fury, you could add your Willpower to the Intimidate check.
...
\skill{Knowledge}{Int}
  ...
  For example, Knowledge (local) refers to the continent or large region that the characters could reasonably be familiar with.
  Sufficiently distant areas in the campaign may require separate Knowledge skills, and the GM could name each skill after the campaign regions.
  \begin{raggeditemize}
    \item Arcana (arcane spells, dragons, highly magical creatures, complex magical phenomena)
    ...
    \item Local (myths and legends, laws and customs, history, nobility and royalty, religious practices, humanoids)
    \item Nature (nature spells, animals, fae, plants, terrain and climate, planets of the Material Plane)
    \item Souls (divine and pact spells, soulforged, undead, deities, Spiritual Planes, the afterlife, resurrection)
  \end{raggeditemize}
...
\newpage
\skill{Might}{Str}
  The Might skill represents your ability to exert your strength in local bursts of energy.

  \subsection{Common Might Tasks}
    \parhead{Heft} You can make a DV 15 Might check to lift an object that exceeds your normal maximum carrying capacity (see \pcref{Weight Limits}).
    While overburdened in this way, you are \unsteady and must use at least one \glossterm{free hand} to control the object.
    This is an exertion, so you can normally only heft an object for up to five minutes between short rests.
    \parhead{Sunder} You can make a Might check to break an object with raw strength.
    The \glossterm{difficulty value} is equal to the object's \glossterm{hardness}, \plus5 for each \glossterm{weight category} above Tiny (see \pcref{Breaking Objects}).
    Success means the object is \glossterm{broken}.
    Critical success means the object is \glossterm{destroyed}.
    % untrained human can throw between 12 and 30 feet
    \parhead{Throw} You can make a DV 5 Might check as a \glossterm{standard action} to throw a creature or object that is not already designed to be thrown.
    Success means that you \glossterm{fling} it to a location within 10 feet plus a number of feet equal to twice your check result.
    If an object is particularly easy to throw, such as a small dense sphere, you can throw it up to ten times that distance.
    Failure means you drop whatever you were trying to throw in your space.

    Normally, a thrown object must be below the maximum \glossterm{weight category} that you can lift (see \pcref{Weight Categories}).
    You can take a \minus10 penalty to the Might check to throw an object whose weight category matches your maximum weight category.
    You can't throw an unwilling creature unless you are already grappling them (see \pcref{Grapple}).
...
\skill{Perform}{Varies}
  ...
  \begin{raggeditemize}
    ...
    \item Object manipulation (card tricks, juggling, puppetry)
    \item Oratory (storytelling, spoken word poetry, debate)
    \item Percussion instruments (bells, drums, piano)
  ...
  However, depending on how you are trying to perform, you can add any attribute to your Perform check.
  For example, if you perform by juggling or dancing, you would add your Dexterity to the Perform check.
  On the other hand, if you perform by telling a tale to capture the hearts of your audience, you would add your Perception to the Perform check.
...
\newpage
\skill{Poise}{Dex}
  The Poise skill represents your ability to maintain stability and control over your movements in difficult circumstances.
  This includes wet or unsteady terrain, riding a mount, or precisely positioning your body to avoid triggering traps.
  ...
  \subsection{Common Poise Tasks}
    \parhead{Creature Balance}\nonsectionlabel{Creature Balance} As part of movement, you can make a Poise vs. Reflex attack against a creature you touch.
    ...
    You must repeat this attack at the end of each of your subsequent turns to stay balancing on the creature.
    For each consecutive turn that you balance on a non-ally in this way, you take a cumulative \minus2 penalty to this attack.

    \parhead{Fall Steady} When you take \glossterm{falling damage}, you must make a Poise \glossterm{reactive check}.
    The \glossterm{difficulty value} of the check is equal to 5 \add 1 per 5 feet of the fall.
    Failure means that you fall \prone.

    \parhead{Guide Mount} When riding on a creature, you can make a Poise check to direct your mount's movement.
    While travelling, this check is only necessary when giving the mount directions.
    In battle, this check must be repeated at the start of your turn.
    ...
    \parhead{Maintain Balance} If you take damage while on an unsteady surface, you must make a Poise \glossterm{reactive check} based on the surface.
    Failure means that you fall \prone.
    You only need to make this check once between your turns, even if you take damage from more than one source.

    \parhead{Maintain Ride} Whenever you take damage or your mount makes a sudden motion, you must make a DV 5 Poise check to continue riding the creature.
    Sudden motions include jumping, attacking, and moving at more than half speed.
    Failure means you fall off of your mount.

    \parhead{Rapid Stand}\nonsectionlabel{Rapid Stand} You make a \glossterm{difficulty value} 15 Poise check as a \glossterm{move action} to stand up from a prone position.
    Success means that you stand up so quickly that you can immediately take another move action.
    Failure simply means that you stand up.

    \parhead{Take Cover} You can make a DV 15 Poise check as a \glossterm{move action} to drop low and take \glossterm{cover} behind your mount.
    This requires the use of a \glossterm{free hand}.
    Failure means you can't get low enough and gain no benefit from the action.

    \parhead{Walk While Balancing} When you move using your \glossterm{walk speed} on an unsteady surface, you must make a Poise check based on the surface.
  ...
  \subsection{Common Poise Modifiers}
    The base \glossterm{difficulty value} to balance on normal terrain is 0.
    Four common circumstances make balancing checks more difficult: slippery, mobile, narrow, and uneven surfaces.
    ...
    Some specific terrain examples are listed below.
    ...
    \begin{raggeditemize}
      \item If a creature is not trained as a mount, the DV to ride it increases by 5.
      \item If it lacks a saddle and other riding gear, the DV to ride it increases by 5.
      \item If it takes a standard action other than movement, such as attacking, the DV to ride it that turn increases by 5.
      \item If it uses a \glossterm{movement mode} other than a walk speed, the DV to ride it that turn increases by 10.
    \end{raggeditemize}
...
\skill{Sleight of Hand}{Dex}
  ...
  \subsection{Common Sleight of Hand Tasks}
    ...
    The action must be at least two size categories smaller than you are.
    This generally makes it impossible to conceal attacks or spellcasting, but you can pick pockets or apply poison to food.
...
\skill{Social Insight}{Per}
  ...
  \subsection{Common Social Insight Tasks}
    ...
    Noticing a \abilitytag{Compulsion} effect is \glossterm{difficulty value} 5, and noticing an \abilitytag{Emotion} or \abilitytag{Subtle} effect is difficulty value 20.
...
\skill{Stealth}{Dex}
  ...
  \subsection{Common Stealth Tasks}
    ...
    \parhead{Hide} As part of movement, you can make an opposed Stealth vs. Awareness check to make creatures that are aware of you lose track of your position.
    To succeed with this check, you must move in a way that makes the observer fully lose sight of you for at least ten feet of your movement.
  ...
  \subsection{Common Stealth Modifiers}\label{Common Stealth Modifiers}
    ...
    This includes creatures who can ignore concealment with abilities like \sense{blindsight}.

    You take a \minus10 penalty to Stealth checks against creatures who can know your location with a special ability like \sense{blindsense}.
    ...
    If you move, you take a penalty to your Stealth check to conceal that movement.
    This is a \minus5 penalty if you move no more than half your speed.
    If you use the \textit{sprint} ability or move farther than half your speed, this penalty increases to \minus10.
...
\skill{Survival}{Per}
  ...
  \subsection{Common Survival Tasks}
    ...
    \parhead{Follow Tracks} You can make a Survival check to follow tracks at up to half your normal \glossterm{speed}.
  ...
  \subsection{Tracking}\label{Tracking}
    ...
    \begin{columntable}
      ...
      \begin{dtabularx}{\columnwidth}{l >{\lcol}X l}
        ...
        Scent            & Tracking using the \sense{scent} ability instead of vision                                                                                                           & 5 \\
      \end{dtabularx}
    \end{columntable}
