...
Each character is proficient with different weapon groups. These indicate the weapons that you can use effectively. You gain a \plus4 bonus to attack when using a weapon you are proficient with. You can wield weapons you are not proficient with, but you cannot use them to defend yourself, which can cause you to be \defenseless.
...
You take a \minus4 penalty to attack with medium and large ranged weapons against creatures adjacent to you. In addition, you are usually \defenseless while using ranged weapons.
...
A creature with multiple natural weapons of the same type can designate one as a primary attack and one as a secondary attack, allowing it to fight with both at once (see \pcref{Dual Wielding}). You are only considered to have one unarmed strike, so you cannot dual wield with only your unarmed strike (but see the unarmed warrior monk ability, \pref{Mnk:Unarmed Warrior}).
...
\lcaption{Weapons}
    ...
    \tb{Weapons} & \tb{Encumbrance} & \tb{Dmg} & \tb{Damage Type\fn{1}} & \tb{Cost} & \tb{Weight\fn{2}} & \tb{Special} \\
\hline
...
\begin{dtable!*}
    \begin{dtabularx}{\textwidth}{p{12em} c c >{\ccol}p{10em} c c >{\ccol}X}
        \tb{Exotic Weapons} & \tb{Encumbrance} & \tb{Dmg} & \tb{Damage Type\fn{1}} & \tb{Cost} & \tb{Weight\fn{2}} & \tb{Special} \\
\hline
        Armor &&&&&& \\
        Axes &&&&&& \\
...
\begin{dtable!*}
    ...
    \begin{dtabularx}{\textwidth}{p{12em} c c >{\ccol}p{15em} >{\ccol}X}
        \tb{Natural Weapons} & \tb{Encumbrance} & \tb{Dmg} & \tb{Damage Type\fn{2}} & \tb{Special} \\
\hline
...
\parhead{Disarming} You gain a \plus2 bonus to disarm attacks using this weapon.
\parhead{Double} This weapon has more than one striking surface. You can fight with both ends simultaneously, just like dual wielding (see \pcref{Dual Wielding}). Alternately, you can attack with one end at a time. If you have the ability to use a double weapon in one hand, you can only fight with one end at a time, not both.
...
\parhead{Forceful} You can use this weapon to make shove attacks to push people away from you. This allows you to add the weapon's proficiency bonus to the shove attack.
...
\parhead{Keen} You gain a \plus4 bonus to confirm critical threats with this weapon.
...
\par A net must be folded to be thrown effectively. The first time you throw your net in a fight, you make a normal ranged touch attack roll. After the net is unfolded, you take a \minus4 penalty on attack rolls with it. It takes 2 full-round actions for a proficient user to fold a net and twice that long for a nonproficient one to do so.
...
\parhead{Dexterity} Heavy body armor limits mobility and agility, halving the character's Dexterity. This halving is applied in addition to the armor's armor check penalty (if any). A Dexterity penalty is not halved.
...
\hline
\tb{Armor} & \tb{Armor/Shield Bonus} & \tb{Dex Modifier} & \tb{Armor Check Penalty} & \tb{Arcane Spell Failure Chance} & \tb{(30 ft.)} & \tb{(20 ft.)} & \tb{Cost} & \tb{Weight\fn{1}} \\
Light armor &  &  &  &  &  &  &  &  \\
\tind Leather 				& \plus1 		& 1\mult 	& \minus1 & 10\% & 30 ft. & 20 ft. & 10 gp        & 15 lb.\\
\tind Studded leather 		& \plus2 		& 1\mult 	& \minus2 & 15\% & 30 ft. & 20 ft. & 25 gp & 20 lb.\\
\tind Chain shirt 			& \plus2 		& 1\mult 	& \minus2 & 20\% & 30 ft. & 20 ft. & 40 gp   & 25 lb.\\
Medium armor 				&  &  &  &  &  &  &  & \\
\tind Hide 					& \plus3 		& 1\mult 	& \minus4 & 20\% & 20 ft. & 15 ft. & 15 gp           & 25 lb.\\
\tind Scale mail 			& \plus3 		& 1\mult 	& \minus4 & 25\% & 20 ft. & 15 ft. & 50 gp     & 30 lb.\\
\tind Chainmail 			& \plus4 		& 1\mult 	& \minus4 & 30\% & 20 ft. & 15 ft. & 100 gp     & 40 lb.\\
\tind Breastplate 			& \plus4 		& 1\mult 	& \minus3 & 25\% & 20 ft. & 15 ft. & 150 gp    & 30 lb.\\
Heavy armor 				&  &  &  &  &  &  &  & \\
\tind Splint mail 			& \plus4 		& 1/2\mult 	& \minus7 & 40\% & 20 ft. & 15 ft. & 50 gp   & 45 lb.\\
\tind Half-plate 			& \plus5 		& 1/2\mult 	& \minus6 & 40\% & 20 ft. & 15 ft. & 250 gp  & 50 lb.\\
\tind Full plate 			& \plus6 		& 1/2\mult 	& \minus6 & 35\% & 20 ft. & 15 ft. & 500 gp  & 50 lb.\\
Shields 					&  &  &  &  &  &  &  & \\
\tind Buckler 				& \plus1 		& \x 		& \minus1 & 5\% & \x & \x & 15 gp             & 5 lb.\\
\tind Shield, light wooden 	& \plus1 		& \x 		& \minus2 & 5\%\fn{2} & \x & \x & 3 gp & 5 lb.\\
\tind Shield, light steel 	& \plus1 		& \x 		& \minus2 & 5\%\fn{2} & \x & \x & 9 gp  & 6 lb.\\
\tind Shield, heavy wooden 	& \plus2 		& \x 		& \minus3 & 15\%\fn{2} & \x & \x & 7 gp & 10 lb.\\
\tind Shield, heavy steel 	& \plus2 		& \x 		& \minus3 & 15\%\fn{2} & \x & \x & 20 gp & 15 lb.\\
\tind Shield, tower 		& \plus3\fn{3} 	& 1/2\mult 	& \minus10 & 50\%\fn{2} & \x & \x & 30 gp       & 45 lb.\\
Extras 						&  &  &  &  &  &  &  & \\
...
When employing a tower shield in combat, you take a \minus2 penalty on attack rolls because of the shield's encumbrance.
...
\begin{itemize}
    \item \itemhead{Plant} Plant-based poisons can typically be harvested by making a Survival check to search in appropriate terrain.
        The DC of this check is usually equal to 10 \add the potency of the poison.
    \item \itemhead{Venom} Venom requires an appropriate body part from a creature -- often, poison it naturally produces.
    \item \itemhead{Alchemical} Alchemical poisons require alchemical materials.
