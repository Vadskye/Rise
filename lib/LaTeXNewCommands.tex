...
\newcommand{\blinded}{\conditionlink{blinded} \reminder{unable to see, defenseless}\xspace}
\newcommand{\bloodied}{\conditionlink{bloodied} \reminder{half hit points}\xspace}
\newcommand{\charmed}{\conditionlink{charmed} \reminder{deluded into liking its charmer}\xspace}
...
\newcommand{\dominated}{\conditionlink{dominated} \reminder{compelled to obey its dominator}\xspace}
%\newcommand{\dominated}{\conditionlink{dominated} \reminder{under total mental control}\xspace}
\newcommand{\exhausted}{\conditionlink{exhausted} \reminder{move at half speed, unable to sprint or charge, impaired}}
\newcommand{\entangled}{\conditionlink{entangled} \reminder{move at half speed, unable to sprint or charge, impaired}\xspace}
...
\newcommand{\grappled}{\conditionlink{grappled} \reminder{physically wrestling with a foe}\xspace}
\newcommand{\helpless}{\conditionlink{helpless} \reminder{physical defenses are 10, vulnerable to coup de grace}\xspace}
\newcommand{\ignited}{\conditionlink{ignited} \reminder{1d6 damage/round, 20\% failure, Dex DC 15 to extinguish}\xspace}
...
\newcommand{\negativelevel}{\conditionlink{negative level} \reminder{\minus1 to accuracy, defenses, and checks, \minus5 hit points}}
\newcommand{\negativelevels}{\conditionlink{negative levels} \reminder{\minus1 to accuracy, defenses, and checks, \minus5 hit points}}
...
\newcommand{\partiallyblinded}{\conditionlink{partially blinded} \reminder{impaired, lose special vision abilities}\xspace}
...
\newcommand{\prone}{\conditionlink{prone} \reminder{\minus4 to melee accuracy and defense, \plus4 defense vs ranged attacks}}
...
\newcommand{\subtlespellnotes}{Subtle spells have no obvious effects. A creature affected by a subtle spell is usually unaware that it is under magical influence. The DC to identify a subtle spell with the Spellcraft skill is 10 higher than normal (see \pcref{Spellcraft}).}
...
\DeclareDocumentCommand{\spelldamage}{m o}
{
    ...
    {
        \IfValueTF{#2}
        {1#2~damage~per~two~spellpower}
        {1d6~damage~per~spellpower}
    }
    {
        \IfValueTF{#2}
        {1#2~#1~damage~per~two~spellpower}
        {1d6~#1~damage~per~spellpower}
    }
}
