...
\newcommand{\confusionexplanation}{A confused creature cannot take actions normally. When it becomes confused, and at the beginning of each round, it randomly decides to have one of four behaviors that round: attack the nearest creature as if it was an \glossterm{enemy}, freeze in place and use the \ability{total defense} ability, flee from its enemies, or act normally. A confused character who can't carry out the indicated action does nothing but babble incoherently.\xspace}
...
\newcommand{\climbing}{\debuff{climbing} \reminder{\minus4 accuracy, defenses}\xspace}
\newcommand{\confused}{\debuff{confused} \reminder{\minus4 defenses, acts randomly}\xspace}
\newcommand{\dazed}{\debuff{dazed} \reminder{\minus2 defenses}\xspace}
\newcommand{\dazzled}{\debuff{dazzled} \reminder{25\% miss chance, no special vision}\xspace}
\newcommand{\deafened}{\debuff{deafened} \reminder{25\% verbal spell failure}\xspace}
\newcommand{\dominated}{\debuff{dominated} \reminder{must obey commands}\xspace}
\newcommand{\frightened}{\debuff{frightened} \reminder{\minus4 Mental, -4 accuracy vs. source}\xspace}
...
\newcommand{\helpless}{\debuff{helpless} \reminder{\minus10 Armor and Ref}\xspace}
...
\newcommand{\prone}{\debuff{prone} \reminder{half speed, \minus2 accuracy, Armor, and Ref}\xspace}
\newcommand{\shaken}{\debuff{shaken} \reminder{\minus2 Mental, -2 accuracy vs. source}\xspace}
\newcommand{\slowed}{\debuff{slowed} \reminder{half speed, \minus2 Ref}\xspace}
\newcommand{\squeezing}{\debuff{squeezing} \reminder{\minus2 Armor and Ref}\xspace}
\newcommand{\stunned}{\debuff{stunned} \reminder{\minus4 defenses}\xspace}
\newcommand{\swimming}{\debuff{swimming} \reminder{\minus4 accuracy, defenses}\xspace}
...
% #1: if provided, skip the hypertargetraised back reference
% #2: class shorthand name
% #3: archetype name
\DeclareDocumentCommand{\archetyperef}{s m m}
{
    \hyperlink{archetype:#2:#3}{#3}
    \IfBooleanF{#1}
    {
...
% #2: archetype name
\DeclareDocumentCommand{\archetypedef}{m m}
{
    \newpage
    \subsection{
        \protect
        \hypertargetraised{archetype:#1:#2}{
            \protect
            \hyperlink{archetypetable:#1:#2}{#2}
        }
    }
    \label{#2}
}
...
\DeclareDocumentCommand{\itemsection}{m o}
{
    \subsubsection[#1]{
        \protect
        \hypertargetraised{item:#1}{
            \protect
            \hyperlink{itemtable:#1}{
                \IfValueTF{#2}{
                    % Compensate for dumb extra paragraph created by hspace
                    \vspace{-1em}
                    \spelltwocol{\hspace{-0.5em}#1}{\normalsize #2}
                }{
                    #1
                }
            }
