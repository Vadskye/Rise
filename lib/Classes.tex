...
Determining defenses works in the same way as determining combat prowess, except that the level multipliers are different.

\begin{enumerate}
    \item For each type of defense progression the character has, sum the character's level in classes with that defense progression.
    \item Determine the character's fastest type of defense progression. Good is faster than Average, which is faster than Poor.
    \item Add (five-quarters levels in Good progression) + (levels in Average progression) + (three-quarters levels in Poor progression). Each part of this calculation rounded down separately.
    \item If the character's fastest defense progression is Good, add 3 to the result. If it is Average, add 2. If it is Poor, add 1.
    \item The result is the character's base defense bonus.
\end{enumerate}
