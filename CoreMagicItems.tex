...
\parhead{Armor and Shields} Magic armor (including shields) offers improved, magical protection to the wearer. Some of these items confer abilities beyond a benefit to Armor Class.

\parhead{Weapons} Magic weapons are created with a variety of combat powers and almost always improve the attack and damage rolls of the wielder as well.

\parhead{Potions} A potion is an elixir concocted with a spell-like effect that affects only the drinker.

\parhead{Rings} A ring is a circular metal band worn on the finger (no more than two rings per wearer) that has a spell-like power (often a constant effect that affects the wearer).

\parhead{Rods} A rod is a scepter-like item with a special power unlike that of any known spell.

\parhead{Scrolls} A scroll is a spell magically inscribed onto paper or parchment so that it can be used later.

\parhead{Staffs} A staff has a number of different (but often related) spell effects. A newly created staff has 50 charges, and each use of the staff depletes one or more of those charges.

\parhead{Wands} A wand is a short stick imbued with the power to cast a specific spell. A newly created wand has 50 charges, and each use of the wand depletes one of those charges.

\parhead{Wondrous Items} These objects include magic jewelry, tools, books, clothing, and much more.
