...
\begin{itemize*}
  \item Apparel items provide access to their powers while worn. A \mitem{retributive full plate} and a \mitem{ring of protection} are apparel item. Apparel items must be attuned to their wearer before they can be used to their full effect.
  \item Implements provide access to their powers when wielded or used. A \mitem{flaming longsword} and a \mitem{wand of fire} are implements. Implements must be attuned to their wielders before they can be used to their full effect.
  \item Tools, like implements, provide access to their powers when used. Unlike implements, tools need not be attuned to their wielder. A \mitem{scroll of cure light wounds} and a \mitem{bag of holding} are tools.
\end{itemize*}
...
Almost all apparel and implements must be attuned to a character before their powers can be used. To attune an item, the character must wear or wield the item for a full hour. During that time, he can act normally, but he cannot use the item's powers until the attunement process is complete. A character can only undergo this process for one item at a time.

Once the item is attuned to the character, a character can use its powers. The item remains attuned to the character until a different character attunes to the same item. Some magic items and effects make it difficult to break another's attunement to an item, or allow multiple characters to attune to the same item.
