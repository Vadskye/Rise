...
\begin{spelleffect}
  The touched target takes damage. This damage ignores the hardness of objects.
\end{spelleffect}
...
\spellsr{No}
\begin{spelleffect}
  A cloud of golden particles covers everyone and everything in the area, visibly outlining invisible things for the duration of the spell. It likewise negates the effects of blur and displacement, and reveals figments, mirror images, and projected images for what they are. All within the area at the time that the spell is cast are covered by the dust, which cannot be removed and continues to sparkle until it fades.
...
\spellsr{Yes (Will)}
\begin{spelleffect}
  The subjects gain a \plus2 bonus on attack rolls and temporary hit points equal to 10 \add 1 per caster level for 5 rounds. \bonusscalingdescription
\end{spelleffect}
\begin{spellnotes}
...
\begin{spelleffect}
  ...
  \par Its combat maneuver attack to grapple equals your caster level \add your casting attribute, \plus4 for being Large. The hand holds but does not harm creatures it grapples. Its CMD is equal to 10 \add its CMA.
\end{spelleffect}
\begin{spellnotes}
  Directing the spell to a new target is a swift action.
\end{spellnotes}
...
\spelllvl{Clr 6, Evil 6, Vitality 6}
\spellrng{\rngtouch}
\spelltgt{Creature touched}
\spelldur{Instantaneous}
...
\spelldmg{9d6 negative energy damage \add d6 per four caster levels above 18th}
\begin{spelleffect}
...
\end{spellhealthy}
\begin{spellblood}
  As the healthy effect, and the subject is flat-footed and must spend a standard action each round to do nothing but laugh uncontrollably. After each time it laughs, the affected creature can attempt a new saving throw. If it succeeds, it can stop laughing, though it is still bewildered.
\end{spellblood}
\begin{spellnotes}
...
%Note: doesn't follow existing formula. Need to change formula.
...
\begin{spellhealthy}
  The target takes damage as its body is partially teleported away, and it is slowed for 5 rounds.
\end{spellhealthy}
\begin{spellblood}
  ...
  If the creature is not touching the ground, it instead suffers the healthy spell effect.
\end{spellblood}
\begin{spellnotes}
...
\spellsr{Yes (Will)}
\begin{spelleffect}
  The subject gains physical damage reduction 4/magic, allowing the subject to ignore the first 4 physical damage it takes each round, except from magical attacks such as magic weapons. This damage reduction increases by 1 per two caster levels above 4th.
\end{spelleffect}
