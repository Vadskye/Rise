...
\spellsr{Yes (Will)}
\begin{spelleffect}
  The subject gains a \plus2 enhancement bonus to its Stealth checks and physical defenses. \bonusscalingdescription
\end{spelleffect}
\begin{spellnotes}
  A \spell{see invisibility} spell does not counteract the blurring effect, but a \spell{true seeing} spell does.
  \par Opponents that cannot see the subject, or who have extraordinary sight that ignores illusions, ignore the spell's effect (though fighting an unseen opponent carries penalties of its own).
\end{spellnotes}
