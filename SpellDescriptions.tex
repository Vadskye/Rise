...
\spellsr{Yes (Fortitude)}
\begin{spelleffect}
The subject gains a \plus2 bonus to its natural armor modifier. \bonusscalingdescription In addition, the subject gains physical damage reduction 2/adamantine, allowing it to ignore the first 2 damage it would take each round, except damage dealt by adamantine weapons. This damage reduction increases by 1 for every two levels above 4th.
\end{spelleffect}
\begin{spellnotes}
...
\spellsr{Yes (Fortitude)}
\spelldmg{1 Strength damage and d6 negative energy damage \add half casting attribute}
\begin{spelleffect}
...
\par An undead creature you touch takes no damage of either sort, but it is sickened, making it vulnerable for 5 rounds. A successful Fortitude save negates the effect.
\end{spelleffect}
\begin{spellnotes}
A vulnerable creature takes a \minus2 penalty to attack rolls, saving throws, checks, DCs, and AC. Creatures whose Hit Values exceed your caster level are unaffected by \spell{word of chaos}.
\end{spellnotes}
...
\spellsr{Yes (Will)}
\begin{spelleffect}
The subject moves at half speed, and takes a \minus2 penalty to attack rolls and armor class. If it is flying, it falls to the ground.
\end{spelleffect}
\begin{spellnotes}
  If the subject gets farther than ten feet from the ground, the spell's effect is broken. As a result, it cannot affect creatures flying high above the ground.
\end{spellnotes}
...
\spellsr{Yes (Will)}
\begin{spellhealthy}
The target is bewildered, making it vulnerable for 5 minutes.
\end{spellhealthy}
\begin{spellblood}
...
\spellsr{Yes (Fortitude)}
\begin{spelleffect}
A 3 foot long beam of red-hot fire springs forth from your hand. In addition to providing illumination like a torch, you can wield this bladelike beam as if it were a scimitar. Attacks with the flame weapon are touch attacks, and deal 1d8 fire damage \add half your key attribute. Since the blade is immaterial, it is treated as a light weapon, and your Strength does not apply to the damage. A flame weapon can ignite combustible materials such as parchment, straw, dry sticks, and cloth. Alternately, you can hurl flames from the weapon up to \rngmed range as if it were a thrown weapon.
\end{spelleffect}
\begin{spellnotes}
...
\spellsr{Yes (Will)}
\begin{spellhealthy}
If the subject fails its Will save, it is shaken, causing it to be vulnerable for 5 rounds.
\end{spellhealthy}
\begin{spellblood}
...
\begin{spellnotes}
A vulnerable creature takes a \minus2 penalty to attack rolls, saving throws, checks, DCs, and AC. Creatures whose Hit Values exceed your caster level are unaffected by \spell{word of chaos}.
The target must be bloodied when the spell is cast to suffer the bloodied effect. A creature whose HV exceeds your caster level are sickened instead of killed, as the healthy effect.
\end{spellnotes}
...
\spellsave{None/Fortitude negates}
\begin{spelleffect}
This spell functions like \spell{fog cloud}, except that creatures within the cloud are sickened, making them vulnerable. A successful Fortitude save negates the sickening. The condition lasts as long as the creature remains in the cloud and for 5 rounds after it leaves.
\end{spelleffect}
\begin{spellnotes}
A vulnerable creature takes a \minus2 penalty to attack rolls, saving throws, checks, DCs, and AC. Your allies, and creatures unable to see the lights, are unaffected. Any creature that succeeds on its save but remains in the cloud must continue to save each round on your turn. \spellindirect{stinking cloud}{Stinking cloud} can be made permanent with a \spell{permanency} spell. A permanent \spell{stinking cloud} dispersed by wind reforms in 10 minutes.
\end{spellnotes}
...
\begin{spelleffect}
The target takes damage. In addition, if it fails a Fortitude save, for the duration of the spell all damage it takes is doubled.
\end{spelleffect}
...
\spelldur{Concentration}
\begin{spelleffect}
  You gain tremorsense. If you are touching a surface, you can automatically pinpoint the location of anything within the area of the spell that is in contact with the surface, including inanimate objects.
\end{spelleffect}
\begin{spellnotes}
...
\spelldmg{4d6 divine damage \add d6 per four caster levels above 8th}
\begin{spelleffect}
Nonevil creatures in the area take damage and are sickened, making them vulnerable for 5 rounds. A successful Will save reduces the damage by half and negates the sickening.
\end{spelleffect}
\begin{spellnotes}
A vulnerable creature takes a \minus2 penalty to attack rolls, saving throws, checks, DCs, and AC. Creatures whose Hit Values exceed your caster level are unaffected by \spell{word of chaos}.
\end{spellnotes}
