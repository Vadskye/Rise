...
You must make some choice about whom the spell is to affect or where the effect is to originate, depending on the type of spell. The next entry in a spell description defines the spell's target (or targets) or its area, as appropriate.
...
\subparhead{Burst, Emanation, Limit, Spread} Many spells that affect an area function as a burst, an emanation, a limit, a spread, or a wall. In each case, you select the spell's point of origin and measure its effect from that point.

A burst affects whatever it catches in its area, even including creatures that you can't see. A burst's area defines how far from the point of origin the spell's effect extends. The effects of burst spells do not extend around corners.

An emanation functions like a burst, except that the effect continues to radiate from the point of origin for the duration of the spell.

A limit is more like a range than a normal area. A spell can have effects within its limiting area, but it will not affect the entire area at once, and it generally cannot affect things outside its limiting area.

A spread radiates out like a burst but can turn corners. You select the point of origin, and the spell spreads out a given distance in all directions. Figure the area the spell effect fills by taking into account any turns the spell effect takes.
...
A wall-shaped spell extends in a single direction from its point of origin. All walls must be contiguous, so every part of the wall is adjacent to another part of the wall. Unless otherwise specified, walls must be vertical and go straight in a particular direction. Some walls are shapeable, as denoted by \shapeable. You can make corners when constructing the path of a shapeable wall.

\subparhead{Specific Areas} Some spells specify a series of volumes that make up the area of the spell. Most commonly, the volumes are cubes. You may arrange the volumes as you want, with the restriction that each volume in the spell's area must be adjacent to one other volume in the spell's area.

\subparhead{Area Sizes} The area affected by many spells falls into one of four sizes. Each size defines the extent to which the spell extends out from its origin, whether as a radius or as the length of a cone or line. Small spells extend 10 feet out. Medium spells extend 20 feet out. Large spells extend 50 feet out. Other spells affect a specific area defined in the spell's description.
